...
本文文档版本见首页修订记录；它与产品、模型、接口和测试报告的版本号分别管理。当前需求基线为 **REQ-BASE-2026-09-15**，含 74 条需求。原型交付以相互匹配的四项工程产物为组合：Buildroot/LoongArch 系统运行时与 Hybrid Rootfs、龙芯板端 LongPet 应用、ESP32-S3 Motion MCU 固件和 Family Desktop 家属端应用。人物视觉正式部署模型为 **Tinyissimo V1.2（FINAL_RECOMMENDED）**，V1.3 为扩域实验模型；文中运动功能测试报告的 V1.3 是另一份测试报告版本，不能与视觉模型版本混用。Vision Handoff 是视觉模型训练、评测及交接资料，LongPet 工程中保留训练代码。

本文定义各交付物之间的产品和接口关系。源码工程及配置承载具体实现；测试报告记录特定版本和条件下的执行结果；模型清单、训练及交接材料说明权重来源、版本和评测；《构建部署指南》承担构建、刷写、配置和现场检查步骤；用户手册承担老人及家属的实际操作说明。本文引用这些材料建立追踪关系，不能以设计文字代替测试结果，也不把尚待编制或补齐的配套材料视为已验收成果。

### 1.4 参考资料与证据来源

本文的事实基线主要来自第 3～18 章已整理的工程设计与验证记录，以及附录 A 的逐项追踪。评审或复测时，应按附录 A 的证据索引回到具体版本、测试环境和结果，而不是只引用本章的概括。当前主要材料及其用途如下。

| 材料 | 在本文中的用途与证据范围 |
| --- | --- |
| 《LongPet 产品运动功能测试报告》V1.3 | 对应已开放运动功能、MCU 停车和人物跟随的受控实机测试；58 项中 56 项通过，左右平移 2 项按版本计划暂缓。 |
| LongPet、Family Desktop 与 Motion MCU 工程的测试及接口资料 | 对应本地业务、语音、家庭连接、媒体、视觉协同和运动协议的软件或联调结果；以各记录的测试对象与条件为准。 |
| Buildroot/LoongArch 构建配置、Hybrid Rootfs 清单和校验资产 | 对应目标架构、依赖、ABI/ISA、基础文件保护、板端运行和系统产物完整性。 |
| Vision Handoff 训练与交接材料、LongPet 工程中的训练代码 | 对应 Tinyissimo 模型谱系、Frozen Test、新域评测、ONNX 导出和模型校验值。 |
| 本文附录 A、B、D | 分别索引需求—模块—验证关系、当前接口契约与硬件连接；附录 A 的证据定位不扩大原始记录的证明范围。 |

板卡规格和第三方组件来源在第 5、6、9 章按各自用途引用；附录 F 为参考资料的集中归档位置。相应用户手册与《构建部署指南》分别承担使用和交付操作说明，其编制与交付状态以各自文件为准，不能作为尚未执行的验证项目的通过证明。
...
老人的日常提醒、聊天和家庭联系往往分散在不同操作入口。若提醒只被触发却没有确认记录，家属难以判断事项是否完成；若管理界面过于复杂，老人又需要记住更多步骤。LongPet 因此把问题界定为：如何让老人以较少操作获得及时的提醒、陪伴和联系入口，同时让家属基于设备的真实状态参与照护，而不要求老人承担远程设置和设备维护。

项目还面对一个具体工程条件：本机交互与感知运行在单核、内存有限的 LoongArch 平台上，产品又带有可运动底盘。开放式对话、持续人物关注和家庭协作不能以牺牲本地基本操作或运动失效保护为代价。因此产品从一开始就区分本地基础能力、在线扩展能力与需独立授权的运动能力。

### 2.2 产品定位与目标

LongPet 定位为**面向老人日常陪伴与家属协同的可移动智能终端原型**，当前面向受控环境中的试用和验证。它把“看得见、听得懂、能联系、可确认”的日常体验放在首位：老人能直接找到说话、今日关怀和视频通话入口；家属能补充提醒、查看已记录的状态，并在需要时通话或人工协助；设备可在授权条件下关注并有限度地靠近人物。

产品遵循“**老人端做减法，家属端承担复杂管理；本地感知保证基础实时能力，在线 AI 提供开放式智能，独立运动 MCU 保证执行安全**”的分工。这里的“感知”指本机关键词识别与人物位置观测；它们在断网时仍可提供各自的基础能力。在线 AI 扩展自然语言问答与受限工具操作，不能绕过本地业务校验。运动须经过主控控制权判断和 MCU 执行侧保护，不能把人物检测结果直接等同于移动指令。

### 2.3 目标用户与使用环境

直接使用者是需要简明触摸、语音和家庭联系入口的老人；协同使用者是为老人设置事项、查看已记录关怀、接听通话和提供远程协助的家属。部署与维护人员负责网络、供电、模型及设备配置，不承担老人端的日常操作角色。

当前使用环境是设备正常供电、系统时间可信、摄像头和音频设备按需可用的室内原型场景。Family Desktop 通过受控家庭局域网连接设备；在线对话和天气更新还取决于相应外部服务可达。人物跟随与人工远控限定在**有人看护、低速、活动区域已确认**的条件下，使用者需要避开障碍、台阶和其他不适合移动的区域。原型未以无看护自主巡航或跨公网直连为使用前提。

### 2.4 用户痛点与核心价值

| 产品问题 | LongPet 当前给出的价值 | 不能误读为 |
| --- | --- | --- |
| 高频操作入口分散、状态难理解 | 首页直接提供说话、关怀、视频通话，并以表情、文字和触摸退出入口反馈当前状态。 | 已完成老年用户易用性量化验收。 |
| 事项提醒与实际完成容易混淆 | 本机统一保存提醒，触摸、家属端和在线语音共用规则；到时触发与老人确认完成分开记录。 | 设备自动确认用药、饮水或健康结果。 |
| 家属协助缺少同源信息 | 家属端读取设备的提醒、已记录关怀及真实能力状态，可管理事项、通话或受控远控。 | 远端拥有独立于设备的另一份权威业务数据。 |
| 在线服务或复杂操作不可用时体验中断 | 已设提醒、本地 KWS 快捷动作、触摸入口与已配置音频按本机条件继续可用。 | 离线开放式对话或离线通话。 |
| 机器人能看见人物却不能直接安全移动 | 视觉观测、家属授权、主控仲裁和 MCU 失效停车分层协作。 | 自主导航、避障或量产级整机安全保障。 |

**表 2-1 用户问题与当前产品价值。** 表中价值描述当前设计和已实现能力；不同功能的验证层级见第 17 章与附录 A。

### 2.5 典型使用场景

**日常提醒与关怀。** 家属在桌面端为老人设置事项，或老人自己通过触摸添加提醒；在线语音可在授权范围内创建、查询或删除。到时设备提示，老人确认后才形成完成记录。老人可记录饮水，家属刷新后查看同一设备上的今日关怀。断网时，已保存的提醒在供电、存储和本机时间有效的前提下继续调度。

**陪伴与快捷操作。** 老人触摸“陪我说话”或说出“小龙小龙”。在线服务可用时，设备完成转写、问答、受限业务操作和语音播报；不可用时，本地关键词可用于报时、进入提醒或联系家人页面、返回主页等快捷操作，并可播放已配置的陪伴音频。触摸入口始终提供相应的取消或返回方式。

**家庭联系。** 老人从首页向家属发起视频呼叫；家属也可发起语音或视频呼叫。设备在媒体准备就绪后显示接通，双方均可结束会话。通话期间，麦克风、扬声器和摄像头按会话占用，相关视觉推理与自动运动按规则暂停或关闭。

**人物关注与协助移动。** 家属可在 AI 视野中查看当前画面和有效人物框，选择关闭、仅头部或人物跟随。仅头部模式保持底盘不动；跟随模式经家属显式开启后，设备先确认目标与朝向，只在人物较远且安全条件满足时低速靠近。家属需要人工协助时，可取得单一远控会话，按住移动、松开停车；退出后跟随不会自行恢复。

### 2.6 产品形态、组成与交互方式

LongPet 的可见形态是一台带 7 英寸触控屏、前置摄像头、麦克风与扬声器、可左右转头和四轮底盘的原型设备。屏幕承担陪伴表情、状态、提醒和通话展示；语音减少菜单操作；人物视觉让设备可面向当前人物。家属使用 PC 上的 Family Desktop，查看设备信息并完成复杂管理。两端通过 FamilyLink 使用设备侧同一份业务数据，远程写入以设备确认为准。

| 组成 | 在产品中的职责 | 主要交互 |
| --- | --- | --- |
| 老人端 LongPet 设备 | 提供触摸、语音、提醒、今日关怀、通话与本地状态反馈；保存权威业务记录。 | 老人直接触摸或说话，设备按实际结果反馈。 |
| Family Desktop 家属端 | 承担提醒和设置管理、状态查看、AI 视野、通话、模式选择及人工远控。 | 在受控局域网内向设备请求、查看或提交意图。 |
| 本地感知与在线 AI | KWS 和人物视觉提供本地唤醒及有效人物位置；在线 Provider 提供开放式语音对话和天气更新。 | 感知结果、回答或工具建议进入设备业务规则，不直接取得运动权。 |
| 独立运动执行侧 | ESP32-S3 MCU 执行头部与底盘动作，并按模式、时效和故障状态撤销不再有效的运动。 | 接收龙芯主控经 UART 提交的受控意图，回报执行状态。 |

**表 2-2 产品组成与交互分工。** 主控运行在龙芯 2K0300 与 Hybrid Rootfs 上，系统实现细节见第 4～11 章。当前约 700～800 元仅为主要原型硬件成本估算，不是产品售价或量产 BOM。

这些能力围绕同一个原则协同：提醒、关怀和设置以本机数据为准；语音和家属端只是不同操作入口；AI 视野、自动跟头和跟随共用带时效的人物观测；通话优先取得所需媒体资源；人工远控优先于自动运动；实际底盘执行及失效停车由 MCU 再次约束。因此老人看到的反馈、家属看到的状态与设备当前可执行的动作应保持一致。
