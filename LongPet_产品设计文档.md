...
本章按真实业务动作串联第 7—11 章的服务、接口和安全边界。图中的实线表示请求、数据或控制意图的前向传递；文字所述状态回传是闭环成立的必要条件。功能入口和使用体验见第 8 章，模型细节见第 9 章，字段与协议见第 10 章及附录 B，MCU 内部保护见第 11 章。

### 12.1 业务流程设计原则与统一闭环

一个流程从老人触摸、家属操作、语音或环境变化开始，设备先识别输入和当前能力状态，再由业务入口选择服务。业务服务负责校验和保存，AI、媒体、视觉和运动只在获准后执行。异步结果、业务记录或 MCU 状态返回后，本机页面、表情、文字、语音或家属端才呈现结果；拒绝、超时和失联同样必须结束当前等待状态。启动阶段只决定哪些能力可用，不把可选 Provider、摄像头或运动链路的故障等同于整机业务失败；本地数据库无法打开则明确报告初始化失败。
...
*图 12-1 LongPet 统一业务闭环。* 数据流传递事实，例如声音、画面、提醒记录和 `TargetObservation`；控制流传递经校验的业务决定或运动意图。前者不能自动充当后者：收到人物框不等于允许底盘移动，发出运动命令也不等于已获 MCU 执行确认。

共同约束是：正常执行与失败均给出可感知反馈；异步回调只属于当前有效会话；旧版本、旧画面和旧目标不得继续写状态或参与控制；自动运动允许人工接管，执行类动作必须经过 LongPet 与 MCU 两级边界；外部 Provider 和网络故障局部降级；Camera、Mic、Speaker 等共享资源须明确获取、释放及异常退出路径。下文以这些约束检查每条业务链的起点、终点与状态来源。

### 12.2 提醒与今日关怀业务流程

触摸提醒页、家属端写入和在线语音工具是三种入口，最终都由设备上的提醒业务校验并写入同一份本地记录。家属端须等设备成功响应才显示保存成功，旧 revision 或不合法内容被拒绝并要求刷新；语音工具把结构化参数交给本地提醒服务，不直接写数据库。已保存提醒由本机调度，在设备供电、应用与本地存储正常且本机时间有效时，家属端断开或互联网不可用仍可到时触发。
...
*图 12-2 三入口统一提醒及关怀闭环。* “已触发”和“已完成”是不同状态；只有老人确认后才记录完成。今日关怀使用实际写入的饮水和提醒完成等事件，不把未自动采集的活动或健康指标表述为测量结果。AI Provider 失败只影响此次语音创建入口；触摸与已有提醒继续工作。若本机时钟无效或存储失败，则不能声称提醒已可靠保存或按时触发，应显示相应失败状态。流程终点是确认事件进入关怀摘要，或提醒保持未完成并等待后续处理。

### 12.3 在线 AI 语音交互与工具调用流程

本地 KWS 检出唤醒词、且在线能力可用时，语音入口申请音频所有权；KWS 暂停得到确认后才开启录音。触摸“陪我说话”也可手动发起在线尝试。录音结束后 ASR 输出文字，LLM 根据会话上下文形成普通回答，或提出受限工具调用。用户看到的主状态从待机进入倾听、思考、说话，再回待机；准备麦克风、识别、执行工具和失败状态以文字补充，不把尚未成功的 Provider 请求显示为操作完成。
...
*图 12-3 在线语音和 Tool Calling 时序。* 当前工具白名单为 `create_reminder`、`list_reminders`、`delete_reminder`、`get_current_time`、`get_current_date`、`open_page`；页面仅允许既有的 home、reminders、care、settings、companion。工具动作还受参数、对象和业务规则限制；没有天气工具，在线天气回答只能引用设备已有且附带时效的天气快照。LLM 不持有任意数据库、网络、通话或运动控制权。查询或删除提醒的结果须以本地服务的真实记录为准，创建成功后可由同一提醒列表及后续触发链验证。

会话使用 Session ID 隔离 ASR、LLM、TTS 和播放回调。取消会中止当前录音、Provider 与播报，并释放音频；迟到的旧回调被丢弃，不得覆盖新会话。工具调用还限制单轮数量与轮次，并按调用 ID 避免同一有效会话内重复执行；若本地动作已提交而导航或取消同步结束了会话，不把该动作回滚，也不继续替已取消的会话编造回答。ASR/LLM 网络、鉴权或超时失败转为当前会话错误；TTS 失败可保留已有文字回答并结束或重试。KWS 因音频独占暂不可监听时，触摸取消仍可使用；释放音频后再恢复监听。状态闭环以文字/播报和本地业务结果为准，而非以 LLM 输出一条工具请求为准。

### 12.4 离线语音交互与能力降级流程

在线能力不可用时，本地 KWS 仍可识别已配置关键词。唤醒词打开有限命令窗口，用户可直接说本地快捷词：报时、打开提醒、返回主页、进入联系家人或紧急页面，以及音量调整；“陪我说话”仅在已配置可用音频时播放本地片段。快捷动作由本机分发到页面或设置，反馈是页面变化、当前时间文字或音频播放结果。音频文件缺失、Mic/Speaker 忙或暂停确认失败时明确提示，不持续保持“正在倾听”。联系家人只是打开通话入口，通话仍依赖网络；紧急页面并不自动报警。
...
flowchart TD
    ...
    N -->|否| L[本地命令窗口与已配置关键词]
    L --> Q{快捷动作或陪伴音频?}
    Q -->|快捷| P[本机页面、时间或音量动作] --> I
    Q -->|陪伴| A[申请音频；播放已配置片段] --> I
    Q -->|无效、缺失或忙| E[说明当前可用范围或失败原因] --> I
    N -. 网络和 Provider 恢复后，下一次唤醒重新判定 .-> O
```

*图 12-4 在线与离线语音分流。* 离线语音不包含完整本地 ASR、LLM、TTS，也不保证任意自由问答。触屏语音入口会提示在线不可用原因并可手动重试；它不会自动变成离线大模型。网络和 Provider 恢复后，能力判定允许后续唤醒重新进入在线会话，已保存的本地提醒无需重新创建。

### 12.5 人物感知、自动跟头与人物跟随流程

Camera 以共享采集源提供画面，Vision 在独立推理路径中形成 `TargetObservation`，包含人物存在、跟踪状态、位置和时效。这个观测是数据，不是电机命令。它同时供 AI 视野绘框、仅头部关注和人物跟随策略消费；后两者只在家属选择相应模式、观测新鲜、Motion 状态允许时提出控制意图，再由 MotionService 经 UART 送 MCU。MCU 后续 `STATUS` 的模式、运动、故障和头偏返回 LongPet，供策略下一步判定与家属状态显示。
...
flowchart TD
    ...
    MS -->|TARGET / FOLLOW_MOVE| U[UART → Motion MCU]
    U --> E[头部舵机 / 底盘]
    U -->|STATUS| MS
    MS --> S[模式、头偏、运动与故障反馈]
    S --> FP
...
*图 12-5 单份视觉观测到三个消费路径。* 仅头部模式发送有效目标以调整舵机，轮组保持停止。人物跟随先确认稳定目标，再依据 MCU 报告的物理头偏决定原地转向；头身对齐且画面距离代理为 FAR 时才低速前进，GOOD 或 NEAR 时停车。人物框高度只是粗略距离代理，不提供障碍或悬崖识别。

目标丢失、观测过期或 Camera 中断时，服务不再使用旧位置，跟随进入丢失/等待状态，停止新的有效移动续租并在可达时发送跟随停止和丢失目标；若上层通信失效，MCU 的独立时效保护仍使底盘最终停车。重新检测后要重新确认目标与状态才恢复自动动作。视觉模块不直接驱动电机，也不执行主动绕障、倒车搜索、SLAM 或身份重识别。

### 12.6 家属端 AI 视野与人工远程控制流程

家属端经 FamilyLink 建立独立的视觉监看会话，收到共享相机 JPEG 与有效目标元数据后显示实时画面、人物框和感知状态。画面和目标可能在不同时间产生，客户端按新鲜度决定是否绘框；无新观测隐藏旧框，断流清除旧画面。关闭 AI 视野只释放该远端消费者，不关闭设备本地 Vision。监看可以在本地推理未启用时仅显示画面，不把“无框”误报为“无人”。

人工远控属于另一条控制链。家属在 AI 视野中请求短时远控会话，LongPet 校验身份、单一控制者、UART、MCU 在线及故障状态，先停止自动动作再进入 MANUAL；PERSON_FOLLOW 被关闭。按住底盘方向键时 Family Desktop 周期刷新意图，LongPet 在远控租约有效时经 MotionService 发送 `MOVE`；松键、失焦、页面隐藏或退出时停止刷新并在通道可用时发 `STOP`。头部按住期间按受限步长调整，松开后保持位置。窗口失焦或异常断连即使停止消息无法送达，远控与 MCU 动作租约到期仍使底盘停止。退出接管后人物跟随必须由家属显式重新开启；仅头部模式可在条件重新满足后恢复并等待新目标。
...
flowchart LR
    ...
    FD -->|意图 / STOP / release| FL --> ARB[LongPet MotionService 仲裁]
    ARB -->|UART 命令| MCU[Motion MCU] --> ACT[电机 / 舵机]
    MCU -->|STATUS 执行状态| ARB -->|motion_status| FL --> FD
```

*图 12-6 AI 视野数据链与远控控制链。* Family Desktop 不直接连接 Motion MCU。远控页面以设备回传的 MANUAL、在线和无故障状态开放控制；发送成功只说明意图进入链路，实际执行和停车以 MCU `STATUS` 为准。连接中断时页面清除“在线可控”状态，不能将最后一帧运动状态继续显示为实时结果。

### 12.7 音视频通话与媒体资源生命周期

通话既可由家属发起语音或视频呼叫，也可由老人从设备发起视频呼叫。信令建立当前 `callId` 与修订状态，来电端显示提示或响铃；设备发起的呼叫由家属接听/拒绝，等待中可取消。进入媒体准备时，通话申请独占音频，等待 KWS 暂停确认，按语音/视频模式开启相应资源；视频同时复用共享 Camera。设备的来电提示在音频可用后播放，媒体真正就绪才将状态置为已接通。任一端挂断、拒绝、媒体失败或连接断开均终止当前会话；旧 `callId` 或 revision 的操作不能修改新会话。
...
*图 12-7 通话会话与资源生命周期。* 通话活动期间 Vision 推理和 HEAD_ONLY 暂停，PERSON_FOLLOW 关闭；共享相机的采集所有权由消费者计数管理。通话结束先停止媒体与来电提示、释放 Camera 和音频，再恢复 KWS；Vision 可用时等待新观测，仅头部在安全条件满足时恢复，人物跟随不会自动恢复。异常断线走相同释放路径，避免 Mic、Speaker、Camera 或 Socket 被永久占用。接通和最终结束状态同时反馈本机与家属端，媒体未就绪时不得显示“已接通”。

### 12.8 多功能并发、资源占用与控制权仲裁

并发协调以设备端为准。Camera 允许多个消费者从一处采集，但通话期间暂停 Vision 推理；Mic/Speaker 由音频所有权管理，KWS 先暂停并确认；底盘与头部统一经 MotionService 和 MCU 模式约束。Network 是共享传输条件，不授予远端直接控制设备的权利。表 12-1 给出冲突发生时当前的让渡和恢复条件。

| 冲突场景 | 优先能力 | 被让渡能力 | 恢复条件 | 设计原因 |
| --- | --- | --- | --- | --- |
| 人工远控接管自动跟头/跟随 | 已认证的单一 MANUAL 会话 | 自动运动先停车；跟随关闭 | 人工释放后仅头部可按条件恢复；跟随须显式重开 | 避免两个来源同时控制电机 |
| 通话占用 Mic/Speaker | 已取得音频所有权的通话 | KWS 监听及新的 Voice/陪伴请求 | 通话释放资源、经过恢复冷却；新请求再判定能力 | 避免音频设备争用；语音业务忙时通话也会被拒绝而非强抢 |
| 在线 Voice 与本地陪伴 | 当前音频 owner | 后发会话等待或收到忙提示 | 当前会话取消或播放结束并释放音频 | 保证录放与旧结果不会交叉 |
| 通话视频与 Vision/AI 视野 | 通话视频取得共享 Camera；Vision 推理暂停 | 自动跟头暂停、人物跟随关闭；AI 视野只显示仍有效的数据 | 通话结束后 Vision 重获新观测；跟随显式重开 | 共享采集并避免通话时自动运动 |
| AI 视野查看与本地 Vision | 共享同一相机帧 | 无需关闭本地推理；各自释放消费句柄 | 关闭视野只结束远端会话 | 避免重复采集与页面退出误停本地感知 |
| 新 Voice Session 与旧 Provider 回调 | 当前 Session ID | 旧 ASR/LLM/TTS/播放结果丢弃 | 新会话独立完成 | 防止旧结果覆盖页面或触发动作 |
| STOP 与任意持续底盘动作 | STOP 与 MCU 安全约束 | 当前 MOVE/FOLLOW_MOVE 授权撤销 | 重新满足模式、会话、目标和安全条件后才可动作 | 停车是高优先级控制语义，且不解除故障锁存 |

*表 12-1 主要共享资源与控制权仲裁。* STOP 用于立即撤销底盘驱动及移动授权，头部保持当前位置；网络断线时无法保证 STOP 送达，独立租约提供最终停车。人工远控、自动跟随、仅头部和安全模式的具体许可与故障恢复规则见第 11 章。

### 12.9 异常、断连与安全降级流程

异常按能力域处理：先撤销当前不可验证的结果或控制权，再给出真实状态，最后在条件恢复后重新获取会话、资源或新鲜数据。表 12-2 的“恢复”不表示自动恢复已被明确关闭的人物跟随或解除 MCU 故障锁存。
...
| Weather Provider 失败 | 保留本次运行已有快照及获取时间；过期则标记旧数据，首次无数据为未知，不声称实时更新 | 本地业务继续；网络与配置恢复后补刷 |
| Vision 观测丢失、过期或 Camera 中断 | 清旧人物框与控制目标；自动跟随停止续租并在可达时发停车；家属端显示搜索/丢失或断流 | 提醒、语音及触摸继续；Camera/Vision 恢复后等待新目标确认 |
| FamilyLink/远控通道断开、窗口失焦或页面退出 | 停止刷新意图，能送达时发 STOP/release；家属端撤销在线可控显示 | 已保存本地提醒照常工作；重连后重新取状态与会话，MCU 超时保护最终停车 |
| UART 写入失败、MCU 离线或故障 | Motion 状态标记不可用/未确认，撤销人工或自动控制；不把发送命令视为已执行 | Reminder、Weather、UI、Voice 不依赖运动运行；链路恢复后重新授权，故障须按第 11 章排障复位 |
...
| 旧 Session、旧 revision、旧 callId 或旧视觉帧 | 丢弃回调、拒绝写入或隐藏过期框；提示刷新/重试，不让旧数据改变当前状态 | 获取新快照、新会话或新鲜观测后继续 |
| 本地数据库无法打开或迁移失败 | 初始化明确报错，不显示提醒保存成功或伪造关怀记录 | 修复存储或版本后重启；此项不是可悄然降级的可选故障 |

*表 12-2 典型异常与局部降级矩阵。* LongPet 通过业务边界将故障限制在相关能力域；运动、媒体和异步结果则额外受会话、资源和时效约束，避免一个失效输入造成持续控制或整机级联失效。

### 12.10 典型流程与需求、模块和验证映射

表 12-3 直接引用第 3 章和附录 A 的需求 ID。UT 表示单元验证，ST 表示软件集成验证，BT 表示板端验证，HT 表示硬件/实机验证，E2E 表示双端或端到端验证；这些是验证类型，不另造测试用例编号。具体已归档状态与证据位置以附录 A 为准。

| 典型流程 | 主要需求 ID | 主要模块 | 关键接口或状态 | 主要验证方式 |
| --- | --- | --- | --- | --- |
| 三入口提醒及今日关怀 | FR-REM-01、FR-REM-03、FR-REM-04、FR-CARE-01、FR-CARE-02 | 提醒业务、关怀业务、FamilyLink、语音工具 | 本地提醒/事件、revision、设备确认 | UT、ST、E2E |
| 在线 Voice 与 Tool Calling | FR-VOICE-02、FR-VOICE-03、FR-VOICE-06、FR-VOICE-07、NFR-SEC-02 | KWS、Voice Session、Provider、Tool Registry、本地业务 | Session ID、Tool Result、音频 owner | UT、ST、BT、E2E |
| Offline Voice | FR-VOICE-01、FR-VOICE-05、FR-UI-04 | KWS、命令分发、本地陪伴、页面 | 本地关键词、音频资源状态 | UT、ST、BT |
| Vision 观测与 AI 视野 | FR-VISION-01、FR-VISION-02、FR-VISION-04、FR-MM-02 | Vision、共享 Camera、FamilyLink、Family Desktop | TargetObservation、JPEG、观测时效 | UT、ST、BT、E2E |
| 仅头部自动关注与人物跟随 | FR-VISION-03、FR-VISION-05、FR-VISION-06、FR-MOTION-05 | 自动视觉策略、MotionService、Motion MCU | TARGET、FOLLOW_MOVE、STATUS | UT、ST、HT、E2E |
| Family Remote Control | FR-FAMILY-04、FR-MOTION-03、FR-MOTION-04、FR-MOTION-08、FR-MOTION-09 | Family Desktop、FamilyLink、MotionService、Motion MCU | 远控会话、MOVE、STOP、motion_status | UT、ST、HT、E2E |
| 双向音视频通话 | FR-CALL-01、FR-CALL-02、FR-CALL-03、FR-MM-03、FR-MM-04 | 通话业务、媒体协调、共享 Camera、Family Desktop | callId/revision/mediaReady、媒体会话 | UT、ST、E2E |
| 异常与局部降级 | FR-WEATHER-01、FR-WEATHER-02、NFR-STAB-01、NFR-STAB-02、CON-RES-01 | 天气、Voice、Vision、媒体、Motion、业务 UI | stale、Session、资源 owner、STATUS | UT、ST、BT、E2E |

*表 12-3 典型业务流程追踪表。* 验证应观察输入、状态变化、执行结果和用户反馈四处证据；运动类还须观察 MCU 回传或安全停车，不能只以主控命令发送成功判定闭环。
