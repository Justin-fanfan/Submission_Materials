...
- 13 非功能需求落实与工程化设计
  ...
  - 13.7 网络、数据与隐私保护
  - 13.8 可维护性与模块化
  - 13.9 可扩展性与 Provider / Port 设计
  - 13.10 可测试性与可复现性
- 14 构建、部署、发布与运行维护
  - 14.1 交付与部署总体流程
  - 14.2 LoongArch 基础系统与 Hybrid Rootfs
  - 14.3 LongPet 主程序构建与部署
  - 14.4 AI Runtime 与模型部署
  - 14.5 Motion MCU 固件构建与烧录
  - 14.6 Family Desktop 构建与部署
  - 14.7 配置管理与首次启动
  - 14.8 服务启动、运行与恢复
  - 14.9 版本、校验与交付物管理
  - 14.10 发布与运行维护
  - 14.11 后续 LoongArch AI 推理基础设施扩展
- 15 关键技术难点与方案权衡
  - 15.1 LoongArch AI 软件栈兼容与数值正确性
  - 15.2 Buildroot 新软件栈与稳定板端系统
  - 15.3 视觉模型选型、领域适配与成本约束
  - 15.4 低频 Detector 与 Sparse Tracker 协同
  - 15.5 单核资源与共享媒体设备竞争
  - 15.6 在线智能与离线基础能力的分工
  - 15.7 Linux 智能主控与 MCU 安全执行的分层
- 16 技术创新与应用价值
  - 16.1 无 LSX/LASX 平台的可验证 AI Runtime
  - 16.2 保留稳定 BSP 的 Hybrid Rootfs 交付方法
  - 16.3 面向持续感知的轻量检测与跟踪
  - 16.4 AI 感知与运动执行安全分层
  - 16.5 老人端、家属端与 AI 的协同体验
  - 16.6 可演进的 LoongArch AI Provider 架构与应用价值
- 17 当前成果、边界与后续规划
  - 17.1 当前完成成果
  - 17.2 当前验证状态
  - 17.3 当前产品边界
...
第 3.11 节给出非功能需求及验收口径，本章说明这些要求在当前原型中如何落到架构、资源调度、运行状态和验证资产。工程上的共同原则是：允许外部 AI、网络或可选设备局部降级，但不能使独立的本地业务失去控制；核心数据库和运动安全则按更严格的失败规则处理。

### 13.1 工程化目标与设计原则

2K0300 为单核 LoongArch 主控，内存和多媒体资源有限；网络与 AI Provider 可能失效，Camera、Mic、Speaker 被多业务共享，运动执行又具有安全后果。因此将同步、可确定的提醒与本地控制保留在设备内，把在线智能封装为可取消的外部请求，把摄像头和音频交给会话协调，把底盘最终安全约束交给 MCU。任何能力的“已实现”不等于全部并发、长期运行或量产安全均已验证，验收范围仍以第 3 章及附录 A 为准。

| 非功能目标 | 当前工程机制 | 主要证据与边界 |
| --- | --- | --- |
| 本地响应与有限资源 | UI/业务流程与 Provider 请求分离；Vision 处理最新帧，Detector 低频、Tracker 更新目标；共享媒体所有权 | 第 7.10、9.4—9.6 节；在线时延与整机长稳尚需持续量化 |
| 局部故障隔离 | Provider、天气、Vision、FamilyLink 分别反馈失败，独立提醒继续运行；核心 SQLite 初始化失败则显式停止启动 | 第 10.10、12.9 节及附录 B.7 |
| 适老可用与安全 | 1024×600 大入口、状态/表情反馈、触摸与语音互补；MotionService 与 MCU 双层约束 | 第 8.2、11 章；适老理解性专项记录待补 |
| 数据与维护 | 敏感配置外置、会话数据限时、Service/Repository/Port/Adapter/Provider 分层 | 第 7、10 章；权限与跨模块诊断专项验收待补 |
| 可复现交付 | 源码及版本基线、模型 hash、Hybrid Rootfs 审计、软件/板端/实机分层测试 | 第 6、9、11 章及附录 A；不承诺跨主机逐字节一致构建 |

**表 13-1 非功能需求落实矩阵。** 各机制对应第 3.11 节的 PERF、STAB、USAB、SAFE、SEC、MAINT 和 REPRO 要求，不替代逐项需求追踪。

### 13.2 性能与资源约束落实

界面事件、提醒查询与取消入口不等待外部 ASR、LLM、TTS 或天气请求完成。Provider 请求由会话标识关联，超时、失败和取消返回明确状态，旧回调不得覆盖新会话。摄像头输入由 Vision 工作线程取最新帧；持续逐帧运行较重模型会使检测结果落后于现实，因此采用 Tinyissimo 人物检测与 Sparse LK 跟踪，检测用于搜索、重获和周期纠偏，跟踪负责检测间隙的位置更新。板端 detector-only 与 Detector+Tracker 的测量口径、结果和并发条件已列于第 9.4 节，这里不将观测更新频率误写成 CNN 推理帧率。

KWS 与 Vision 会在单核上竞争；通话、录音、播报和视觉也涉及共享设备。MediaSessionCoordinator 按会话转交音频所有权，KWS 暂停并确认后才进入录音；视频通话让渡摄像头并暂停或重置相关 Vision/自动运动，AI 视野则复用共享画面。资源不足时降低可选感知或在线能力，优先保留页面、提醒和停车保护。当前并发结论仅适用于第 9.6 节已验证工况。

### 13.3 稳定性与故障隔离

故障按“识别来源—局部降级—状态反馈—条件恢复”处理。AI Provider 超时或限流只结束当前在线语音会话，TTS 失败时可保留文字结果；天气失败标记缓存时效；Vision 无新鲜观测时隐藏旧人物框并停止依赖目标的自动控制；FamilyLink 断开使家属端标记离线，不删除设备上的提醒。Motion UART 或 MCU 状态无效时，人工/自动运动退出并撤销授权，UI、语音及提醒仍可按自身条件工作。核心 SQLite 无法打开或迁移失败属于启动前提失效，应明确报告并停止初始化，不能把空数据当作正常照护状态。异常语义见附录 B.7。

### 13.4 可恢复性与服务生命周期

systemd 负责 LongPet 开机启动和异常退出后的重新拉起，Wi-Fi 条件式恢复服务处理已知驱动探测失败。应用内的 KWS 独立进程、Voice Session、Camera/Media Session 和远控会话分别管理启动、暂停、取消、断连与资源释放；会话 ID、版本号及目标时效约束阻止旧异步结果复活。重启后的语音、通话、视觉和运动会话不继承旧授权，运动恢复需重新满足 MCU 状态和控制模式条件。异常关机后，本地持久数据依靠 SQLite 恢复；提醒正确调度仍以本机时间可信为前提，RTC 断电保持尚待复测。服务恢复机制与已验证范围见第 6.9—6.10 节。

### 13.5 适老性与易用性落实

老人端面向 1024×600 触控屏使用可见的说话、关怀和通话入口、大尺寸操作目标及浅层导航，避免将部署参数置于高频页面。等待、失败、不可用、结束等状态通过文字、界面与表情反馈，操作可退出或重试；触摸和语音互补，离线快捷动作减少断网时的操作断层。家属端承担提醒编辑、设备状态查看和远程协同等复杂管理。实现与页面检查已形成证据，但老年用户对异常提示的理解性及任务成功率仍需专项使用测试，不能由界面实现直接推断适老效果。

### 13.6 运动安全与执行边界

LongPet 主控决定跟头、跟随和人工接管等高层行为，MotionService 校验会话、控制权、目标新鲜度和 MCU 状态；ESP32-S3 固件独立校验协议、模式、命令时效与故障状态，超时或失联撤销底盘驱动。Vision 与 LLM 的输出本身不具有运动授权。STOP 将 TB6612 STBY 拉低使底盘自由滑行，头部舵机仍带电；实际停距没有由软件保证。V1.3 运动报告对已开放功能给出实机证据，左右平移两项主动暂缓。物理急停、独立限流、完整电源保护、机械防护与认证属于后续产品化要求，不能由当前软件 fail-safe 代替。

### 13.7 网络、数据与隐私保护

提醒、照护事件和设置按第 10.6 节持久化；有限 AI 对话上下文、天气快照及通话、视觉、远控会话属于进程或会话态，重启后不恢复旧控制权。音频和摄像头数据随当前任务采集、传输与释放，家庭媒体流使用短时会话令牌；长期 FamilyLink Bearer 留在受控配置及 Electron 主进程，普通连接状态仅暴露是否配置令牌。AI/天气 Key、Token 由外部配置或 service drop-in 提供，不写入文档和业务响应。受控局域网的 HTTP/WS 鉴权不能等同于传输加密，当前通道不直接暴露公网；跨不可信网络需要后续加密或可信中转。配置访问权限和凭据保护专项验收仍待归档，系统不宣称医疗诊断或全面隐私合规认证。

### 13.8 可维护性与模块化

Controller 接收页面与远程操作，Service 维护业务规则，Repository 管理持久数据，Port 定义能力边界，Adapter 对接设备/协议，Provider 封装外部服务。同一提醒或设置规则由本地、语音和家属入口复用，故障可从业务状态、Provider 响应、媒体会话、UART/MCU 状态与 systemd 日志定位。LongPet、Family Desktop、Motion MCU 和 Buildroot/LoongArch 四工程各自交付应用、家属端、执行固件和系统运行时；模型、设备与服务参数由配置解耦。跨模块诊断专项记录仍是附录 A 所列待补项。

### 13.9 可扩展性与 Provider / Port 设计

现有 ASR、LLM、TTS Provider 抽象与设备 Port 使业务逻辑不直接绑定供应商协议或摄像头/音频具体节点。更换已支持 Provider 或 Adapter 时，应保持 Service 的输入、结果、取消和错误语义；接入新的硬件或 AI 服务还需完成协议适配和验证。系统在 AI Provider 层预留自建 LoongArch AI 推理服务接入边界；远程/局域网服务器与本地 AI 算力板插件是两种后续形态，当前仅有终端侧抽象，并未完成服务器或算力板产品交付，均不纳入本次功能验收。

### 13.10 可测试性与可复现性

软件测试验证业务规则、接口状态和取消/失效语义，板端测试验证 LoongArch 依赖、模型数值与资源表现，实机测试验证运动动作、停车和双端链路；不同层级证据不互相替代。Vision V1.2 Frozen Test、PyTorch/ONNX parity、正式/实验模型清单与 SHA-256 固定模型谱系；ONNX Runtime 算子参考比对防止“可加载但数值错误”；Hybrid Rootfs 的组件清单、依赖闭包、ISA/ABI 审计及受保护基础文件 hash 保证部署输入可核对。UART 与 FamilyLink 协议基线、服务配置和附录 A 的需求—模块—证据矩阵共同支撑复测。第三方库及 KWS 权重的再分发许可仍需在正式发布前核验，不把模型复现性等同于许可完备。

## 14 构建、部署、发布与运行维护

本章描述从四工程源码形成可运行原型的交付链和检查点。具体命令、板卡刷写步骤及现场参数由独立《构建部署指南》承载；本章规定版本关系、验证顺序和运行边界。

### 14.1 交付与部署总体流程

交付顺序为“源码与工具链基线 → Buildroot 用户态组件 → Hybrid Rootfs → LongPet 主程序 → AI Runtime/模型 → 设备配置 → Motion MCU 固件 → Family Desktop → 软件、板端与实机联调 → 发布包”。其中稳定板端基础 rootfs、内核和 ramdisk 是 Hybrid 合并的输入，不是从本轮 Buildroot 整体替换而来。四工程分别产生可追溯产物，最后由集成验收确认它们的接口和配置相互匹配。

| 交付层 | 关键输入与产物 | 放行检查 |
| --- | --- | --- |
| 基础系统与运行时 | 稳定板端系统；Buildroot 2024.08 的 Qt/AI/媒体组件；Hybrid Rootfs | ABI/ISA、依赖闭包、受保护文件及 SHA-256 |
| LongPet 与模型 | LoongArch 应用、systemd 服务、配置、KWS 与正式 Vision ONNX | 可启动、模型路径/hash、设备访问、业务回归 |
| Motion MCU | ESP32-S3 固件、串口协议基线 | 烧录版本、STATUS、SAFE/STOP、失联停车 |
| Family Desktop | 桌面应用及 FamilyLink 配置 | 鉴权、状态/提醒、AI 视野、通话和受控远控 |
| 集成发布 | 上述产物、测试记录、清单和回退资料 | 版本一致，按证据等级记录通过范围 |

**表 14-1 从源码到发布包的交付检查链。**

### 14.2 LoongArch 基础系统与 Hybrid Rootfs

系统方案以已验证的板端 rootfs、内核、驱动和核心运行库为稳定基线，从 Buildroot output-qt6/target 选择性合入 Qt6、Python/NumPy、OpenCV、ONNX Runtime、sherpa-onnx、FFmpeg/GStreamer 等用户态组件。components.yaml 限定合入范围与必需项；自动检查新增 ELF 的 DT_NEEDED 依赖闭包，显式补充 Python 动态加载库与 GStreamer 插件。合并前后核对 glibc 2.38、GLIBC/GLIBCXX/CXXABI 符号要求、LoongArch ELF 架构、目标 CPU 不支持的 LSX/LASX 指令，以及内核、模块、动态加载器和核心库的受保护 SHA-256。UTF-8 locale、归档权限/属性与最终 rootfs、内核、ramdisk 的 SHA256SUMS 同属放行条件。第 6.8 节给出检查范围与已有记录；默认 ISA 扫描针对新增 ELF，不能据此声称所有外部二进制均已覆盖。

### 14.3 LongPet 主程序构建与部署

LongPet 使用与 Hybrid Rootfs 匹配的 LoongArch 交叉工具链和 Qt 运行库构建。部署包包含应用可执行文件、所需资源、longpet.service 与运行配置示例；安装时将可写数据库路径、模型路径、设备节点及服务用户权限分开设置，不把密钥编入二进制。板端先检查 ELF/动态库与 Qt 平台插件，再验证触控 UI、SQLite schema 初始化、本地提醒、网络和服务启动。服务依赖的 display、audio、camera 与 UART 节点以本机枚举和第 10.9 节配置为准。

### 14.4 AI Runtime 与模型部署

本地 KWS 依赖 Python/NumPy、音频采集组件和标量适配后的 ONNX Runtime；正式视觉模型为第 9.3 节的 V1.2 person-only ONNX，由 CPU Execution Provider 在板端执行。部署时把 KWS 声学文件、token/词条配置与视觉模型路径列入清单，逐一核对 SHA-256、运行库实际加载位置和模型导入/推理结果。V1.3 视觉模型保留 EXPERIMENTAL 标识，不覆盖 V1.2 正式部署文件。模型训练过程、冻结集及 ONNX parity 证据按 Vision Handoff 和第 9 章追溯；部署只引用已核准模型。

### 14.5 Motion MCU 固件构建与烧录

Motion MCU 从独立源码按 ESP32-S3/Arduino 构建配置生成固件，记录固件版本并烧录至实物。接线与 UART 参数依据第 11 章和附录 D 核对，先在无负载或安全台架确认上电 SAFE、STATUS、STOP、协议拒绝和通信失效停车，再进入低速落地与主控联调。固件版本与 LongPet 协议版本须成对放行；新固件不能仅因编译和烧录成功就视为运动安全通过。当前 V1.3 报告的 56/58 通过范围见第 11.10 节。

### 14.6 Family Desktop 构建与部署

Family Desktop 独立构建桌面应用，配置设备地址、FamilyLink 长期凭据和受控局域网连接。部署验收先检查状态读取、提醒/设置修订冲突与错误反馈，再检查 AI 视野、音视频通话及远控短时会话的建立、断开和资源释放。家属端仅在设备端真实可用时展示可执行控制，人工远控仍由 LongPet 与 MCU 联合裁决；配置与联调协议以第 10 章和附录 B 为准。

### 14.7 配置管理与首次启动

首次启动依次核对显示/触摸、网络与时间、摄像头、USB 音频、UART、数据库、KWS/视觉模型、systemd 状态以及 FamilyLink 可达性。AI 与天气配置、FamilyLink Token、数据库与设备路径、Vision/Motion 开关分别由受控配置提供；启用远程监听须先配置 Token。检查不仅看进程存在，还需确认提醒可读写、模型可导入、媒体资源可交接、MCU STATUS 合法及断连后停车。若本机时间不可信，提醒绝对时间的正确性不能直接验收。

### 14.8 服务启动、运行与恢复

systemd 管理 LongPet 自启动、日志和异常退出重启，Wi-Fi 条件式恢复服务处理已知驱动故障；板端启动后确认服务用户与设备权限、Qt 显示和网络状态。运行期间以日志和业务状态区分 Provider 失败、媒体占用、模型路径错误、FamilyLink 离线与 MCU 故障。重启不复用旧 AI、通话或运动会话，摄像头与音频失败后释放或明确标记不可用；故障解除后由新会话恢复。数据库初始化失败则明确停启并修复存储或 schema，不以空白业务数据继续运行。

### 14.9 版本、校验与交付物管理

一次发布由四工程源码版本、Buildroot 配置与工具链、稳定 rootfs/内核/ramdisk、Hybrid 合并清单与报告、LongPet 应用和 service、AI Runtime 与模型、MCU 固件、Family Desktop、配置模板、SHA-256 manifest 和对应测试记录共同定义。发布清单记录版本标识与产物 hash，不记录可追溯个人身份的 Git 作者信息，也不包含真实 API Key/Token。只替换模型、固件或 Provider 也必须更新受影响的清单及回归结论；回退时成组恢复接口兼容的应用、模型、系统和 MCU 基线。

### 14.10 发布与运行维护

当前运行维护面向本地原型设备，不按云 SaaS 运维描述。发布前归档构建输入、校验报告、配置模板与验收记录；现场维护关注本地数据备份、日志、服务健康、模型与 Provider 版本、设备节点及网络状态。升级先备份 SQLite 与配置，再核验新包 hash、兼容性和可回退基线；模型替换须复核冻结域及新域表现，固件变更须复做安全项。用户反馈应记录软件版本、板端环境和复现条件，不能用无条件“自动恢复”掩盖需要现场处理的电源、存储或硬件故障。第三方组件及模型权重许可清单在正式分发前完成核验。

### 14.11 后续 LoongArch AI 推理基础设施扩展

后续可以将计算密集型 ASR、LLM、TTS 或视觉服务部署为局域网/远程 LoongArch AI 推理服务器，由 LongPet 经网络 Provider 接入；也可以增加本地/近端 LoongArch AI 算力板，经本地接口或网络接口提供推理服务。两条路线共用终端的 AI Provider/Service 边界，使提醒、界面和运动业务无需直接绑定新后端；实际接入还需定义鉴权、延迟、离线回退、算力与成本等验收条件。当前交付仍以已有在线 Provider、本地 KWS 和本地视觉为主；自建服务器与算力板均未作为正式能力部署或验收。

## 15 关键技术难点与方案权衡

技术取舍围绕目标板算力、稳定基础系统、交互连续性、运动安全和原型成本展开。表 15-1 概括方案选择；以下各节说明尝试为何不足、最终方案解决了什么，以及仍须接受的代价。

| 难点 | 已尝试或比较的路线 | 当前选择 | 代价与边界 |
| --- | --- | --- | --- |
| 无 LSX/LASX 的 AI 栈 | 复用现成二进制或原有 ORT 路径 | 源码构建、标量 MLAS、数值/ISA 审计 | 标量性能受限，升级需重验 |
| 新软件栈与稳定 BSP | 整体替换基础系统 | 保留板端基线，选择性 Hybrid 合入 | 合并规则和 ABI 审计复杂 |
| 人物视觉 | 重型姿态/关键点与较大检测模型 | Tinyissimo person-only，V1.2 域微调 | 任务和视角受限 |
| 持续观测 | 全帧 CNN 检测 | 低频 Detector + Sparse LK Tracker | 遮挡或检测阶段有更新空档 |
| 单核并发 | KWS、Vision、媒体全时运行 | 资源所有权与按场景让渡 | 通话期间部分感知暂停 |
| 开放式语音 | 完整本地 LLM 或完全在线 | 本地 KWS/有限离线 + 在线 Provider | 开放式能力依赖网络 |
| 运动执行 | Linux 智能决策直接驱动 | MotionService + 独立 MCU 安全状态机 | 增加固件、协议与联调成本 |

**表 15-1 关键技术难点、方案与保留代价。**

### 15.1 LoongArch AI 软件栈兼容与数值正确性

2K0300 无 LSX/LASX，部分同为 LoongArch 的现成包或运行时路径并不适合直接使用。起初以能交叉编译、加载 ONNX Runtime 和导入 Python/NumPy 为可运行标志，但板端语音样例出现数值异常，说明“加载成功”不足以证明推理正确。可选做法是继续依赖原有运行时路径、放弃本地模型，或在固定工具链下从源码适配并逐层验证。项目选择后者：NumPy 等组件源码构建，ONNX Runtime 1.17.1 对目标构建启用一致的标量 MLAS 路径，并检查 LoongArch ELF、ABI、16 KiB 可加载段对齐、实际加载库和新增产物的 ISA。第 6.5 节记录了已定位的打包布局与标量内核不匹配及十项算子、语音样例的板端比对；不将这一定位外推为所有 LoongArch 软件的统一故障原因。代价是源码补丁和标量执行维护成本，后续升级必须重新做数值与指令审计。

### 15.2 Buildroot 新软件栈与稳定板端系统

Qt6、AI 和媒体组件需要较新的用户态依赖，但直接用 Buildroot 生成的系统完整替换板端基础系统，在早期实机测试中未达到稳定性要求。完全停留在旧系统会阻碍应用功能，整机重刷则可能同时改变已验证的内核、驱动和运行库，扩大故障面。因此采用 Hybrid Rootfs：稳定板端 rootfs、内核和 ramdisk 保持基线，Buildroot target 只按清单提供新增用户态组件。合并过程审计 DT_NEEDED、动态加载插件、glibc/GLIBCXX/CXXABI、LoongArch ISA、locale 与受保护文件 hash，结果见第 6.8 节。代价是合并和回归检查比单一构建树更复杂；此方案保证的是当前输入组合经检查可部署，并非任意版本包均可混合。

### 15.3 视觉模型选型、领域适配与成本约束

早期 YOLO11n-Pose、手部 Landmark 等方案在目标板上达到数秒一帧，不适合作为持续人物交互入口；较轻的 FastestDet 仍慢于 Tinyissimo 基线。方案比较并不只看通用识别能力，还要考虑 128×128 输入、单核推理和普通 USB 摄像头的成本与延迟。项目选用 TinyissimoYOLO-v1-small 的 person-only 结构，从 COCO 人物数据训练，再用固定机位实拍图进行 V1.2 域微调。第 9.3 节的 Frozen Test 与 ONNX parity 支持正式 V1.2；V1.3 扩域实验改善新场景但降低原冻结域部分检出表现，因此未替换正式模型。代价是不能宣称通用姿态、身份或环境识别，遮挡、无人负样本和多人关联仍需继续验证。选用普通摄像头、单轴头部、通用底盘而非深度相机、LiDAR 或高算力主机，使当前主要硬件成本估算维持在约 700～800 元量级，但这不是量产成本。

### 15.4 低频 Detector 与 Sparse Tracker 协同

即使 V1.2 在 Vision 基本独占时 CNN 平均约 735 ms，持续全帧检测也不能提供流畅的位置观测；KWS 并发时单次推理更慢。只降低检测频率会导致目标位置陈旧，单用 Tracker 又难以从丢失状态重获。最终用 Tinyissimo 低频搜索、重获和纠偏，用 Sparse LK 在检测间隙追踪，并只处理最新相机帧。板端单人物工况的目标观测更新结果见第 9.4 节；该频率与 detector-only FPS 是不同指标。代价是 CNN 同步执行期间仍有更新空档，检测漏掉困难场景时 Tracker 无法凭空建立目标，故上层消费必须检查 TargetObservation 的时间戳和 freshness。

### 15.5 单核资源与共享媒体设备竞争

KWS 与 Vision 并发测量已显示 CNN 推理时间明显增加；录音、播报、通话和 AI 视野又会争用摄像头或音频。让所有能力始终全速运行会增加排队、旧观测和设备抢占风险。当前选择是 KWS 的 VAD 减少静音推理、Detector 低频调度、Voice/Media 会话确认麦克风交接、AI 视野共享摄像头画面、通话时暂停或重置相关感知及自动运动。这一方案优先保证交互和停车链路，不追求所有功能同时满负载运行。代价是通话期间不持续提供 KWS/Vision/跟随，且第 9.6 节的并发结论尚不能替代长时间、多家庭环境压测。

### 15.6 在线智能与离线基础能力的分工

在单核、有限可见内存且无 swap 的板端部署完整本地 LLM，会与界面、KWS 和 Vision 争夺资源，也无法满足当前开放式交互时延目标。完全依靠在线服务则使断网时连基础入口也失效。当前分工为本地 KWS、有限快捷动作和离线陪伴支撑基础操作，在线 ASR/LLM/TTS Provider 提供开放式对话；Provider 超时或取消仅影响当前会话。本地词条“救命”打开紧急页面，不自动拨号；“停止”取消语音/陪伴，不等同底盘 STOP。代价是开放式能力依赖网络和第三方服务。后续 LoongArch AI Server 或本地算力板可逐步提高自主推理比例，但当前仅完成终端侧 Provider 接入边界。

### 15.7 Linux 智能主控与 MCU 安全执行的分层

视觉观测和家属远控存在延迟、丢帧、网络中断与状态过期，Linux/AI 决策不适合成为底盘最终安全保证。直接由高层命令驱动电机虽可减少一层协议，却会把故障处理依赖于主控任务仍能及时运行。当前由 LongPet 决定模式和行为，通过 MotionService 校验控制权与目标时效；ESP32-S3 独立执行协议校验、租约、SAFE/STOP 和故障锁存。人工接管先停自动动作，退出后人物跟随不自动恢复；目标框大小只是远近代理，不能当作测距或避障。额外的 MCU、协议和测试提高工程复杂度，但使通信失效时仍有执行侧停车条件。V1.3 证据覆盖已开放运动功能；STBY 低电平是自由滑行，物理停距、急停和供电保护尚需产品化验证。

## 16 技术创新与应用价值

第 15 章解释方案为何被选中，本章归纳这些选择形成的可复用能力及应用意义。“创新”指本项目的系统工程方法和产品组合，不将采用开源模型结构、第三方服务或尚未部署的基础设施表述为原创算法成果。

### 16.1 无 LSX/LASX 平台的可验证 AI Runtime

项目建立了从目标 ISA 与 ABI 约束、源码构建、ONNX Runtime 标量路径，到板端算子和模型结果比对的完整验证链。其价值在于让本地 KWS 和人物推理在受限 LoongArch 平台上达到“可运行且数值可信”，而非只证明 ONNX 文件能加载。当前方法可作为同类无向量扩展平台集成 AI 组件的工程参考，适用范围以第 6 章已检查的版本和产物为限。

### 16.2 保留稳定 BSP 的 Hybrid Rootfs 交付方法

Hybrid Rootfs 将稳定的板端内核、驱动与核心运行库同 Buildroot 的 Qt/AI 用户态组件组合，并用清单、依赖闭包、符号版本、ISA、受保护 hash 与整包 SHA-256 约束合并。其价值是把“系统能启动”和“应用组件可更新”放在同一可审计交付流程中；已有板端结果证明当前组合可用，但跨环境重建和完整长期可靠性仍需继续验证。

### 16.3 面向持续感知的轻量检测与跟踪

正式 V1.2 模型经 COCO 人物训练、产品视角微调、Frozen Test 和 ONNX parity 固定，再由低频 Detector 与 Sparse LK Tracker 形成带时间戳的 TargetObservation。产品价值不在单次检测峰值，而在普通摄像头与单核 CPU 上向 AI 视野、跟头和跟随提供统一、可判断新旧的观测。动态闭环受目标丢失、遮挡和同步 CNN 空档限制，不能延伸为导航或安全感知能力。

### 16.4 AI 感知与运动执行安全分层

Vision/LLM 只产生感知或意图，MotionService 检查模式、会话和时效，MCU 再独立执行协议约束、租约与故障停车。该分层把计算不确定性限制在高层，使通信失效或旧目标不会自动保持底盘控制。应用价值是原型具有可解释的运动授权与停车链，而不是宣称软件已经覆盖物理急停、碰撞或量产安全。

### 16.5 老人端、家属端与 AI 的协同体验

老人端用大入口、浅导航、语音和状态反馈完成高频操作，复杂提醒管理和远程协助转移到 Family Desktop；AI 降低部分操作负担，本地快捷能力在断网时保留基本入口。提醒、关怀、语音、视频、AI 视野和受控运动共享业务规则及状态，不要求老人理解各 Provider 或设备协议。其应用价值体现在多端协同闭环，适老效果仍需老年用户专项测试。

### 16.6 可演进的 LoongArch AI Provider 架构与应用价值

现有 Provider/Service 边界让第三方云 AI、后续自建 LoongArch AI 服务器和本地 AI 算力板有机会采用一致的终端业务接口。当前创新是可演进的接口基础；服务器和算力板均未完成正式交付。产品经济性还体现在约 700～800 元量级的原型主要硬件成本下，整合 LoongArch 主控、普通摄像头、本地视觉/KWS、在线语音、家属端与四轮/单轴运动能力。该金额不包含量产外壳、认证、人工、物流、售后或未来算力扩展；系统集成与成本约束是应用价值，不是单项算法突破。

## 17 当前成果、边界与后续规划

本章按附录 A 的证据等级区分已验证、已实现但专项验证未归档，以及未来规划。状态说明针对当前比赛原型和相应测试条件，不自动扩展为量产级验收。

### 17.1 当前完成成果

当前原型已经整合龙芯 2K0300、Qt6 触控界面、提醒/关怀/天气、在线语音与本地 KWS/有限离线陪伴、Tinyissimo V1.2 人物视觉、Detector+Tracker、自动跟头与人物跟随、Family Desktop 的管理/AI 视野/通话/远控、ESP32-S3 Motion MCU，以及 Hybrid Rootfs 和部署资产。各能力的已验证层级不同：软件接口、板端数值、目标观测和实机运动分别对应附录 A 与相关报告。当前原型主要硬件成本估算约 700～800 元，不含量产和运营费用。

### 17.2 当前验证状态

VERIFIED 表示指定版本、条件和证据范围内已有通过记录；IMPLEMENTED 表示实现存在但专项验收不足；PLANNED 表示后续路线。附录 A 的需求基线为 74 条，其中 69 条 VERIFIED、5 条 IMPLEMENTED；本章不把原型级通过改写为量产认证。

| 状态 | 当前能力或事项 | 证据/待补范围 |
| --- | --- | --- |
| VERIFIED | Hybrid Rootfs 合并与板端启动、ONNX Runtime 标量数值链、V1.2 Frozen Test/ONNX parity、板端 Detector+Tracker；本地 KWS 与语音软件链、提醒和 FamilyLink 接口；自动跟头、人物跟随及 MCU 已开放运动功能 | 第 6、9、11 章及附录 A；运动 V1.3 为 58 项中 56 项通过，横移两项暂缓 |
| VERIFIED | Family Desktop 的设备管理、AI 视野、音视频通话及远控，在相应软件、双端或运动实机测试条件下通过 | 附录 A 的 E-FAMILY、E-VIEW、E-CALL、E-MOT；不等于任意网络或长稳工况均已验收 |
| IMPLEMENTED | 关怀未知数据展示、核心存储故障显式报告、适老异常提示、敏感凭据保护、跨模块诊断 | 附录 A 对应 FR-CARE-02、NFR-STAB-04、NFR-USAB-02、NFR-SEC-03、NFR-MAINT-03；各自专项记录待归档 |
| PLANNED | 自建 LoongArch AI Server、AI Compute Board 插件、障碍/悬崖感知、量产电源与物理急停等 | 仅架构或版本规划，当前无正式交付及功能验收 |

**表 17-1 当前能力与证据等级。** VERIFIED 只覆盖相应测试对象与工况；Family Desktop 通话、远控等以附录 A、接口测试和实机记录中的具体范围为准。

### 17.3 当前产品边界

当前不提供人脸身份识别、医疗诊断、自动跌倒诊断、SLAM、完整自主导航或全场景避障；固定镜头与单轴头部也不构成三维环境感知。人物框高度仅用于远近分类，不是精确测距。没有完整本地 LLM、自建 LoongArch AI Server 正式服务或 AI 算力板插件。横向平移按版本计划暂缓；独立物理急停、完整量产 BMS/电源保护、机械防护与 EMC 等认证尚不在当前验收范围。现有家庭 HTTP/WS 通道仅面向受控局域网，不能作为公网远控能力宣传。

### 17.4 当前工程化限制与成本口径

单核 CPU、约 512 MB 物理内存配置下的一次板端可见内存约 369 MiB且无 swap，使高负载视觉与完整本地大模型不适合并发常驻；在线 ASR/LLM/TTS、天气和远程通话依赖相应网络与服务。原型摄像头固定、头部只有水平单轴，自动跟随限于有人看护的低速环境；实际速度、停距、长期运行、断电 RTC 保持以及量产电气/外壳仍需量化。约 700～800 元只是比赛原型主要硬件的估算，不覆盖外壳、认证、人工、物流、售后。未来服务器作为共享或外部算力不必逐台计入终端 BOM；本地算力板会增加单机成本，需单独形成档位与核算。

### 17.5 后续版本规划

后续路线按可验收阶段推进，而不是将规划写入当前成果。VNext-1 评估 LoongArch AI 推理服务器的 ASR/LLM/TTS 接口、时延、鉴权和网络失效回退；VNext-2 验证本地 LoongArch AI 算力板的供电、散热、驱动、接口、模型兼容与 BOM；VNext-3 增加障碍/悬崖等环境传感及安全策略，再讨论更复杂移动能力；VNext-4 完成急停、默认禁能、限流、电源树、机械结构和必要认证；VNext-5 扩大私有/本地 AI 能力并补齐适老用户、长稳、停距与跨模块诊断测试。各阶段需独立定义版本、测试环境、退化策略和成本上限，不能以接口预留替代完成验收。

### 17.6 LoongArch AI 推理基础设施演进路线

当前基础形态是“2K0300 终端本地 KWS/视觉 + 已接入在线 AI Provider”，实时界面、业务、运动裁决留在 LongPet/MCU。服务器协同版计划由终端经受控 LAN/WAN 接入 LoongArch AI Inference Server，承载 ASR、LLM、TTS 等密集推理；增强版计划由终端经高速本地接口或网络接口接入 LoongArch AI Compute Board，承载近端加速推理。两条路线统一通过 Provider/Service 边界向业务层提供能力，并分别设计超时、离线回退、数据保护与版本兼容。最终形成“本地 CPU 实时感知/控制—近端算力板—共享 LoongArch 服务器”的可选分层架构；服务器协同版和增强版均属 PLANNED，尚无已部署的服务器或算力板可供当前验收。

```mermaid
flowchart LR
    C["当前基础版：2K0300<br/>本地 KWS / Vision<br/>在线 AI Provider"] --> P["统一 AI Provider / Service 接口"]
    P -. 后续服务器协同版 .-> S["LoongArch AI Server<br/>LAN / WAN 推理"]
    P -. 后续增强版 .-> B["LoongArch AI Compute Board<br/>本地 / 近端加速"]
    P --> M["LongPet 业务与实时控制<br/>Motion MCU 安全执行"]
```

图 17-1 LoongArch AI 推理能力演进路线。实线为当前终端能力，虚线为尚未交付的两条算力扩展路线；运动安全仍由终端和 MCU 裁决。

## 18 总结

LongPet 是面向老年日常陪伴和家属协同的 LoongArch 智能终端原型。老人端以触摸、语音和清晰状态完成提醒、关怀、联系家人等高频任务；家属端承担管理、查看和受控协助。

项目在单核、无 LSX/LASX 的 2K0300 上完成 AI Runtime 适配、可校验 Hybrid Rootfs、本地 KWS 与轻量人物感知，并把在线语音、媒体会话和运动控制接入统一业务架构。Detector+Tracker 提供带时效的人物观测；MotionService 与 ESP32-S3 MCU 分担行为决策和执行侧安全约束，避免将 AI 输出直接作为电机授权。

当前比赛原型形成从本地多模态感知、在线/离线语音交互、家属端协同，到低速运动执行与失效停车的产品闭环。约 700～800 元为主要原型硬件成本估算，不包括量产相关费用。软件、板端、模型和运动已有分层验证，但五条需求的专项记录、老年用户体验、长稳与量产级硬件保护仍需补充。

后续将在现有 AI Provider 边界上评估自建 LoongArch AI 推理服务器和本地算力板插件，按不同成本与算力需求形成部署形态，同时完善环境感知、电源、物理急停和整机验证。两种推理扩展目前均为规划，不属于本次正式功能验收。
