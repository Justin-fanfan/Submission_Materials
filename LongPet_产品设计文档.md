...
VERIFIED 表示在指定版本和工况下已有证据，IMPLEMENTED 表示代码能力已具备但专项记录不足，PLANNED 表示未来能力。核心业务、本地 AI、FamilyLink/Family Desktop 协同及已开放运动功能具有相应验证；部分状态呈现、异常处理、配置保护与诊断能力仍待专项验收。具体待补项、逐项状态和证据由附录 A 与相应报告维护。

| 状态 | 能力范围 | 结论边界 |
| --- | --- | --- |
| VERIFIED | Hybrid Rootfs、本地 KWS、正式 V1.2 视觉、提醒、FamilyLink/Family Desktop 协同、已开放运动功能 | 仅覆盖相应软件、板端、双端或实机测试工况 |
| IMPLEMENTED | 部分状态呈现、异常处理、配置保护与诊断能力 | 实现与专项验收结论分开管理，具体待补项见附录 A |
...
后续首先补齐适老用户、长期稳定性、真实停距、断电时间保持、配置安全和跨模块诊断等专项验证；运动与整机工程化继续完善障碍/悬崖感知、物理急停、**独立硬件级默认禁能/失控保护**、限流、电源/BMS、机械结构及必要认证。反馈驱动的版本迭代与发布放行按第 14.7 节执行。更私有化或本地化的 AI 服务可随部署条件演进，但不得以接口预留替代实现与验收；两种可选算力形态集中见第 17.6 节。
