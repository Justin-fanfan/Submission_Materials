...
原型以龙芯 2K0300 为主控，在单核、无 LSX/LASX 向量指令的条件下运行 Qt6 界面、本地业务、本地关键词识别和人物视觉。预训练声学模型支撑本地 KWS 唤醒与有限快捷操作；项目基于 TinyissimoYOLO 结构训练并部署正式 V1.2 人物检测模型，以低频 Detector 和帧间 Tracker 共同形成带时效的人物观测，供家属端 AI 视野、自动跟头与人物跟随复用。开放式对话由在线 ASR、LLM、TTS Provider 提供，工具调用须经本机业务规则校验；断网时保留已设提醒、本地快捷操作和已配置的陪伴音频，不将离线能力表述为完整本地大模型。

人物跟随由家属显式开启，只在有人看护的低速场景中依据有效目标、头身朝向和画面远近代理受限靠近，不承担自主导航或避障。龙芯主控负责感知、业务与控制权仲裁，ESP32-S3 运动 MCU 独立校验模式、命令时效与故障，并在控制失效时撤销底盘驱动。为使图形、语音和视觉在目标板上可靠运行，项目修复了 ONNX Runtime 无向量指令路径的数值问题，采用保留稳定基础系统、按清单合入用户态组件并校验兼容性与完整性的 Hybrid Rootfs。当前原型已完成相应的软件、板端、双端和受控实机功能验证，长期稳定性已有初步测试，适老性、整机安全与量化性能仍需继续验收。约 700～800 元仅为主要原型硬件成本估算，不代表售价或量产成本。该原型展示了在受限国产架构平台上组合适老交互、端侧感知、在线智能与失效安全运动的可行路径。
...
本文是 LongPet 的产品设计主文档，用于说明产品为何设计、面向谁、当前交付什么，以及各能力如何在同一原型上协作。文档把产品需求、系统与硬件设计、软件及模型、接口数据、运动安全、验证状态和后续规划连接起来，为设计评审、集成验收和后续迭代提供共同基线。阅读本文件时，应将产品目标、已实现行为、已有验证和未来计划分别理解，不把需求陈述直接等同于完成结论。

### 1.2 文档范围与读者对象

本文覆盖老人端与家属端的使用场景和功能边界，以及龙芯主控、Family Desktop、运动 MCU、端侧 AI 与系统运行环境的协同设计。第 2 章从产品视角给出定位与使用方式；第 3 章及附录 A 将需求和验收状态关联；第 4～12 章说明架构、硬件、软件、业务、模型、接口、运动安全和典型流程；第 13～18 章说明工程化、交付、技术权衡、当前成果与后续路线。附录 B、D 给出接口与硬件连接索引，附录 C、E、F 保留配置、第三方资产和参考资料的归档位置。

主要读者为产品设计评审者、开发与测试人员、硬件及系统集成人员，以及负责原型部署和维护的人员。老人和家属的实际操作步骤应以用户手册为准；本文说明的是产品行为、设计依据与能力边界，不替代操作说明或现场部署步骤。

### 1.3 产品版本、交付基线与文档关系

本文档当前修订版本为 **V0.12**，需求基线为 **REQ-BASE-2026-09-15**。本文所称“当前版本”或“当前原型”，均指在该需求和工程交付基线下形成的 LongPet 产品原型。
...
| 材料类型       | 主要作用                                                   | 与本文档的关系                             |
| -------------- | ---------------------------------------------------------- | ------------------------------------------ |
| 产品设计文档   | 描述产品定位、需求、架构、业务设计、技术路线和能力边界     | 项目的设计主文档                           |
| 测试报告       | 记录具体功能、运动系统、软硬件联调和验证结果               | 为本文档中的 VERIFIED 状态提供测试证据     |
| 用户手册       | 面向最终使用者说明设备操作方式和注意事项                   | 将本文档中的产品能力转化为用户操作说明     |
| 构建与部署指南 | 说明编译环境、系统依赖、部署路径、服务启动和运行检查方法   | 支撑软件交付与环境复现                     |
| 源码工程       | 保存 LongPet 主程序、Family Desktop、Motion MCU 等实际实现 | 是实现层面的直接工程依据                   |
| 模型与交接材料 | 保存 KWS、TinyissimoYOLO、ONNX 导出、测试数据及训练说明    | 为 AI 模型来源、指标和部署结论提供专项证据 |

当不同材料描述的粒度不同时，应区分“产品设计”“实际实现”和“验证结论”三个层次。本文档负责给出统一的产品设计和当前交付边界，专项测试报告和交接材料用于补充具体测试条件、数据和工程细节。

后续规划中的 LoongArch AI Server、LoongArch AI 算力扩展模块、更完整的本地人工智能能力以及更高级的环境感知和自主运动能力，不属于当前原型的既有交付能力。

### 1.4 参考资料与证据来源

本文档中的设计和结论主要来源于以下几类项目证据：

**需求与产品设计资料。** 包括需求基线、功能设计、系统架构、业务流程、接口定义以及各章节形成的设计记录，用于确定产品当前应具备的能力和边界。

**源码与配置。** 包括 LongPet 主程序、Family Desktop、Motion MCU 固件，以及 AI Provider、视觉、语音、媒体、运动和系统配置等实现，用于确认具体模块职责、接口关系和当前实现状态。

**测试与联调记录。** 包括软件测试、运动功能测试、板端测试、端到端联调和硬件验证，用于确认相应能力是否在规定条件下达到当前设计目标。

**模型训练与部署材料。** 包括 KWS 模型来源与集成说明、TinyissimoYOLO 训练与微调记录、ONNX 导出及一致性验证、板端推理性能和 Detector/Tracker 联调结果，用于支撑本地感知相关结论。

**系统构建与运行证据。** 包括 LoongArch Linux 运行环境、Hybrid Rootfs 构建与部署、ONNX Runtime 适配、systemd 服务、网络恢复机制以及运行资源观测结果，用于证明当前软件栈能够在目标硬件平台上部署和工作。

本文档引用测试结果时，仅表示对应能力在文档所述测试条件和当前原型环境下已经获得相应证据，不将原型阶段测试结果扩大解释为量产级认证、安全认证或所有环境下的绝对保证。

### 1.5 完成状态与证据等级说明

本文沿用附录 A 的状态口径，状态针对**某条需求在所列版本及工况下**的证据，不是对整个产品的无条件评级。

| 状态 | 含义 | 使用边界 |
| --- | --- | --- |
| `VERIFIED` | 已有对应的软件、板端、实机、双端或文档校验记录支持表内所述范围。 | 须同时查看测试对象、环境、方法与结果；软件通过不自动等于实机、长期或老年用户测试通过。 |
| `IMPLEMENTED` | 相应实现已经完成，专项验收或记录仍需补充。 | 可以描述当前实现，不能表述为该专项已验证。 |
| `PLANNED` | 属于后续版本或工程化工作。 | 不计作当前已交付能力；后续纳入需求基线并验证后再改变状态。 |

当前 74 条需求中，附录 A 标记 69 条 `VERIFIED`、5 条 `IMPLEMENTED`；后续规划能力不因出现在路线图中而计入当前交付。软件单元或集成测试（UT、ST）说明规则和接口行为，板端测试（BT）说明目标平台运行与数值表现，硬件实机测试（HT）说明受测执行条件，端到端测试（E2E）说明具体多端闭环，文档与产物检查（DOC）说明版本、清单和完整性。各类证据解决的问题不同，应结合实际记录使用。运动功能已有受控实机测试，长期运行已有初步测试；适老用户任务、停距、整机保护及长期多业务工况等仍按第 17 章继续量化和验收。

## 2 产品概述

## 2.1 项目背景与问题定义

LongPet 面向老年人日常生活中的陪伴、信息获取、家庭联系和基础移动交互需求。

随着智能终端和在线服务不断增加，传统智能手机、通用平板电脑以及复杂应用往往要求用户理解多级菜单、账户体系、输入法和大量界面元素。对于部分老年用户而言，这类交互方式存在一定使用门槛。与此同时，家属通常希望能够远程了解设备状态、维护提醒事项、发起视频联系或在必要时辅助控制设备，但如果将这些配置和管理操作全部放在老人端，会进一步增加终端复杂度。

另一方面，纯云端智能终端又容易受到网络状态影响。人物感知、关键词响应、运动控制和安全停车等具有实时性或安全要求的能力，不适合完全依赖远程服务。开放式自然语言对话等复杂 AI 能力则需要较高的模型和计算资源，在当前资源受限的低成本 LoongArch 原型平台上全部本地运行并不经济。

LongPet 因此需要解决的并不是单一的“增加一个 AI 对话功能”，而是在有限硬件资源下协调以下问题：

- 如何让老人端保持简单、直接和低学习成本；
- 如何让家属在远端承担较复杂的管理和辅助操作；
- 如何在无网络或网络波动时仍保留必要的本地交互能力；
- 如何利用在线 AI 提供开放式自然语言服务，同时避免 AI 直接控制高风险执行机构；
- 如何通过本地人物感知和运动能力，使陪伴终端具备一定的空间交互能力；
- 如何在低成本、资源受限的平台上完成完整系统的工程部署和稳定运行。
...
> **面向老年日常陪伴与家庭协同场景的 LoongArch 智能陪伴终端与可运动机器人原型。**

产品以老人日常使用为中心，并通过家属端提供远程管理和协同能力。其目标不是将所有功能集中到老人面前，而是按照不同用户的使用能力和操作复杂度重新分配系统职责。

LongPet 的核心产品原则是：

> **老人端做减法，家属端承担复杂管理；本地感知保证基础实时能力，在线 AI 提供开放式智能，独立运动 MCU 保证执行安全。**
...
LongPet 当前采用低成本普通 RGB 摄像头和资源受限的 LoongArch 平台完成整套原型，当前主要原型硬件成本估算约为 **700～800 元人民币**。该数字仅用于反映当前主要原型硬件路线的成本水平，不代表整机售价、量产 BOM 或完整商业化成本。
...
老人是 LongPet 的主要本地使用者。产品尽量减少复杂设置、深层菜单和需要持续学习的操作，通过触摸大按钮、语音关键词、自然语言对话、视频通话和自动人物感知等方式降低使用门槛。
...
这种职责划分避免将设备配置和管理压力转移给老人，同时使家属能够参与产品的长期使用过程。

### 典型使用环境

LongPet 当前主要面向家庭、宿舍式居住空间以及类似的室内日常环境。

运动系统当前适用于有人监督、环境相对明确的低速室内使用场景。由于现阶段没有完整的深度感知、障碍物传感、跌落检测、SLAM 和自主导航能力，因此产品不将复杂开放环境、无人监督自主移动或高风险空间作为当前版本的目标应用场景。

## 2.4 用户痛点与核心价值

LongPet 的产品价值并不来自单一算法或单一硬件，而来自对老人、家属和设备三方关系的重新组织。

### 1. 降低老人使用数字设备的认知负担

传统智能设备通常要求用户完成较多界面导航和配置操作。LongPet 将老人端设计为浅层级、直接操作的交互入口，并结合语音关键词和自然语言交互，使常用能力能够以更简单的方式获得。

其价值并非“提供更多菜单”，而是尽可能减少老人需要理解的系统复杂度。

### 2. 将复杂管理能力转移至家属端

提醒维护、部分设置、模式管理和远程辅助并不适合全部放在老人端完成。

LongPet 将 Family Desktop 作为独立的家属协同入口，使老人负责“使用”，家属更多负责“管理和辅助”，由此形成清晰的双端分工。

### 3. 同时保留本地实时能力和在线智能能力

开放式 AI 对话具有较高计算需求，而关键词响应、人物感知和运动安全具有实时性要求。

LongPet 没有简单选择“全部本地”或“全部云端”，而是采用分层方式：
...
因此，在在线 AI 不可用时，产品仍能保留部分基础本地交互能力。

### 4. 让陪伴设备具备一定的空间交互能力

LongPet 不只是固定在桌面上的语音终端。

通过摄像头、人物检测、目标跟踪、单轴头部转动和移动底盘，设备可以感知人物位置，控制头部朝向目标，并在明确启用条件下进行低速人物跟随。

这使语音、视觉和运动能力能够围绕同一个用户形成连续交互，而不是彼此独立的功能演示。

### 5. 将安全边界从高级业务中独立出来

AI、网络和上层软件都可能产生延迟、异常或状态切换，因此不能直接承担底层运动安全。

LongPet 将电机实际执行交由独立 Motion MCU 管理，并通过模式、租约、命令时效和失效停车等机制约束运动行为。高级系统输出的是受控运动意图，而不是绕过安全层直接驱动电机。

这种架构使产品能够在引入 AI 和网络能力的同时保持明确的执行安全边界。
...
天气数据来自在线服务。系统能够保留最近获得的天气结果，在在线语音引用已经超过有效时间的旧天气信息时，会明确提示该信息已经过期，避免将旧数据表达为实时天气。

提醒和关怀属于日常生活辅助信息，不作为医疗诊断或健康评估结论。

### 2.5.2 本地语音快捷交互

在不需要开放式语义理解的场景中，本地 KWS 可以识别预设关键词，并触发相应快捷操作。

例如用户可以通过关键词进入对应页面、获取本地快捷响应或中止当前交互。其中“停止”作为明确的安全控制关键词，可以触发当前运动停车，同时终止相关语音或陪伴流程。

本地语音能力的价值在于快速响应和基础可用性，它并不等同于完整的离线大语言模型。
...
AI 可以参与天气、对话和部分业务信息处理，但只能通过预先定义的业务接口调用受控能力。大语言模型本身不直接控制底盘、电机或其他高风险执行设备。
...
该模式使产品既能满足老人端的低复杂度要求，又不牺牲系统维护和家庭协同能力。
...
人物跟随需要由明确的业务状态或家属端操作启用，而不是系统在发现人物后自动进入运动状态。
...
当前人物跟随不执行自动倒车、横移和复杂轨迹规划。当目标丢失、目标信息失效、通信超时或相关控制条件不再满足时，系统进入停车状态。

其定位是**受监督环境下的低速人物跟随能力**，而非自主导航系统。
...
因此，远程控制被设计为一种“持续人工参与”的辅助能力，而不是长期无人值守的远程驾驶模式。

## 2.6 产品形态、组成与交互方式

LongPet 当前由四个相互协作但职责明确的部分组成：
...
产品历史资料或部分底层软件环境中可能出现 `2K300`、`LS2K300`、`2K300-PAI` 等名称。本文档统一以当前正式平台名称 **龙芯 2K0300** 表述。

### 2. Family Desktop 家属端

Family Desktop 面向家属或照护者。

它承担老人端不宜承载的复杂交互和管理操作，包括状态查看、提醒和关怀管理、部分设置、视频联系、模式管理和人工远程控制等。

因此，老人端与家属端并不是简单复制相同界面的两个客户端，而是根据不同用户角色进行职责划分：

> 老人端强调简单使用，家属端强调管理与协助。
...
不同 AI Provider 通过统一接口接入，使业务层不需要与某一个具体云服务深度绑定。

视觉系统则采用 TinyissimoYOLO 人物检测模型和 Detector/Tracker 协同方案，在低算力平台上兼顾人物检测精度和连续目标更新频率。

当前正式推荐的人物检测权重为 **V1.2 FINAL_RECOMMENDED**；V1.3 主要用于验证新场景数据的继续适配能力，目前保持 **EXPERIMENTAL** 状态，不替代 V1.2 作为正式交付模型。
...
视觉模型、在线 AI 和家属网络连接都不能绕过 Motion MCU 直接控制电机。
...
- **在线 AI**负责开放式智能交互；
- **2K0300 主系统**负责业务协调；
- **Motion MCU**负责可靠、安全地执行运动。

这些部分共同构成 LongPet，而不是彼此独立的功能模块集合。

### 2.7 产品能力边界与非目标

当前版本是具备已验证功能链的比赛原型，不能按医疗设备或无人值守移动机器人理解。以下边界决定了产品对外描述、使用条件和后续验收范围。

| 能力领域 | 当前边界 | 后续规划或非目标 |
| --- | --- | --- |
| 语音与 AI | 本地预训练模型提供 KWS 和有限快捷动作，开放式 ASR/LLM/TTS 依赖在线 Provider；已配置音频可离线播放。 | 完整本地 LLM、长期记忆、LoongArch AI Server 和近端 AI 算力板均未作为本版交付能力。 |
| 关怀与天气 | 今日关怀汇总实际记录的饮水和提醒完成；天气依赖联网更新，在线语音引用过期快照时提示时效。 | 未自动采集的活动或健康指标不作为监测结果；医疗诊断、自动报警和自动健康监测不在当前能力内。 |
| 视觉与运动 | 单目相机和单轴头部支持人物关注；人物框仅作远近代理，跟随在有人看护的低速条件下受限前进、原地转向并按失效条件停车。 | 无真实测距、SLAM、自主导航、全场景避障或悬崖检测；横向平移入口暂缓开放。 |
| 连接与安全 | 家属端使用带鉴权的受控局域网 HTTP/WS；MCU 在自身供电及程序正常时提供软件级失效停车，STOP 撤销驱动输出。 | 当前链路不内置跨不可信网络的安全传输；物理急停、独立硬件失控保护、一体化电源/BMS 与量产级整机防护待工程化。 |
| 验证与成本 | 核心链路已有对应的软件、板端、双端和受控实机记录，长期运行已有初步测试；主要原型硬件估算约 700～800 元。 | 老年用户任务、长期多业务工况、速度和实际停距、RTC 断电保持及整机可靠性仍需专项验收；估算不是售价或量产成本。 |

**表 2-3 当前能力边界。** 单项功能的状态以第 17 章和附录 A 所列版本、方法与结果为准；规划能力在完成独立需求、设计和验证前不纳入当前交付。
...
**表 14-1 四工程交付物及集成关系。** 放行以组合版本的板端与实机验收结果为准。
...
**表 17-1 当前能力与验证状态。** 逐项需求和测试记录见附录 A。
...
图 17-1 LoongArch AI 推理能力演进路线。实线为当前终端能力，虚线为并行可选的规划方向；运动安全仍由主控与 MCU 裁决。
