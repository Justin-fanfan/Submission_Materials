...
老人触摸提醒页、家属端远程管理和在线语音工具都能发起提醒操作。三种入口将请求交给 LongPet 的同一提醒业务，由它统一校验、保存和调度；家属端通过 FamilyLink 等待设备确认，语音工具通过受控本地能力执行，不形成独立的数据副本。保存结果回到发起入口，家属端只有收到设备确认才显示成功；请求与设备当前记录冲突时提示刷新。已保存提醒由本机按有效时间触发，触发状态在老人端呈现，老人确认后才写入完成事件并更新今日关怀；家属端刷新时读取同一设备摘要。设备正常运行且时间有效时，这条已保存提醒的闭环不依赖家属端持续连接。
...
*图 12-2 三入口统一提醒及关怀闭环。* 流程的终点是完成事件进入当日关怀摘要，或事项保持未完成并等待后续确认；“到时触发”本身不等于“已经完成”。今日关怀仅汇总实际记录的饮水、提醒完成等事件，不把未采集的活动或健康指标写成测量结果。AI Provider 故障只影响语音入口，触摸操作与已有本地提醒继续工作；时间、存储等异常的反馈见 12.9。

### 12.3 在线 AI 语音交互与工具调用流程

本地 KWS 检出唤醒词且在线能力可用，或老人触摸语音入口后，LongPet 建立 Voice Session 并申请音频资源。KWS 暂停得到确认后才开始录音；ASR 将声音转为文本，LLM 结合有限会话上下文决定普通回答或受限 Tool 操作。前者直接形成回答，后者先由 Tool Registry 校验并调用本地业务，再把真实执行结果交还 LLM 组织答复。文字、TTS 播报和表情把结果反馈给老人，播放结束后释放音频、恢复 KWS 并回到待机。用户看到的主状态为待机→倾听→思考→说话→待机；识别或执行中的补充提示应反映当前阶段，而非提前宣布成功。
...
*图 12-3 在线语音和 Tool Calling 时序。* 当前授权范围是提醒的创建、查询与按明确对象删除，设备日期时间查询，以及限定页面导航；工具名称、参数与页面枚举见第 10 章及附录 B。天气回答只能引用设备已有且标明时效的天气快照，不以语言模型的表述代替实时获取。LLM 不能直接操作数据库、通话或运动设备。工具结果回到同一 Voice Session 后才生成最终回答；提醒的实际变化还可由本地列表和后续触发链核对。

Voice Session 隔离当前交互与迟到的 ASR、LLM、TTS 或播放结果；旧结果不能覆盖新状态或再次触发业务。老人取消时，当前录音、在线请求和播报退出，音频资源被释放，触摸取消入口在 KWS 暂停期间仍可使用。已经由本地业务确认提交的操作以业务记录为准，不因后续播报取消而伪称未执行。在线服务失败时当前会话退出等待并显示原因；若仅合成或播放失败，已有文字回答仍可反馈，具体异常与恢复路径集中见 12.9。这样，语音流程以本地业务结果和老人实际收到的反馈闭合，而不是以 LLM 发出工具意图为终点。

### 12.4 离线语音交互与能力降级流程

在线语音不可用时，本地 KWS 仍识别已配置的唤醒和快捷词。命令分发将有效意图交给本机页面或业务能力，有限陪伴能力则在取得音频资源后播放已配置的本地内容。老人通过页面变化、文字提示或播放结果得知执行状态，动作完成或命令窗口结束后回到待机。缺少音频资源或可用内容时，设备说明当前能力范围并结束等待；本地的联系入口不等于通话离线可用，紧急页面也不自动报警。具体快捷词配置与动作范围见第 8、9 章。
...
*图 12-4 在线与离线语音分流。* 离线能力是本地快捷动作与有限陪伴，并不等于完整本地 ASR、LLM、TTS 或本地大语言模型。触屏语音入口可提示在线不可用原因并供手动重试；网络与 Provider 恢复后，下一次交互重新判定在线能力。无论在线状态如何，已保存的本地提醒仍由本机业务管理。

### 12.5 人物感知、自动跟头与人物跟随流程

Camera 作为共享采集源交付画面，Vision 将人物检测和连续跟踪统一为 `TargetObservation`，记录人物状态、位置与时效。这是供多个模块消费的数据流，不直接驱动电机：AI 视野据此显示有效人物信息；家属开启仅头部或人物跟随模式后，控制策略结合新鲜观测、当前模式和运动状态形成受控意图。MotionService 统一仲裁后经 UART 交给 Motion MCU 执行，MCU 的 `STATUS` 再把实际模式、运动、故障与头部状态送回 LongPet，供下一次决策及家属端反馈。完整路径是 `TargetObservation` → 控制策略 → MotionService → Motion MCU → `STATUS` → 策略与界面。
...
*图 12-5 单份视觉观测到三个消费路径。* 仅头部模式使用有效目标调整舵机，底盘不因此启动。人物跟随先确认目标，再协调头部朝向、机身方向与粗略距离；仅在控制条件满足时低速靠近，达到合适距离或目标过近时停车。具体策略与 MCU 许可规则见第 11 章。人物框仅提供距离代理，不具备障碍或悬崖识别能力。

目标丢失、观测过期或 Camera 中断时，旧目标不能继续产生控制意图；跟随转入等待/丢失状态并停止有效移动授权。LongPet 将停止结果和目标状态反馈给家属端，MCU 安全约束在链路异常时仍独立生效。家属关闭模式或人工接管时，本次自动控制结束；新目标出现后须重新确认并取得有效运动状态，才可继续获准的自动动作。具体失效停车路径汇总于 12.9。本流程不包含自主搜索、绕障、自动倒车、SLAM 或身份重识别。

### 12.6 家属端 AI 视野与人工远程控制流程

家属打开 AI 视野时，Family Desktop 经 FamilyLink 建立监看会话。LongPet 把共享 Camera 的画面和 `TargetObservation` 中有效的人物信息送到家属端；画面属于视觉数据流，人物框只随新鲜观测显示。页面关闭时释放该远端消费者，本地 Vision 仍可运行。设备状态和画面在家属端形成反馈；本地推理未启用时仍可能有实时画面，因此“无人物框”不能被解释为“画面中无人”。

家属选择人工远控时，同一页面转入独立的运动控制链：Family Desktop 请求控制会话，LongPet 经 FamilyLink 校验控制者和 MCU 状态，MotionService 先停止自动动作再取得人工控制权。按住方向键期间，家属端持续刷新运动意图；LongPet 将有效意图送往 Motion MCU，并将实际模式、运动和故障状态回传页面。松键或退出时停止刷新并请求停车，以 MCU 状态作为最终执行反馈。窗口失焦、页面隐藏或断连时同样终止续租；即使停止消息未能送达，独立时效约束也使底盘最终停车。人工接管结束后，人物跟随须家属显式重开；仅头部模式在条件满足时可等待新目标恢复。具体停车与控制权限见第 11 章，断连路径见 12.9。
...
*图 12-6 AI 视野数据链与远控控制链。* Family Desktop 不直接连接 Motion MCU；远控页面只有在设备确认人工模式、在线且无故障时才开放操控。发出意图不等于完成动作，执行与停车以 MCU `STATUS` 为准。监看或控制连接中断后，页面清除旧画面和“在线可控”状态，流程以失联反馈及最终安全停车结束。

### 12.7 音视频通话与媒体资源生命周期

家属可发起语音或视频呼叫，老人可从设备发起视频呼叫。信令为本次呼叫建立独立会话标识和版本状态，并将来电或外呼等待状态呈现给两端；设备发起的呼叫由家属接听或拒绝，等待中可以取消。随后通话协调共享资源：取得音频所有权、确认 KWS 暂停，按模式启用 Mic、Speaker 和必要的共享 Camera。设备来电提示在音频可用后播放，只有媒体就绪才向两端反馈“已接通”。通话期间媒体会话持续传输声音及相应画面；任一端挂断后，双方收到结束状态，媒体和设备资源释放，本地能力按各自恢复条件重新运行。过期呼叫操作不能改变当前会话；接口字段和信令规则见第 10 章及附录 B。
...
*图 12-7 通话会话与资源生命周期。* 通话活动期间 Vision 推理和仅头部自动控制暂停，人物跟随关闭；视频复用共享相机而非另开采集链。结束时先停止媒体和提示、释放 Camera 与音频，再恢复 KWS；Vision 重新等待新观测，仅头部在条件满足时恢复，人物跟随不会自动重启。异常断线也须释放已占资源并将失败状态反馈两端，具体降级路径见 12.9。
...
| 新 Voice Session 与旧 Provider 回调 | 当前有效语音会话 | 旧 ASR/LLM/TTS/播放结果丢弃 | 新会话独立完成 | 防止旧结果覆盖页面或触发动作 |
| STOP 与任意持续底盘动作 | STOP 与 MCU 安全约束 | 当前人工或自动移动授权撤销 | 重新满足模式、会话、目标和安全条件后才可动作 | 停车是高优先级控制语义，且不解除故障锁存 |
...
| --- | --- | --- | --- | --- |
| 三入口提醒及今日关怀 | FR-REM-01、FR-REM-03、FR-REM-04、FR-CARE-01、FR-CARE-02 | 提醒业务、关怀业务、FamilyLink、语音工具 | 同源提醒与事件、版本校验、设备确认 | UT、ST、E2E |
| 在线 Voice 与 Tool Calling | FR-VOICE-02、FR-VOICE-03、FR-VOICE-06、FR-VOICE-07、NFR-SEC-02 | KWS、Voice Session、Provider、Tool Registry、本地业务 | 会话隔离、工具执行结果、音频所有权 | UT、ST、BT、E2E |
...
| 仅头部自动关注与人物跟随 | FR-VISION-03、FR-VISION-05、FR-VISION-06、FR-MOTION-05 | 自动视觉策略、MotionService、Motion MCU | 有效目标、受控运动意图、STATUS | UT、ST、HT、E2E |
| Family Remote Control | FR-FAMILY-04、FR-MOTION-03、FR-MOTION-04、FR-MOTION-08、FR-MOTION-09 | Family Desktop、FamilyLink、MotionService、Motion MCU | 远控会话、持续授权、STOP、执行状态 | UT、ST、HT、E2E |
| 双向音视频通话 | FR-CALL-01、FR-CALL-02、FR-CALL-03、FR-MM-03、FR-MM-04 | 通话业务、媒体协调、共享 Camera、Family Desktop | 会话与版本状态、媒体就绪、资源释放 | UT、ST、E2E |
| 异常与局部降级 | FR-WEATHER-01、FR-WEATHER-02、NFR-STAB-01、NFR-STAB-02、CON-RES-01 | 天气、Voice、Vision、媒体、Motion、业务 UI | 数据时效、会话与资源状态、STATUS | UT、ST、BT、E2E |
