...
第 3.11 节规定质量要求，本章说明它们如何落实到响应链路、共享资源、故障恢复、适老交互和可复现交付。2K0300 为单核主控，网络和外部 AI 服务可能失效，摄像头与音频被多业务共享，因此设计允许可选能力局部降级，同时保留满足自身前提的本地提醒与安全停车。核心数据库不可用时则明确停止初始化，不能以空数据伪装正常就绪。

| 非功能目标 | 主要设计机制 | 证据与边界 |
| --- | --- | --- |
| 响应和资源 | 本地操作不等待在线请求；视觉处理最新帧；媒体设备按会话交接 | 第 7.10、9.4—9.6 节；长稳与在线时延仍需量化 |
| 稳定和安全 | 可选能力局部降级；会话失效；主控与 MCU 分层约束运动 | 第 10.10、11、12.9 节 |
| 适老和隐私 | 大入口、清晰状态、触摸/语音互补；凭据与会话数据受控 | 第 8.2、10.7—10.9 节；适老与凭据专项验证待补 |
| 维护和复现 | Service/Port/Adapter 分层；版本、模型和系统产物可核对 | 第 6、7、9 章及附录 A；不承诺跨主机逐字节一致构建 |

**表 13-1 非功能需求落实矩阵。** 对应第 3.11 节 PERF、STAB、USAB、SAFE、SEC、MAINT、REPRO 要求；逐项状态以附录 A 为准。

### 13.1 性能预算、响应链路与资源约束

界面导航、提醒查看和取消等本地操作不以外部 ASR、LLM、TTS 或天气请求完成为前提。在线请求按会话管理超时、取消和旧结果，避免网络等待占住当前业务。视觉链只处理最新画面，轻量检测负责搜索与纠偏，跟踪在检测间隙更新目标；目标观测的时效与检测吞吐分别评估，具体板端数据见第 9 章。性能预算优先保障本地交互、有效观测和停车链路，不把单核平台描述为可同时满负载运行所有 AI 与媒体能力。

### 13.2 视觉、语音与媒体并发资源调度

KWS 和 Vision 竞争单核算力，录音、播报、通话及 AI 视野还共享麦克风、扬声器与摄像头。MediaSessionCoordinator 按会话交接音频所有权，KWS 暂停确认后进入录音；AI 视野复用共享画面，视频通话则让渡摄像头并暂停或重置相关 Vision 与自动运动。资源不足时可降低可选感知或在线能力，但页面、提醒和运动失效保护仍按自身条件运行。现有并发验证只覆盖第 9.6 节列明的工况。

### 13.3 适老可用性与交互一致性

1024×600 触控界面把说话、关怀和通话等高频入口直接呈现，采用较大的操作目标和浅层导航；复杂提醒管理与远程协助由 Family Desktop 承担。等待、失败、不可用和结束状态使用文字、界面与表情反馈，并保留退出或重试入口。触摸、语音和有限离线快捷动作互补，使网络失效不切断基本操作。界面实现和页面检查已有证据，老年用户对异常提示的理解性与任务成功率仍需专项使用测试。

### 13.4 稳定性、故障隔离与服务恢复

故障按“识别来源—局部降级—状态反馈—条件恢复”处理：Provider 失败结束当前在线会话，天气数据标记时效，Vision 无新鲜观测时隐藏旧目标并退出依赖目标的运动，FamilyLink 断开不删除本地提醒。Motion UART 或 MCU 状态无效时撤销运动授权；核心 SQLite 无法打开或迁移失败时显式停止启动。systemd 管理 LongPet 自启动与异常退出恢复，已知 Wi-Fi 探测问题由条件式恢复服务处理；KWS、语音、媒体和远控会话各自释放资源，重启不继承旧授权。具体 STOP、租约和故障锁存机制见第 11 章；软件机制不能替代物理急停、电流保护与机械防护。

### 13.5 网络、接口、凭据与隐私安全

本地提醒、照护事件和设置持久化，有限对话上下文及媒体、视觉、远控状态属于进程或会话态。长期凭据由受控配置和可信进程持有，普通业务响应不暴露完整密钥；家庭媒体与远控使用短时会话令牌。音频和画面随任务采集、传输与释放。当前家庭 HTTP/WS 通道仅面向受控局域网，鉴权不能等同于传输加密，跨不可信网络仍需后续安全接入方案。配置访问权限与凭据保护专项验收待归档，数据边界见第 10 章。

### 13.6 可维护性、日志诊断与可观测性

Controller 接收操作，Service 维护规则，Repository 管理持久数据，Port、Adapter 和 Provider 隔离设备与外部服务。相同业务规则供本地、语音和家属入口复用；模型、服务与设备参数通过配置解耦。维护者可结合 systemd 日志、Provider 响应、媒体会话、模型状态及 UART/MCU 状态区分故障来源，并避免把最近一次停车原因当作唯一故障记录。LongPet、Family Desktop、Motion MCU 与 Buildroot/LoongArch 各自维护交付边界；跨模块诊断的专项验收仍见附录 A。

### 13.7 自动化测试、实机验证与可复现性

软件测试覆盖规则、接口、取消和失效语义，板端测试覆盖 LoongArch 运行库、模型数值与资源表现，实机测试覆盖运动、停车和双端闭环；各层证据不能互相替代。Vision 正式 V1.2 的 Frozen Test、ONNX parity 与模型 hash 固定版本，ONNX Runtime 参考比对防止“能加载但算错”；Hybrid Rootfs 以组件清单、依赖与 ISA/ABI 审计、受保护文件 hash 核对部署输入。协议基线和附录 A 的需求—模块—证据矩阵支持复测，具体测试过程留在第 6、9、11 章及对应报告。

### 13.8 第三方组件、模型来源与许可管理

Qt、ONNX Runtime、TinyissimoYOLO 结构及 KWS 声学权重等第三方资产应在发布清单中记录来源、版本、用途和适用许可。项目训练的人物权重与所用上游结构分开标识，正式 V1.2 与实验 V1.3 模型也分开管理。模型 hash 和训练复现材料证明版本可定位，不等同于再分发授权；第三方库与 KWS 权重的许可核验须在正式分发前完成。清单归档与发布放行关系见第 14 章。

## 14 构建、部署、发布与运营计划

本章规定从四工程源码到原型发布、试用和迭代的链路，关注版本关系、放行条件与使用支持。具体 shell、刷写及现场配置命令由独立《构建部署指南》承担。

### 14.1 四工程构建链与交付物关系

交付链为“源码与工具链基线 → Buildroot 用户态组件和稳定板端系统的 Hybrid Rootfs → LongPet 应用与 AI 模型 → Motion MCU 固件 → Family Desktop → 集成验收 → 发布包”。四工程分别产出系统运行时、终端应用、执行固件和家属端应用；版本及协议必须在同一交付基线中成组核对。
...
**表 14-1 四工程交付物及集成关系。** 单一工程构建成功不替代组合版本的板端与实机验收。

### 14.2 龙芯系统、应用、家属端与 MCU 部署概览

龙芯系统保留已验证的板端 rootfs、内核和驱动，从 Buildroot 选择性合入 Qt、Python/AI 与媒体用户态组件，形成经清单和完整性检查的 Hybrid Rootfs；合并规则与 ISA/ABI 边界见第 6 章。LongPet 使用匹配工具链交叉构建，随运行资源、systemd 服务、数据库路径和受控配置部署。本地 KWS 与正式 V1.2 人物 ONNX 模型按路径和 SHA-256 校验，实验 V1.3 不覆盖正式模型。

Motion MCU 固件按 ESP32-S3 配置构建并烧录，随 UART 协议版本与终端应用成对放行；具体接线、安全测试见第 11 章与附录 D。Family Desktop 独立构建，在受控局域网配置设备连接，并核对状态、提醒、媒体和远控会话。未来 LoongArch 推理算力部署不属于本次发布包，路线集中见第 17.6 节。

### 14.3 配置、首次启动与验收检查点

首次部署依次确认显示/触摸、网络和可信时间、摄像头、音频、UART、数据库、KWS/视觉模型、LongPet 服务及 FamilyLink 可达性。AI 与天气服务、FamilyLink 凭据、设备和数据路径以及 Vision/Motion 开关由受控配置提供；远程监听需先具备令牌。检查不止于进程存在，还须确认提醒可读写、模型可导入、媒体资源能交接、MCU 状态合法且断连后按安全规则退出。时间不可信时不能把提醒的绝对时间调度判为通过。

### 14.4 版本基线、发布包与完整性校验

发布基线由四工程源码版本、Buildroot 配置与工具链、稳定系统及 Hybrid Rootfs、LongPet 应用与 service、AI Runtime/模型、MCU 固件、Family Desktop、配置模板、SHA-256 manifest 和相应验收记录共同定义。Hybrid 合并检查依赖闭包、符号版本、LoongArch 架构、目标 CPU 不支持的指令、受保护基础文件与 UTF-8 locale；默认 ISA 扫描范围以第 6.8 节为准。发布清单不含真实密钥或个人 Git 作者信息。更换模型、固件或 Provider 时，须更新受影响的版本关系与回归结论。

### 14.5 升级、备份、回退与故障恢复

升级前备份 SQLite 业务数据及受控配置，核对新包 hash、协议兼容性与可回退基线；回退时成组恢复相互兼容的系统、应用、模型与固件。模型替换需重新核对正式版本及目标场景表现，固件升级需重做受影响的安全验收。systemd 支持应用自启动和异常退出恢复，已知 Wi-Fi 探测故障有条件式恢复路径；重启不继承旧语音、通话或运动会话。数据库初始化失败、电源或硬件故障须明确报告并按原因处置，不能以服务重启代替现场修复。

### 14.6 原型试用、使用支持与维护计划

当前交付面向受控环境中的比赛原型试用，而非规模化云服务。试用前确认设备供电、网络、运动活动区域和家属端连接条件，运动保持有人看护的低速边界。使用支持围绕常见的网络不可达、模型或设备路径错误、媒体占用、提醒时间异常和 MCU 故障提供可读状态与处理指引；维护人员依据发布版本、配置及日志定位问题，必要时恢复备份或回退。尚未通过的量产保护、长稳和适老专项验证不纳入已完成试用承诺。

### 14.7 后续运营、反馈收集与迭代计划

原型运营以设备与家属端的使用反馈为输入，记录问题发生的版本、设备/网络条件、操作路径、实际结果及可复现证据；按影响范围区分交互、业务、AI、媒体、运动与硬件问题。每个问题经确认、定位、修复、回归和版本归档形成闭环，涉及运动安全或数据完整性的变更优先复测。迭代计划依据用户理解性、可靠性、维护成本和部署条件调整功能，新增能力只有在定义放行标准并取得相应证据后才进入正式交付；LoongArch AI 算力扩展的可选路线见第 17.6 节。

## 15 关键技术难点与方案权衡

本章从问题、备选方案、最终选择及代价说明关键设计决策。具体构建补丁、模型训练和性能测量分别留在第 6、9 章，安全协议和测试细节留在第 11 章。

| 难点 | 比较与取舍 | 最终方案及边界 |
| --- | --- | --- |
| 指令集兼容 | 现成 LoongArch 包不必然适配无 LSX/LASX 目标 | 固定工具链源码构建并审计 ISA；升级需重验 |
| 推理数值正确性 | 运行库可加载不等于算子和模型输出正确 | ORT 标量路径及参考比对；维护目标构建补丁 |
| 基础系统 | 整体替换扩大内核和驱动风险 | 稳定 BSP + 选择性 Hybrid Rootfs；增加合并审计 |
| 低算力视觉 | 重模型覆盖范围大但延迟高 | V1.2 person-only + Detector/Tracker；任务与场景受限 |
| 语音与媒体 | 全本地开放式智能超出板端资源，全在线又丧失离线入口 | 本地 KWS/有限离线 + 在线 Provider；并发需让渡 |
| 人物跟随 | 图像框不能提供地图或真实距离 | 头身分层、远近代理与新鲜目标；不导航或避障 |
| 远程运动 | 网络/主控状态可过期 | 主控授权 + MCU 独立时效和故障停车；需硬件保护 |

**表 15-1 七项技术难点与方案权衡。**

### 15.1 2K0300 无 LSX/LASX 的软件兼容问题

2K0300 虽为 LoongArch64，但没有 LSX/LASX，部分同架构预编译包及默认优化路径不能直接作为交付输入。依赖现成包可缩短构建时间，却难以保证目标 ISA 与 ABI；全部放弃本地 AI 又无法满足唤醒和人物感知需求。最终采用固定工具链与源码构建 NumPy、ONNX Runtime 等关键组件，并检查 ELF 架构、ABI 和新增产物指令。代价是软件栈升级和补丁维护成本；具体检查范围见第 6 章。

### 15.2 ONNX Runtime 从可加载到数值正确的修复

仅证明运行库可加载不足以证明推理正确：目标板曾出现语音模型输出异常。继续使用原有运行时路径会将数值错误带入产品感知链；放弃模型则失去本地能力。最终在目标构建中使用一致的标量 MLAS 路径，并以算子参考结果和语音样例验证修复，再核对实际加载的运行库。该选择让本地模型结果具有板端证据，但标量性能及特定版本补丁需继续维护；数值问题定位和验证过程见第 6.5 节。

### 15.3 基础系统稳定性与 Hybrid Rootfs 取舍

Qt6、AI 与媒体运行时需要新的用户态组件，完整替换板端基础系统却未达到项目稳定性要求；完全保留旧软件栈又不能支撑目标应用。最终保留已验证的内核、驱动和核心运行库，仅按清单合入 Buildroot 用户态组件，形成 Hybrid Rootfs。依赖、ABI/ISA、受保护文件和整包校验约束合并风险。代价是版本组合管理更复杂，不能把当前可部署组合外推到任意包版本。

### 15.4 低算力视觉模型、领域适配与 Detector/Tracker 协同

YOLO11n-Pose、手部关键点等较重方案难以提供目标板所需的持续人物观测；单用轻量 Detector 仍有检测间隙，单用 Tracker 又无法可靠重获目标。最终选用 TinyissimoYOLO-v1-small person-only 结构，以 COCO 人物数据和固定机位实拍数据形成正式 V1.2 权重，再由低频 Detector 搜索/纠偏、Sparse LK Tracker 更新间隙位置。V1.3 扩域模型保持 EXPERIMENTAL，未替换正式 V1.2。该方案在普通摄像头和单核 CPU 上形成带时效的 TargetObservation，但遮挡、漏检、多人关联及同步检测空档仍是边界；数据和指标见第 9 章。

### 15.5 语音在线/离线链路与资源竞争

完整本地 LLM 与持续全速视觉、KWS 同时运行超出当前板端适宜的资源组合；完全在线则让断网时基础操作也失去入口。最终以本地 KWS、有限快捷动作和离线陪伴保障基本操作，在线 ASR/LLM/TTS 提供开放式对话，并通过 VAD、低频检测和媒体会话所有权协调单核及音频/摄像头竞争。通话期间相关感知可暂停，外部 Provider 失败只结束当前在线会话。代价是开放式能力依赖网络，已验证并发范围不能替代长稳测试。

### 15.6 头身协调、距离代理与无地图跟随边界

图像人物框可提供方向和相对尺度，不能提供真实距离、地图或障碍信息。若直接用画面水平偏差驱动底盘，会把视觉误差转成未经约束的移动。最终在 HEAD_ONLY 与 FOLLOW 模式间区分头部关注和底盘动作：LongPet 依据 MCU 报告的头部偏移决定是否调整机身，以人物框大小作 FAR/GOOD/NEAR 远近代理，并要求目标新鲜且稳定；不把该代理称为测距。代价是只能在有人看护的低速环境跟随，不提供 SLAM、自主导航或全场景避障，安全条件见第 11.9 节。

### 15.7 远程控制时效性与双处理器失效安全

家属远控、视觉目标及 Linux 主控状态都可能因网络、任务延迟或通信故障而过期；只在高层发送一次停止命令不足以覆盖失联。最终由 LongPet 的 MotionService 仲裁人工和自动控制、校验会话与目标时效，ESP32-S3 MCU 独立执行模式、租约、故障锁存和停车约束。人工接管先停止自动动作，退出后人物跟随不自动恢复。双处理器和协议增加实现、测试成本，却使执行侧具备失联停车条件；实际停距、物理急停和电源保护仍需后续产品化验证。

## 16 技术创新与应用价值

本章的创新指项目已形成的系统工程方法和产品组合，不把开源模型结构、第三方服务或未交付基础设施表述为原创算法成果。

### 16.1 LoongArch 低算力平台的可验证 AI 适配实践

项目把目标 ISA/ABI 约束、源码构建、标量推理和板端数值比对连为一条可核对的 AI Runtime 路线，使本地 KWS 与人物视觉在无 LSX/LASX 平台上达到已验证范围内的可运行、数值可信状态。价值在于建立受限平台的工程验证方法，而非仅让模型文件可加载；适用范围以第 6 章的版本和产物为准。

### 16.2 可校验的 Hybrid Rootfs 交付方法

以稳定板端内核、驱动和核心运行库为基础，引入 Buildroot Qt/AI 用户态组件，并通过清单、依赖闭包、符号版本、ISA、受保护 hash 和整包 SHA-256 约束交付。这使基础系统稳定性与应用更新需求可以在同一发布基线下核对；已有板端结论覆盖当前组合，跨环境重建和长期可靠性仍需验证。

### 16.3 面向持续人物感知的轻量视觉与分层推理

正式 V1.2 人物模型、低频 Detector 与 Sparse LK Tracker 形成带时间戳和新鲜度的 TargetObservation，供家属端 AI 视野、自动跟头和人物跟随复用。价值是围绕持续感知闭环组织模型与调度，而不是追求单帧通用识别峰值；检测漏失、遮挡和推理空档限制了场景覆盖。

### 16.4 语音、视觉与家属端协同的适老体验

老人端用大入口、浅导航、语音和状态反馈承担高频任务，Family Desktop 承担复杂管理与远程协助，AI 降低部分操作负担。本地快捷能力使断网时仍有基本入口；提醒、关怀、通话、AI 视野和受控运动共享业务规则及状态。该价值来自多端协同，具体适老效果尚需老年用户专项测试。

### 16.5 AI 感知与多层运动安全机制

Vision/LLM 输出只作为感知或意图，MotionService 校验模式、会话与时效，MCU 独立执行故障和停车约束。该分层避免旧目标或失联会话继续获得运动授权，使原型具备可解释的控制边界；它不等同于物理急停、避障或量产安全认证。

### 16.6 应用价值、成本优势与推广条件

LongPet 在当前主要原型硬件成本估算约 700～800 元的条件下，集成 LoongArch 主控、触控显示、普通摄像头、本地 KWS/人物视觉、在线语音、Family Desktop、四轮底盘和单轴头部。轻量模型、普通摄像头和独立 MCU 支撑了这一完整产品闭环，而不依赖高价 GPU 主机、LiDAR 或深度相机。成本控制是产品工程价值，不是单项算法创新；该估算不是量产 BOM 或售价，不含外壳、认证、人工、物流、售后及未来算力扩展。推广仍取决于适老使用验证、长稳、硬件保护和部署支持；现有 Provider 架构为未来算力服务预留接口，但未来服务本身不是当前创新成果。

## 17 当前成果、限制与后续规划

本章统一当前交付、验证边界与后续路线。VERIFIED、IMPLEMENTED 和 PLANNED 按附录 A 的证据口径使用，不把原型级验证写成量产认证。

### 17.1 交付基线与已验证成果

当前交付基线包括龙芯 2K0300 上的 Qt6 终端、Hybrid Rootfs、提醒/关怀/天气、在线语音与本地 KWS/有限离线陪伴、正式 Tinyissimo V1.2 人物视觉与 Detector/Tracker、自动跟头与人物跟随、Family Desktop 管理/AI 视野/通话/远控，以及 ESP32-S3 Motion MCU。系统、模型、软件接口和已开放运动功能分别具有对应的板端、软件、双端或实机证据；具体版本与适用条件见第 6、9、11 章及附录 A。

### 17.2 已实现能力与验证边界

VERIFIED 表示在指定版本和工况下已有证据，IMPLEMENTED 表示代码能力已具备但专项记录不足，PLANNED 表示未来能力。核心业务、本地 AI、FamilyLink/Family Desktop 协同及已开放运动功能具有相应验证；关怀未知数据展示、核心存储故障报告、适老异常提示、敏感凭据保护和跨模块诊断仍需补充专项验收。详细逐项状态和测试数量仅由附录 A 与相应报告维护。

| 状态 | 能力范围 | 结论边界 |
| --- | --- | --- |
| VERIFIED | Hybrid Rootfs、本地 KWS、正式 V1.2 视觉、提醒、FamilyLink/Family Desktop 协同、已开放运动功能 | 仅覆盖相应软件、板端、双端或实机测试工况 |
| IMPLEMENTED | 关怀展示、存储故障提示、适老异常反馈、凭据保护与跨模块诊断中的待补项 | 实现与专项验收结论分开管理，见附录 A |
| PLANNED | 后续算力服务、环境感知和量产级硬件保护 | 不纳入当前功能验收 |

**表 17-1 当前能力与验证边界。** 状态不代替逐项需求和测试记录。

### 17.3 实验性能力、接口预留与未完成功能

Vision V1.3 扩域模型保留 EXPERIMENTAL 标识，正式部署仍采用 V1.2 FINAL_RECOMMENDED。AI Provider/Service 层已形成终端侧扩展边界，尚未交付自建 LoongArch AI Server 或本地/近端 AI Compute Board。当前也不具备人脸身份识别、医疗或自动跌倒诊断、完整本地 LLM、SLAM、完整自主导航与全场景避障；横向平移按版本计划暂缓。物理急停、量产级 BMS/电源保护、机械防护及 EMC 等认证不属于本次验收。

### 17.4 已知性能、环境、硬件与成本限制

受单核 CPU、有限内存和无交换空间等条件约束，高负载视觉与完整本地大模型不适合长期并发常驻；开放式在线语音、天气和远程通话依赖相应网络及服务。摄像头视角固定，头部只有水平单轴，人物框大小不是精确测距；自动跟随限于有人看护的低速环境。实际速度、停距、长期运行、断电 RTC 保持以及量产电气和外壳仍需量化。当前主要原型硬件成本约 700～800 元只是估算，不包括量产外壳、认证、人工、物流、售后；服务器作为共享算力与本地算力板对单机成本的影响不同，需在未来配置档位中分别核算。

### 17.5 后续测试、工程化与产品化路线

后续首先补齐适老用户、长期稳定性、真实停距、断电时间保持、配置安全和跨模块诊断等专项验证；运动与整机工程化继续完善障碍/悬崖感知、物理急停、驱动默认禁能、限流、电源/BMS、机械结构及必要认证。反馈驱动的版本迭代与发布放行按第 14.7 节执行。更私有化或本地化的 AI 服务可随部署条件演进，但不得以接口预留替代实现与验收；两种可选算力形态集中见第 17.6 节。

### 17.6 LoongArch AI 推理基础设施演进路线

当前基础形态为“2K0300 终端本地 KWS/视觉 + 已接入在线 AI Provider”，实时业务与运动裁决保留在 LongPet 主控和 Motion MCU。后续可选方向 A 为受控 LAN/WAN 上的 LoongArch AI Server，提供共享 ASR/LLM/TTS 等推理；方向 B 为本地/近端 LoongArch AI Compute Board，通过本地或网络接口提供加速服务。两者共用终端 Provider/Service 边界，可按成本、隐私、网络、时延、功耗及部署环境选择其一或组合，并非固定先后版本。两条路线都需验证鉴权、数据保护、模型/接口兼容、超时和离线回退；Server 与 Compute Board 均属 PLANNED，未纳入当前正式功能验收。
...
LongPet 是面向老年日常陪伴与家属协同的 LoongArch 智能终端原型。设计以大入口触控、语音、本地提醒与关怀为老人端基础，以 Family Desktop 承担复杂管理和远程协助；在单核、无 LSX/LASX 的 2K0300 上组合可验证 AI Runtime、Hybrid Rootfs、本地 KWS、轻量人物感知、在线语音、媒体会话和受控运动。当前形成从感知、交互、家属协同到执行侧失效停车的原型闭环，适用范围和量产边界见第 17 章。

### 18.2 需求、技术方案与证据的对应关系

本设计以第 3 章需求为起点，由总体架构、硬件和系统平台落实约束，应用 Service、Provider、视觉与 Motion MCU 承担实现，再由软件、板端、双端及实机测试形成证据，构成“需求 → 设计 → 实现 → 测试 → 证据”的追踪闭环。逐项状态、验证范围与证据定位由附录 A 维护。
