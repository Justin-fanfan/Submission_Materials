...
| V1.0 | 2026-09-15 | 完成全文内容与格式审校，统一设计口吻、性能与验收范围、术语、图表目录及交叉引用，整理为正式提交版 |
...
- 表 3-3 功能需求分类与产品结果
- 表 3-4 FR-UI 需求及验收标准
- 表 3-5 FR-REM 需求及验收标准
- 表 3-6 FR-CARE 需求及验收标准
- 表 3-7 FR-WEATHER 需求及验收标准
- 表 3-8 FR-VOICE 需求及验收标准
- 表 3-9 FR-VISION 需求及验收标准
- 表 3-10 FR-CALL 需求及验收标准
- 表 3-11 FR-FAMILY 需求及验收标准
- 表 3-12 FR-MOTION 需求及验收标准
- 表 3-13 FR-MM 需求及验收标准
- 表 3-14 NFR-PERF 需求及验收标准
- 表 3-15 NFR-STAB 需求及验收标准
- 表 3-16 NFR-USAB 需求及验收标准
- 表 3-17 安全性与隐私保护需求及验收标准
- 表 3-18 NFR-MAINT 需求及验收标准
- 表 3-19 NFR-REPRO 需求及验收标准
- 表 3-20 平台与资源约束表
- 表 3-21 需求验证类型
...
LongPet 使用普通 RGB 摄像头和资源受限的 LoongArch 平台完成整套原型，主要原型硬件估算约为 **700～800 元人民币**，体现了方案在有限成本下集成多模态交互与运动能力的可行性。量产成本将在结构、电源、保护和制造方案确定后单独核算。
...
LongPet 将电机执行交由独立 Motion MCU 管理，通过模式、租约、命令时效和失效停车约束运动行为。家属控制与视觉策略生成运动意图，所有动作经过控制权判断和 MCU 校验后执行，从而降低上层延迟或异常影响底盘的风险。
...
本地 KWS 将预设关键词映射为页面入口、快捷回应和取消动作；“停止”按当前业务状态取消语音或陪伴操作，或发送底盘 STOP。识别在本机完成，断网时仍可使用。录音、播放和通话占用麦克风时暂停监听，触屏退出入口保持可用。

#### 2.5.3 开放式 AI 对话

当网络和在线 Provider 可用时，用户可以通过老人端进行自然语言对话。

典型流程为：语音输入 → 在线 ASR → LLM → 受控工具调用或业务处理 → TTS → 语音回复。

AI 支持日常对话、时间与天气查询、提醒管理和限定页面操作，工具参数由本地业务接口校验。人物跟随和人工远控通过独立运动链路运行。

#### 2.5.4 视频联系与家庭协同

老人从首页直接呼叫家属并查看通话状态，Family Desktop 提供双向语音或视频联系、提醒管理和状态查看。两端共享设备确认的业务结果，家属可协助完成较复杂的配置，老人端保留简洁的日常入口。

#### 2.5.5 人物感知与头部跟随

本地视觉通过 Detector/Tracker 输出统一的 `TargetObservation`，供 AI 视野、自动跟头和人物跟随复用。家属开启仅头部模式后，单轴机构随有效目标调整朝向，底盘保持静止，使设备持续面向用户。

#### 2.5.6 人物跟随

人物跟随由家属显式开启；发现人物只更新观测，运动模式由控制服务独立管理。

跟随策略结合目标位置、MCU 头部偏移和人物框归一化高度，协调头部与机身朝向，在 FAR 且对齐时低速前进，GOOD 或 NEAR 时停车；底盘转向采用原地旋转。
...
| 视觉与运动 | 单目相机与单轴头部支持人物关注；低速跟随使用人物框归一化高度估计远近，在有人看护环境下前进、原地转向并按失效条件停车 | 增加真实测距、障碍与悬崖感知，再扩展横移、定位、导航及复杂场景运动 |
...
当前交付需求基线为 **REQ-BASE-2026-09-15**，共 74 条，按功能、质量和平台约束组织。每项需求都有稳定 ID、优先级和验收标准，便于设计、实现与测试对应。
...
| FR-CARE-02 | 系统应按实际照护事件汇总关怀信息，并区分用户记录与自动采集数据。 | P0 | 关怀信息与事件来源一致；尚未接入的数据项显示未知或待接入。 | 8.3、8.8、10.6 |
...
当前视觉需求聚焦人物发现、位置持续更新、头部关注和受限低速跟随。归一化人物框高度用于远近分级，运动策略结合目标时效和 MCU 安全条件执行。身份识别、跌倒检测与自主导航列入后续独立能力设计。

| ID | 需求 | 优先级 | 验收标准 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-VISION-01 | 系统应在摄像头和视觉能力开启、可用时发现受测场景中的人物，并区分发现、搜索和丢失状态。 | P1 | 在第 9 章的测试场景中检出人物；无有效目标时显示搜索或丢失。 | 8.5、9.3—9.4 |
...
| NFR-REPRO-02 | 交付模型应明确正式和实验版本，保留模型清单、训练及导出记录与完整性校验值。 | P0 | V1.2 正式模型与 V1.3 实验模型标识清晰，部署文件与清单校验值一致，训练、导出及评测结果可对应到具体版本。 | 9.3、10.8 |
| NFR-REPRO-03 | Hybrid Rootfs 交付应保留组件清单、依赖、受保护基础文件和产物完整性校验结果。 | P0 | 清单与实际合并产物一致，兼容性和完整性校验可追溯，固定输入可用于重建相同软件集合。 | 6.7—6.10 |
| NFR-REPRO-04 | 关键能力应保留测试对象版本、环境、方法与结果，并按软件、板端数值和实机闭环分层验证。 | P0 | 验证结论可关联到明确版本、环境、方法和结果；数值推理与参考输出比对，各层测试范围分别记录。 | 6.5、6.10、9.3—9.6、11.10 |
...
| CON-HW-02 | 当前运动应在有人看护、无障碍和悬崖风险的低速原型环境中运行，供电使用主控与执行侧分路方案。 | P0 | 验收覆盖已开放动作及分路供电；横移、导航和量产电源/BMS 按后续版本独立验证。 | 5.8、5.10、5.13、11.11、D.8—D.9 |
| CON-HW-03 | 本地提醒应在供电、应用、存储和本机时间有效时离线调度，重启后按 RTC 或网络校时条件确认时间。 | P0 | 本机时间有效时，断网仍正常调度已有提醒；RTC 断电保持按后续专项测试验证。 | 6.9、8.3、D.1 |
...
| CON-NET-02 | 当前家庭 HTTP/WS 通道应在受控局域网部署并进行接口鉴权；跨不可信网络时增加加密传输或可信中转。 | P0 | 本版验收覆盖受控局域网鉴权连接；加密传输或可信中转在扩展部署时单独验证。 | 4.5、10.9、11.8 |
...
每条需求都按“需求 ID → 设计章节 → 实现模块 → 验证方式 → 结果”建立追踪链路。第三章定义可观察的验收判据，附录 A 汇总实现位置、测试结果和专项检查计划，形成贯通设计、实现与验收的索引。

| 验证类型 | 含义 | 验证范围与条件 |
| --- | --- | --- |
| UT | 软件单元或自动化逻辑测试 | 校验、状态转换、数据与协议逻辑；声学和电气性能由实机测试验证 |
...
| DOC | 配置、构建、模型与交付检查 | 核对版本、manifest、hash 和接口；运行效果另以相应测试验证 |

*表 3-21 需求验证类型*

同一需求可组合多种方法，实际完成情况按附录 A 的状态与测试记录判定。
...
系统构建见第 6 章，应用分层见第 7 章，接口见第 10 章及附录 B，固件安全见第 11 章。
...
LongPet 围绕单核 LoongArch 平台组合本地基础能力、在线 Provider、家属端协作和独立 MCU 执行。设备由此提供触摸交互、语音入口、人物感知与家庭连接；外部服务或感知模块异常时，各能力按自身状态反馈和降级。表 4-5 汇总技术选择及其产品作用。
...
龙芯主控使用独立 5 V 充电宝。主控与执行侧采用分路供电，减少执行负载与主控计算负载之间的供电影响。该配置在共用 LM2596 路径的高负载联调后确定，已用于当前原型功能验证。主控与执行侧保持共地，为 UART 提供共同信号参考；高负载电源瞬态将在后续电气测试中量化。
...
| 物料或结构 | 选型理由 | 当前取舍与边界 |
| --- | --- | --- |
| 龙芯 2K0300 先锋派开发板 | 满足 LoongArch 原生平台要求，Linux、Qt6、端侧 AI 及多媒体链路已完成目标板集成 | 以轻量模型和受控并发适配单核资源 |
...
目标主控龙芯 2K0300 采用单核 LA264、约 1 GHz 的 LoongArch64 处理器，不支持 LSX/LASX 向量指令。编入这些指令的二进制包无法在目标板运行；语音、视觉和界面并发时还需控制 CPU 与内存占用。板端测试中系统可见内存约 369 MiB，未配置 swap。
...
| GStreamer | 1.22.9 | Buildroot 源码交叉构建 | 插件化音视频处理能力 | 以显式插件清单补充 ELF 依赖闭包，覆盖动态加载组件 |
...
目标板隔离部署的十项 ONNX Runtime 算子测试均通过。Conv、LayerNorm、Softmax 相对独立参考的最大绝对误差分别约为 `1.9×10⁻⁶`、`1.3×10⁻⁶`、`3.0×10⁻⁸`；其余受测算子为零误差。判定阈值为绝对误差 `1×10⁻⁵`、相对误差 `1×10⁻⁴`。FP32、INT8 流式识别测试中，静音输出为空串，语音结果连续三次一致，四类样例与 x86 参考逐字匹配。两端 sherpa-onnx 版本不同，此项比对用于验证受测样例的输出一致性。
...
系统采用稳定板端基础系统与 Buildroot 用户态组件组合的交付路线。早期整套替换测试出现 CPU 占用异常波动，项目据此保留已验证的出厂基础系统，按清单合入图形、AI 和多媒体组件，在扩展应用能力的同时复用成熟的内核与驱动。Buildroot 构建仓库为 [open-loongarch/buildroot-2024.08](https://gitee.com/open-loongarch/buildroot-2024.08)。
...
架构检查遍历合并 rootfs 的全部 ELF，要求 `Machine=LoongArch`。ISA 检查默认反汇编本次新增的 ELF，拒绝 LSX/LASX 指令；也可启用全 rootfs 严格扫描。该次检查发现非 LoongArch ELF 0 个，新增 ELF 的向量指令命中 0 次。
...
8 月 29 日合并记录显示受保护文件未变、locale 验证通过，rootfs、内核和 ramdisk 的校验通过。构建流程以固定输入和配置重建软件集合，并生成完整性清单；跨主机逐字节一致性作为后续复现测试项目。
...
针对特定 Wi-Fi 驱动探测失败，系统配置了条件式恢复服务。系统已启用时间同步并完成 RTC 读写验证；提醒调度以可信系统时间为前提，RTC 断电保持及重启后时间恢复的稳定性纳入专项复测。合并系统在 `/etc/locale.conf` 中设置 C.UTF-8；冷启动验收已检查 Qt 字符编码、媒体设备权限、网络恢复及并发资源占用。
...
| AI/Python 组件需匹配目标指令集与 ABI | 固定源码、工具链和 ABI 基线 | 目标包构建、符号版本及板端导入检查 | 构建与运行验证通过 |
...
| 模块层 | 主要职责 | 接口约束 |
| --- | --- | --- |
| Qt 表示层 | 呈现页面、表情、状态栏及会话快照；发出语义化用户事件 | 通过控制层提交操作，数据、网络和设备访问由后续层承接 |
| 控制层 | 将本地或远程事件映射为业务动作，组织页面跳转和请求/响应 | 调用共享业务规则，协议与设备实现交由适配层 |
| Service 层 | 校验业务约束、维护状态机和服务间协作，给出可显示结果 | 面向业务模型和能力接口，供不同页面与入口复用 |
| Repository / Port | 隔离持久化与外部能力契约 | 提供数据与能力接口，流程和界面由上层组织 |
| Adapter / Provider | 实现平台、网络、音视频和 UART 接入及错误映射 | 向 Service 返回能力结果与错误，由 Service 更新业务状态和控制权限 |
...
陪伴表情用于提示设备状态：待机、倾听、思考、说话和异常分别配有表情或文字反馈。提醒触发时显示提示，并区分已完成与未确认事项；通话页面显示对方、连接状态和挂断入口。网络、AI 或设备能力不可用时，页面提示重试或功能状态，其他本地功能保持可用。表情由交互状态驱动，情绪识别作为后续扩展方向。
...
“今日关怀”将饮水记录、用药提醒完成等事件汇总为当日信息。老人可在本机记录喝水并查看进度，家属读取同源关怀摘要，便于共同跟进日常事项。活动分钟与互动次数按实际接入的数据展示，尚未接入的指标显示未知或待接入；后续通过明确的数据源和算法扩展自动采集。

天气直接显示在状态栏，并将带时效的快照提供给在线语音回答。网络正常且配置可用时更新当前天气；更新失败时，本次运行中已取得的快照会保留，首次无数据则显示“--”。若快照过期，在线语音使用旧快照时会提示信息可能过期。

### 8.4 语音交互、工具操作与离线陪伴

语音让老人少走菜单。在线能力可用时，老人说“小龙小龙”后进入倾听状态，说完需求后看到思考状态，随后听到回答并看到说话状态，结束后回到陪伴状态。老人也可以触摸“陪我说话”进入对话。在线模式支持日常聊天、询问时间和当前天气，以及提醒创建、查询、按明确对象删除和打开限定页面等业务操作。工具调用将自然语言请求转为经业务校验的提醒和页面操作，减少菜单查找与手动输入。

```mermaid
flowchart TD
    W[小龙小龙] --> N{在线 AI 可用?}
    N -- 是 --> L[倾听需求] --> Q[思考并进行 Tool Calling] --> P[更新有限对话上下文] --> O[回答与播报] --> H[返回陪伴]
    N -- 否 --> K[离线快捷提示与关键词]
...
```mermaid
flowchart LR
    P[有效人物观测] --> V[家属端 AI 视野<br/>画面·人物框·状态]
    P --> U{家属选择的运动模式}
    U -- 仅头部 --> H[头部自动关注·底盘静止]
    U -- 人物跟随 --> F[确认目标并调整朝向] --> M[条件满足时靠近] --> S[距离合适或目标丢失时停车]
    U -- 关闭 --> D[保持自动运动关闭]
```
...
| 天气 | 状态栏显示天气，在线对话可引用已获取的信息 | 联网更新；失败时保留本次运行快照并标记时效，语音引用旧数据时提示可能过期 |
| 在线 AI 语音与工具操作 | 自然语言聊天、提醒管理和限定页面操作 | 需要在线服务；具体动作由本机规则授权 |
| 本地唤醒与离线陪伴 | 离线保留关键词唤醒、快捷动作和已配置的陪伴音频 | 录音、播放和通话占用麦克风时暂停监听；后续扩充本地陪伴内容 |
...
通话、录音、播报和离线播放占用麦克风时，KWS 监听暂停。无视觉工况下，KWS 进程在安静环境下 CPU 占用约 13%，嘈杂环境下约 23%～31%；实测端到端延迟控制在几百毫秒以内。
...
人物检测选用低分辨率轻量网络，以适配单核约 1 GHz 的 2K0300。早期对比中，YOLO11n-Pose 和手部 Landmark 的每帧耗时达到数秒，FastestDet 的人物检测延迟也较高。表 9-2 在同图、同 benchmark 条件下比较纯 CNN 推理耗时，展示轻量模型的速度优势。
...
![图 9-2 同条件板端纯推理延迟对比](assets/vision/board_latency_comparison.png)
...
在同一组冻结测试帧上，Recall 从 0.8813 提升至 0.9647，正样本逐帧检出率提升 11.8 个百分点。逐帧检出按 `score≥0.25` 且最佳框 `IoU≥0.50` 统计。该组测试仍有 3 张困难场景帧未检出。Precision 为该测试集上的结果；冻结集未包含确认的纯无人样本，空场误报在 V1.3 新域负样本中另行评估。
...
总链路包含 JPEG 解码、预处理、CNN 推理和后处理；两组 RSS 分别为 41,296/41,920 kB，`LongPetVisionBench` 进程 CPU 分别为 90.29%/55.91%。两组数据体现了单核并发条件对检测吞吐的影响，为 Detector/Tracker 调度提供性能基线。
...
端侧视觉以 Detector+Tracker 调度在有限算力下保持目标位置更新，并通过最新帧处理与观测时效避免旧画面积压。表 9-6 测量连续 CNN 检测吞吐，表 9-7 测量协同后的目标更新频率。项目通过 KWS 自适应 VAD、低频检测和媒体资源仲裁控制并发开销。相关模块启用的实机工况下，加载期 CPU 短时约 100%，随后约 40%～80%，内存约 0.18 GB，支持当前测试中的功能组合。录音、播报和通话按会话交接资源；持续运行、温升及不同负载下的稳定性在后续专项测试中量化。
...
LongPet 通过业务 Service、统一数据契约和设备适配层连接老人端、家属端、AI 能力与运动 MCU。页面和外部请求提交操作意图，业务层完成校验，设备与服务反馈执行结果。这一设计明确了数据归属和控制权限，也便于在网络、设备或算法状态变化时保持一致的用户反馈。协议字段与实现索引汇总于附录 B。
...
具体错误与恢复方式见附录 B.7，MCU 独立停车保护见第 11 章。

## 11 运动控制与系统安全设计

### 11.1 设计目标与安全原则

LongPet 的运动控制以控制权明确、命令有效、失效停车和受控恢复为原则。系统分别校验模式、会话、目标与命令时效，在目标丢失或过期、远控端失焦、串口断线、故障或租约到期时撤销底盘驱动；主控任务阻塞导致无法续租时，由 MCU 超时保护停车。恢复运动须重新满足模式、命令和时效条件，通信在线状态单独维护。视觉误检风险通过模型评估、稳定性门控和受控运行条件降低。

### 11.2 双处理器运动控制架构

运动安全采用**上层策略约束 + MCU 独立执行保护**。龙芯 2K0300 主控负责远控接入、视觉感知、运动策略和控制权仲裁。ESP32-S3 运动 MCU 负责模式与命令校验、时效检查、传感器故障处理和输出撤销。MCU 通过 TB6612 驱动电机，在自身供电和程序正常运行时，即使 Linux 应用失去响应，也能独立撤销底盘输出。
...
Family Desktop 发起远控会话，LongPet 校验会话身份、串口状态、MCU 在线状态及故障状态，同一时间只允许一名远控控制者。进入 `MANUAL` 前，`MotionService` 预留手动控制权、释放自动控制，再执行 `STOP → MODE MANUAL → STATUS`；退出时执行 `STOP → MODE SAFE`。MCU 失联、模式意外变化、故障或串口断线时，主控撤销控制权。后续业务 HTTP 请求由 Electron 主进程携带长期 Bearer Token，渲染进程使用短时运动会话令牌；连接弹窗输入时的凭据处理见第 10.9 节。当前 HTTP/WS 通道面向受控局域网，跨不可信网络接入时增加加密传输或可信中转。
...
《LongPet 产品运动功能测试报告_2026-09-13_V1.3》附录 A 共列 58 项：56 项通过，左右平移两项（15、16 号）按版本计划暂缓，待验证项为 0。表 11-3 列出运动安全设计与测试编号的对应关系。
...
| --- | --- | --- |
| 响应和资源 | 本地操作独立于在线请求；视觉处理最新帧；媒体设备按会话交接 | 板端感知吞吐与目标更新已测量；补充长稳与在线时延测试 |
...
| --- | --- | --- | --- |
| LoongArch AI 软件栈 | 直接使用同架构预编译包及默认优化路径 | 固定工具链源码构建、标量 MLAS 路径和 ISA/ABI 检查 | 以受控构建和源码补丁保障目标板兼容性与推理数值正确性，按标量性能安排任务负载 |
| 系统构建 | 完整替换为新 Buildroot Rootfs；早期实机稳定性未达要求 | 保留稳定 BSP，选择性合入组件形成 Hybrid Rootfs | 复用稳定内核与驱动，通过清单管理新增组件及兼容版本 |
| 人物检测模型 | YOLO11n-Pose、手部方案及 FastestDet；后者板端延迟较高 | TinyissimoYOLO person-only 正式 V1.2 模型 | 聚焦人物检测，降低输入规模与单次推理延迟 |
| 持续人物感知 | 每次目标更新都运行 Detector | 低频 Detector 搜索与纠偏，Sparse LK Tracker 更新间隙位置 | 在检测间隙更新人物位置，以状态、重获和时效管理维持观测有效性 |
| 语音交互 | 完整本地开放式 AI，或全部依赖在线服务 | 本地 KWS、离线快捷能力与在线 ASR/LLM/TTS 组合 | 本地保留唤醒与快捷操作，在线服务补充开放式对话，控制端侧资源需求 |
| 人物跟随 | 深度测距或 SLAM 等完整空间感知路线 | 人物框远近代理、头身协调与目标时效门控 | 以普通摄像头的相对尺度实现低成本、受控低速跟随，真实测距与自主导航另行扩展 |
| 电机控制 | 由 Linux 主控直接承担运动执行与失联判断 | ESP32-S3 独立执行模式、租约和故障停车 | 将实时执行与失联保护集中到 MCU，通过双向 UART 同步控制和状态 |
...
人物框提供方向与相对尺度，LongPet 用 `HEAD_ONLY` 和 `FOLLOW` 区分头部关注与底盘动作：结合 MCU 反馈的头部偏移决定是否转动机身，以人物框归一化高度划分 FAR/GOOD/NEAR 距离代理，并要求目标新鲜、稳定。该策略在有人看护的低速环境实现受控跟随；真实测距、地图与障碍感知属于后续扩展，安全条件见第 11.9 节。
...
项目将目标 ISA/ABI、源码构建、标量推理和板端数值比对串成完整的 AI Runtime 适配链，使本地 KWS 与人物视觉在无 LSX/LASX 的 2K0300 上运行。通过统一 MLAS 标量内核与矩阵打包格式，十项板端算子测试全部通过，受测 Conv、LayerNorm、Softmax 最大绝对误差均低于 `1×10⁻⁵`。项目形成了从源码适配到数值验证的完整方法，可用于后续运行库升级与模型迁移。

### 16.2 可校验的 Hybrid Rootfs 交付方法

Hybrid Rootfs 保留稳定板端的内核、驱动和核心运行库，选择性引入 Buildroot 的 Qt/AI 用户态组件，再通过清单、依赖闭包、符号版本、ISA、受保护文件 hash 和整包 SHA-256 校验交付。2026 年 8 月 29 日的合并检查覆盖 1739 个 ELF，必需组件缺失和未解析依赖均为 0，受保护文件保持一致。该方法把系统扩展纳入可检查、可追踪的发布流程，降低组件遗漏与基础系统覆盖风险；跨环境重建与长期可靠性纳入后续验证。

### 16.3 面向持续人物感知的轻量视觉与分层推理

正式 V1.2 人物模型经产品视角微调，在 85 张冻结测试帧上的正样本逐帧检出率由 84.7% 提升至 96.5%。低频 Detector 与 Sparse LK Tracker 协同，在单人物板端测试中提供约 7～9 Hz 的目标位置更新；带时间戳和新鲜度的 `TargetObservation` 由 AI 视野、自动跟头和人物跟随复用。这一方案同时优化产品视角检出表现、目标更新频率与跨功能复用，使普通摄像头和单核 CPU 能够支撑持续人物感知。遮挡、多人关联和推理空档是下一阶段的优化重点。
...
Vision 提供人物观测，LLM 在白名单内提出业务工具操作；相关输出先进入业务层校验，`MotionService` 校验模式、会话与时效，MCU 独立执行故障和停车约束。旧目标或失联会话因此无法持续获得运动授权，感知、决策和执行的责任边界清晰。物理急停、障碍感知和整机安全验证列入后续工程化。
...
LongPet 已形成可运行的比赛原型，系统平台、端侧 AI、家属协同和运动功能分别完成相应层级的验证。成果按验收状态成组管理，下一阶段集中完善适老测试、整机保护与部署可靠性。
...
当前单核主控以轻量模型、受控并发和在线 Provider 组合提供能力；在线语音、天气和远程通话需要网络及对应服务。单 RGB 相机与水平单轴头部支持人物关注，跟随使用人物框归一化高度作为远近代理，运行范围为有人看护的低速场景。下一阶段量化速度、停距、长期运行、RTC 断电保持及整机电气和结构表现。约 700～800 元为主要原型硬件估算，量产成本与服务器或近端算力板配置分别核算。
...
验证结论按实际版本和条件使用：软件测试用于逻辑与接口，板端测试用于目标环境，实机测试用于硬件动作。测试路径与执行结果分别记录，既有实机报告沿用原测试编号。

测试记录包含需求 ID、版本、设备与网络条件、步骤、期望与实际结果、结论及记录位置。正常与异常场景共同检验需求；新增正式用例编号及基线变更时同步更新映射。
...
| FR-CARE-02 | 关怀信息真实性 | 8.3、8.8、10.6 | LongPet：CareService、CareEventRepository；Family Desktop 关怀摘要 | UT；E2E 数据核对 | IMPLEMENTED | 设计见 8.3、8.8；展示与事件来源专项验收待完成 |
...
当前状态为 `VERIFIED` 69 条、`IMPLEMENTED` 5 条，共 74 条，与第三章逐项对应。专项验收按对应版本和工况补充。

### 附录 B 系统接口索引

本附录汇总接口方向、关键字段、错误处理和实现位置，便于开发联调与后续维护。路径为各工程内的相对路径；`LongPet:`、`Family:`、`MCU:` 分别指主程序、Family Desktop 和 Motion MCU 工程。`<BASE_URL>`、`<API_KEY>`、`<TOKEN>`、`<DEVICE_ID>` 均为脱敏示例参数。
...
| 服务权限 | `User`、`Group`、`SupplementaryGroups` | 设备访问身份 | 部署必需 | 专用部署用户/组；附加 `video input audio tty dialout` | 部署时按设备权限配置专用用户与附加组 |
...
| Wi-Fi 恢复 | `rtl8xxxu-wlan0-recover.service`：`ExecStart`、`TimeoutStartSec` | 启动时无线恢复 | 按外设需要 | `<SYSTEM_SBIN>/rtl8xxxu-wlan0-recover`、`70` s | oneshot，日志到 journal；无线凭据由系统网络配置管理 |
...
| 宏科头部舵机（商品规格“20 kg”） | 信号 GPIO2；5 V 与 GND | 中心约 1570 μs，软件 870～2270 μs；实机机械总行程约 120° |
...
| 机械范围与验证 | 原型机械总行程约 120°；软件边界、方向、回中及与底盘并发在 V1.3 报告中通过 |
...
| --- | --- | --- |
| 摄像头 | OV2735 方案、200 万像素 UVC；`1bcf:2281`，`XHH-260128-A 2M`，`uvcvideo`；`/dev/video0` | `LONGPET_CAMERA_DEVICE=/dev/video0`；当前 GStreamer/V4L2 请求 640×480 MJPEG@30，物理倒装配置旋转 180°；设备枚举支持 1920×1080 MJPEG@30 |
| USB 音频 | C-Media Electronics Inc. USB Audio Device，`0d8c:0014`，ALSA `CARD=Device` | 录放 `plughw:CARD=Device,DEV=0`；mixer `hw:CARD=Device`；当前适配器依次尝试 `PCM`、`Speaker`，实机 `Speaker` 控件可用 |
| USB Wi-Fi | Realtek 802.11n，`0bda:f179`，`wlan0` | 当前主要联网接口；软件识别 RTL8188FU，驱动 `rtl8xxxu`；启动恢复已验证 |
...
ONNX Runtime 与 OpenCV 版本及项目兼容修改见 E.3、E.4；训练与评测结果见第 9 章。
