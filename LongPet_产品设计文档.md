...
LongPet 主控决定跟头、跟随和人工接管等高层行为，MotionService 校验会话、控制权、目标新鲜度和 MCU 状态；ESP32-S3 MCU 在停止、失联和故障状态下独立撤销运动授权。Vision 与 LLM 的输出本身不具有运动授权。具体 STOP、STBY、租约和故障锁存机制及已开放功能的验证范围见第 11 章。当前软件安全机制不能替代物理急停、电流与电源保护、机械防护等量产级硬件保护。

### 13.7 网络、数据与隐私保护

提醒、照护事件和设置按第 10.6 节持久化；有限 AI 对话上下文、天气快照及通话、视觉、远控会话属于进程或会话态，重启后不恢复旧控制权。音频和摄像头数据随当前任务采集、传输与释放，家庭媒体流使用短时会话令牌；长期凭据由受控配置和可信进程持有，不向普通界面状态或业务响应暴露。AI 与天气服务密钥不写入文档，具体配置与进程边界见第 7、10 章。受控局域网的 HTTP/WS 鉴权不能等同于传输加密，当前通道不直接暴露公网；跨不可信网络需要后续加密或可信中转。配置访问权限和凭据保护专项验收仍待归档，系统不宣称医疗诊断或全面隐私合规认证。
...
现有 ASR、LLM、TTS Provider 抽象与设备 Port 使业务逻辑不直接绑定供应商协议或摄像头/音频具体节点。更换已支持 Provider 或 Adapter 时，应保持 Service 的输入、结果、取消和错误语义；接入新的硬件或 AI 服务还需完成协议适配和验证。当前完成的是终端侧可扩展接口基础，自建推理服务与算力板接入仍属后续规划，不纳入本次功能验收；具体部署形态见第 14.11 节。
...
后续部署可选两种形态：LongPet 经网络连接局域网/远程 LoongArch AI 推理服务器，或经本地/近端接口连接 LoongArch AI 算力板。两者均通过终端 AI Provider/Service 边界接入，部署时分别配置服务地址或设备接口，并核对鉴权、网络或设备失效后的回退条件。当前自建服务器与算力板均未交付或验收；正式路线与选择条件见第 17.5—17.6 节。

## 15 关键技术难点与方案权衡

技术取舍围绕目标板算力、稳定基础系统、交互连续性、运动安全和原型成本展开。表 15-1 概括备选方案与最终设计；以下各节说明选择依据及其代价和边界。

| 设计问题 | 比较的方案 | 最终设计 | 代价与边界 |
| --- | --- | --- | --- |
...
2K0300 无 LSX/LASX，部分同为 LoongArch 的现成包或运行时路径并不适合直接使用。仅以交叉编译成功、运行库可加载或 Python/NumPy 可导入作为验收标准，不足以保证板端 AI 推理正确；语音样例曾出现数值异常。相较于继续依赖原有运行时路径或放弃本地模型，最终设计选择在固定工具链下源码适配并逐层验证：NumPy 等组件源码构建，ONNX Runtime 1.17.1 对目标构建启用一致的标量 MLAS 路径，并检查 LoongArch ELF、ABI、16 KiB 可加载段对齐、实际加载库和新增产物的 ISA。第 6.5 节记录了已定位的打包布局与标量内核不匹配及算子、语音样例的板端比对；这一定位只适用于已验证问题，不外推为所有 LoongArch 软件的统一故障原因。代价是源码补丁和标量执行的维护成本，后续升级必须重新做数值与指令审计。

### 15.2 Buildroot 新软件栈与稳定板端系统

Qt6、AI 和媒体组件需要较新的用户态依赖，而完整替换板端基础系统的实机稳定性未达到项目要求。保留旧系统而不扩展用户态会阻碍应用功能；整体替换又会同时改变已验证的内核、驱动和运行库，扩大故障面。因此最终采用 Hybrid Rootfs：稳定板端 rootfs、内核和 ramdisk 保持基线，Buildroot target 只按清单提供新增用户态组件。合并过程审计 DT_NEEDED、动态加载插件、glibc/GLIBCXX/CXXABI、LoongArch ISA、locale 与受保护文件 hash，结果见第 6.8 节。代价是合并和回归检查比单一构建树更复杂；此方案保证的是当前输入组合经检查可部署，并非任意版本包均可混合。

### 15.3 视觉模型选型、领域适配与成本约束

YOLO11n-Pose、手部 Landmark 等方案在目标板上的数秒级单帧耗时，不适合作为持续人物交互入口；较轻的 FastestDet 仍慢于 Tinyissimo 基线。模型选择同时权衡通用识别范围、128×128 输入下的单核推理延迟，以及普通 USB 摄像头的成本。最终采用 TinyissimoYOLO-v1-small 的 person-only 结构，从 COCO 人物数据训练，再用固定机位实拍图进行 V1.2 域微调。第 9.3 节的 Frozen Test 与 ONNX parity 支持正式 V1.2；V1.3 扩域实验改善新场景但降低原冻结域部分检出表现，因此未替换正式模型。代价是不能宣称通用姿态、身份或环境识别，遮挡、无人负样本和多人关联仍需继续验证。普通摄像头、单轴头部和通用底盘也有助于控制原型成本；产品经济性集中见第 16.7 节。
...
在单核、有限可见内存且无 swap 的板端部署完整本地 LLM，会与界面、KWS 和 Vision 争夺资源，也无法满足当前开放式交互时延目标。完全依靠在线服务则使断网时连基础入口也失效。因此当前设计以本地 KWS、有限快捷动作和离线陪伴支撑基础操作，以在线 ASR/LLM/TTS Provider 提供开放式对话；Provider 超时或取消仅影响当前会话。本地词条“救命”打开紧急页面，不自动拨号；“停止”取消语音/陪伴，不等同底盘 STOP。这一分工以网络和第三方服务依赖换取当前终端可承担的开放式智能能力，后续算力扩展路线见第 17.5—17.6 节。

### 15.7 Linux 智能主控与 MCU 安全执行的分层

视觉观测和家属远控存在延迟、丢帧、网络中断与状态过期，Linux/AI 决策不适合成为底盘最终安全保证。直接由高层命令驱动电机虽可减少一层协议，却会使故障处理依赖于主控任务仍能及时运行。因此由 LongPet 决定模式和行为，通过 MotionService 校验控制权与目标时效；ESP32-S3 独立执行协议校验、租约、SAFE/STOP 和故障锁存。人工接管先停自动动作，退出后人物跟随不自动恢复；目标框大小只是远近代理，不能当作测距或避障。额外的 MCU、协议和验证增加工程复杂度，但为通信失效提供执行侧停车条件。已开放运动功能的验证及自由滑行边界见第 11 章；物理停距、急停和供电保护仍需产品化验证。
...
现有 Provider/Service 抽象使终端业务可通过一致的接口使用第三方在线 Provider，并为自建 LoongArch AI 推理服务器或本地/近端 LoongArch AI 算力板预留接入边界。其技术价值是把终端业务语义与推理算力部署位置分开，使未来选择后端时不必重新绑定提醒、交互和运动业务。当前完成的是终端侧接口基础；Server 和 Compute Board 均为后续规划，尚未作为正式能力交付。

### 16.7 低成本条件下的完整产品闭环

LongPet 在约 700～800 元量级的当前原型主要硬件成本下，完成 LoongArch 主控、触控显示、普通摄像头、本地 KWS、本地人物视觉、在线语音、Family Desktop、四轮底盘和单轴头部的系统集成。设计没有依赖高价 GPU 主机、LiDAR 或深度相机，而以轻量模型、普通摄像头和低成本 MCU 实现本地 AI、家属协同及受控运动闭环。成本控制属于产品工程价值，不是单项算法创新。该金额不是量产 BOM 或最终售价，也不包含外壳、认证、人工、物流、售后和未来算力扩展。
...
当前能力按 VERIFIED、IMPLEMENTED 和 PLANNED 三类管理：分别表示在指定条件下已有验证证据、实现已完成但专项验收仍待补充，以及未来规划。核心业务、本地 AI、FamilyLink 和已开放运动功能已形成相应软件、板端或实机证据；逐项状态与测试数量见附录 A。本章的成熟度判断不等同于量产认证。

| 状态 | 当前能力或事项 | 设计与验收边界 |
| --- | --- | --- |
| VERIFIED | Hybrid Rootfs、正式 V1.2 视觉与本地 KWS、提醒及 FamilyLink、Family Desktop 协同、自动跟头、人物跟随和 MCU 已开放运动功能 | 在各自软件、板端、双端或实机测试条件下成立；详细证据见附录 A |
| IMPLEMENTED | 关怀未知数据展示、核心存储故障报告、适老异常提示、敏感凭据保护、跨模块诊断 | 实现已具备，专项验收记录仍待归档；对应需求见附录 A |
| PLANNED | 自建 LoongArch AI Server、AI Compute Board 插件、环境感知和量产级硬件保护 | 当前无正式交付及功能验收 |

**表 17-1 当前能力与证据等级。** VERIFIED 只覆盖相应测试对象与工况；具体版本、测试数量和待补项由附录 A 及对应报告管理。
...
受单核 CPU、有限内存和无交换空间等资源条件约束，高负载视觉与完整本地大模型不适合长期并发常驻；在线 ASR/LLM/TTS、天气和远程通话依赖相应网络与服务。原型摄像头固定、头部只有水平单轴，自动跟随限于有人看护的低速环境；实际速度、停距、长期运行、断电 RTC 保持以及量产电气/外壳仍需量化。约 700～800 元只是比赛原型主要硬件的估算，不覆盖外壳、认证、人工、物流、售后。未来服务器作为共享或外部算力不必逐台计入终端 BOM；本地算力板会增加单机成本，需单独形成档位与核算。具体板端资源测量见第 6、9 章。

### 17.5 后续版本规划

近期 AI 算力扩展有两条并行可选方向。方向 A 是 LoongArch AI Server 协同，重点验证 ASR/LLM/TTS 等服务的接口、鉴权、时延和网络失效回退；方向 B 是本地/近端 AI Compute Board，重点验证供电、散热、驱动、模型兼容与单机成本。两者不是严格先后版本关系，可依据成本、隐私、网络条件、时延、功耗和部署环境选择其一或组合使用。

其他产品化工作包括增加障碍/悬崖等环境感知及相应安全策略，完善物理急停、驱动默认禁能、限流、电源与 BMS、机械结构和必要认证，逐步扩展私有化/本地化 AI 服务，并补齐老年用户体验、长期稳定性、停距和跨模块诊断验证。每项后续能力都需独立定义版本、测试条件和放行标准，不能以接口预留替代完成验收。

### 17.6 LoongArch AI 推理基础设施演进路线

当前基础形态是“2K0300 终端本地 KWS/视觉 + 已接入在线 AI Provider”，实时界面、业务与运动裁决留在 LongPet 主控和 Motion MCU。未来的 Server 协同版经受控 LAN/WAN 获取共享推理服务，Compute Board 增强版经本地或近端接口获取加速推理；两者通过同一 Provider/Service 边界接入。两种形态可选用或组合，并非固定先后版本关系。相关接口还需验证超时、离线回退、数据保护与版本兼容；服务器和算力板均属 PLANNED，当前没有可供正式验收的部署产物。
...
flowchart LR
    ...
    P --> M["LongPet 业务校验与 MotionService<br/>Motion MCU 安全执行"]
```

图 17-1 LoongArch AI 推理能力演进路线。实线为当前终端能力，虚线为两条并行可选、尚未交付的算力扩展路线；实时控制与运动安全仍由 LongPet 主控和 MCU 裁决。
...
当前比赛原型形成从本地多模态感知、在线/离线语音交互、家属端协同，到低速运动执行与失效停车的产品闭环。当前主要原型硬件成本估算约 700～800 元，不代表量产 BOM 或产品售价。软件、板端、模型和运动已有分层验证；尚待补充的专项验收、老年用户体验、长稳与量产级硬件保护见第 17 章。

后续可根据部署条件选择 LoongArch AI Server 或本地 AI 算力板扩展，并继续完善整机工程化与验证。两种推理扩展目前均为规划，不属于本次正式功能验收。
