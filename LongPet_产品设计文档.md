...
本章以当前已完成设计、实现及验证材料为依据，建立 LongPet 比赛交付版本的需求基线。产品面向老人日常操作负担、事项易遗漏、家属不在身边时的信息与联系需求，以及可运动设备必须受控的问题，提供本地陪伴与提醒、家庭协作、人物感知和受限运动。每条需求描述应得到的行为结果，设计与实现由第 4—11 章说明。

### 3.1 需求来源、优先级与版本范围

本轮需求基线标识为 **REQ-BASE-2026-09-15**，用于追踪本章和附录 A，不改变其他章节的软件、模型或文档版本号。

| 来源层级 | 本轮使用范围 | 判定规则 |
| --- | --- | --- |
| 当前已确认设计与代码 | 第 4—11 章、附录 B/D；必要时回查当前主程序、家属端及运动接口 | 决定真实功能、约束与当前用户流程 |
| 最新测试与交接 | 运动功能测试 V1.3、第 6 章板端验收、第 9 章感知与并发结果、Vision Final Handoff | 支持验证状态；保留测试工况和版本边界 |
| 专题与历史报告 | 当前工程内相关业务、语音、天气、视觉、通话报告及 tests | 补充软件证据；早期“待上板”不得推翻后续已确认实机结果 |
| 早期设想与未来计划 | 当前尚未完成的扩展项 | 不进入当前正式需求表和优先级统计 |

**表 3-1 需求来源与事实优先级。** 测试文件存在仅表示有验证入口，不能单独证明测试通过；已有确认结论与原始测试记录的关联方式见 3.13 和附录 A。
...
**表 3-2 统一需求优先级。** 优先级不等同于开发难度或验证状态；例如跟随功能为 P1，但其目标失效停车及控制权规则为 P0。

需求 ID 采用 `类型-领域-两位序号`：FR 为功能需求，NFR 为非功能需求，CON 为约束。FR 使用 UI、REM、CARE、WEATHER、VOICE、VISION、CALL、FAMILY、MOTION、MM；NFR 使用 PERF、STAB、USAB、SAFE、SEC、MAINT、REPRO；CON 使用 PLAT、HW、NET、RES。编号在所属领域内递增，移动章节不改号，删除后保留作废记录而不复用；新增需求追加编号，语义发生实质变更时记录变更及受影响测试。

当前基线不含自建 LoongArch AI Server、完整本地 LLM、持久长期记忆、人脸身份识别、医疗监测或跌倒诊断、自动报警、SLAM、完整自主导航与避障、自动倒车追人、横向平移产品入口、量产电源/BMS、独立物理急停及尚未完成的硬件保护。以上均为 Future／非当前交付基线，不能因为底层存在接口或界面占位就转为 P0 要求。

### 3.2 功能需求总览

| 用户问题与需求类别 | 需求编号范围 | 产品应提供的结果 | 对应设计 |
| --- | --- | --- | --- |
| 老人操作负担大、难理解状态 | FR-UI | 浅层触摸入口、陪伴反馈及触摸语音互补 | 第 7、8 章 |
| 日常事项易遗漏、家属难了解已记录照护 | FR-REM、FR-CARE、FR-WEATHER | 提醒闭环、真实今日关怀、可降级天气信息 | 第 7、8、10 章 |
| 老人希望自然表达、网络不总可靠 | FR-VOICE | 在线问答与受限工具；离线快捷和配置音频 | 第 7—10 章 |
| 希望设备朝向人物并提供受控协助 | FR-VISION | 人物观测、AI 视野、仅头部关注和受限跟随 | 第 8、9、11 章 |
| 家人异地于设备、需要联系和管理 | FR-CALL、FR-FAMILY | 双向家庭通话、真实状态、管理及人工协助 | 第 7、8、10、11 章 |
| 自动能力和人工操作可能冲突 | FR-MOTION、FR-MM | 明确控制权、失效停车和跨模态让渡 | 第 4、7、10、11 章 |

**表 3-3 功能需求分类与产品结果。** 老人以触摸和语音提出意图，LongPet 保存本地业务并执行受控动作，家属承担管理与协助；同一业务状态在三方间保持一致。本章以需求表为主，不重复后续技术架构图。

### 3.3 老人端适老交互与本地业务需求

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-UI-01 | 系统应在触摸首页直接提供陪我说话、今日关怀和视频通话入口，并允许从今日关怀进入提醒管理。 | P0 | 从首页逐一进入三个功能并返回；从关怀页可进入提醒列表，页面切换不丢失已保存事项。 | 7.2—7.3、8.2 |
| FR-UI-02 | 系统应以表情或文字区分待机、倾听、思考、说话及异常，并显示网络和通话的实际状态。 | P0 | 驱动各业务状态核对界面；能力不可用时出现提示，不能持续显示成功或在线。 | 7.2、8.2、10.10 |
| FR-UI-03 | 系统应允许老人调整音量、亮度和陪伴风格，并在正常重启后保留已保存设置。 | P1 | 修改可用设置并重启读取；相应硬件能力不可用时反馈失败或不可用。 | 7.4、8.2、10.6 |
| FR-UI-04 | 系统应在语音不可用或监听暂停时保留触摸导航、取消及挂断入口。 | P0 | 关闭在线语音或进入媒体占用状态，仍可触摸进入本地业务并取消当前会话。 | 7.6、8.2、8.4、8.6 |

**表 3-4 FR-UI 需求及验收口径。**

### 3.4 提醒、今日关怀与状态查看需求

提醒的断网能力以设备供电、应用与本地存储正常、系统时间有效为前提；断电期间无法提醒，RTC 断电保持不作已验收承诺。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-REM-01 | 系统应支持老人触摸、家属端和在线语音工具创建提醒，并统一管理同一份本地提醒数据。 | P0 | 分别创建合法提醒，在本机和家属端刷新后查到同一记录；不合法标题、时间或重复规则被拒绝。 | 7.2—7.4、8.3—8.4、10.6 |
| FR-REM-02 | 系统应支持本机和家属端管理提醒，并允许在线语音查询及按明确对象删除提醒。 | P1 | 修改、查询、删除指定提醒，其他提醒不受影响；远程旧版本修改被拒绝并提示刷新。 | 8.3—8.4、10.3、10.10 |
| FR-REM-03 | 系统应按本机时间触发已启用提醒，区分触发与完成；仅在老人确认后记录完成。 | P0 | 在有效时钟下触发事项，未确认保持未完成；确认后状态更新，同日调度不重复投递已处理事件。 | 7.4、8.3、10.6 |
| FR-REM-04 | 系统应持久保存提醒及事件，并在断网但本机正常运行、时间有效时继续调度已有提醒。 | P0 | 保存后正常重启仍可读取；断网推进到提醒时刻仍触发，不要求网络服务返回。 | 4.1、7.4、8.3、10.6 |

**表 3-5 FR-REM 需求及验收口径。**

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-CARE-01 | 系统应允许记录饮水，并将已记录饮水和提醒完成等事件汇总为本机及家属可查看的今日关怀。 | P1 | 记录饮水、确认提醒后核对当日摘要及持久记录；家属刷新后与设备数据一致。 | 7.4、8.3、10.6 |
| FR-CARE-02 | 系统应仅将具有可靠来源的照护事件作为有效记录，未自动采集的活动或健康指标不得表述为自动健康监测结果。 | P0 | 核查摘要来源与字段语义；无可靠数据项呈现未知或待接入，不以界面占位值声称测量结果。 | 8.3、8.8、10.6 |

**表 3-6 FR-CARE 需求及验收口径。**

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-WEATHER-01 | 系统应在网络和配置可用时更新天气；更新失败保留本次运行已有快照，首次无数据时显示未知，网络恢复后补刷。 | P2 | 覆盖成功、首次失败、已有快照后失败及重连；无数据为“--”或明确未知，不生成虚假实时天气。 | 7.4—7.5、8.3、10.4 |
| FR-WEATHER-02 | 系统应保留天气最近成功获取时间，判定快照是否过期；在线语音引用过期快照时应携带旧数据提示。 | P2 | 将快照置于过期状态，检查更新时间、过期状态和送入语音回答的提示；不将缓存当作刚获取的数据。 | 8.3、10.10、B.4、B.7 |

**表 3-7 FR-WEATHER 需求及验收口径。**

### 3.5 AI 语音交互与离线陪伴需求

离线范围为本地关键词、快捷业务和已配置音频，不包括完整离线大模型。紧急页面仅是入口，不承诺自动报警；语音“停止”受监听是否可用约束，不能替代触屏取消或独立运动停车保护。自建 LoongArch AI Server 不属于本轮交付基线。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-VOICE-01 | 系统应在本地监听可用时响应已配置唤醒词，并按在线能力进入语音会话或本地快捷提示及动作。 | P0 | 在线唤醒进入倾听；离线唤醒可进入报时、提醒页面、返回主页或联系家人入口等已配置动作。 | 7.6、8.4、9.2 |
| FR-VOICE-02 | 系统应在在线服务可用时完成语音识别、自然语言处理和语音合成播放，并允许触摸发起会话。 | P0 | 输入日常问题，核对识别文本、回答与播报；结束后恢复待机，失败呈现对应状态。 | 7.6、8.4、10.4 |
| FR-VOICE-03 | 系统应仅执行已授权的提醒、时间查询及限定页面等工具动作，校验参数后将真实执行结果用于回答。 | P0 | 创建、查询及明确对象删除提醒成功；未知工具、错误参数、失效对象被拒绝，不能口头虚报操作成功。 | 7.6、8.4、10.4 |
| FR-VOICE-04 | 系统应保留有限轮次的进程内对话上下文供连续问答使用，并支持清空；重启后不恢复该历史。 | P1 | 连续问答可引用前文；超过配置轮次后裁剪；清空或重启后不再引用旧历史。 | 8.4、10.7 |
| FR-VOICE-05 | 系统应在在线 AI 不可用时保留本地快捷能力，并在配置了可用陪伴音频时支持播放，缺少文件时明确提示。 | P0 | 断开外网后测试本地快捷词及已配置音频；音频缺失提示不可播放，触屏在线尝试失败可结束或重试。 | 7.5—7.6、8.4、9.2 |
| FR-VOICE-06 | 系统应支持取消语音会话及陪伴播放，取消后的旧结果不得改变新会话或继续触发业务动作。 | P0 | 录音、请求、播报阶段分别取消；迟到结果不更新新会话；可重新发起会话。 | 7.6、7.10、8.4、10.10 |
| FR-VOICE-07 | 系统应将网络、鉴权、超时和音频失败转为可理解提示；合成失败时允许保留已有文字回答并结束或重试。 | P0 | 注入各阶段失败，界面退出相应等待状态，触摸和本地业务仍可使用。 | 7.5—7.6、8.4、B.7 |

**表 3-8 FR-VOICE 需求及验收口径。**

### 3.6 视觉感知、自动跟头与人物跟随需求

当前人物位置不包含身份认证，画面尺寸仅用于粗略远近判断。人脸身份识别、医疗跌倒诊断、SLAM、完整自主导航、完整避障、悬崖检测和自动倒车追人均不属于当前需求。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-VISION-01 | 系统应在摄像头和视觉能力开启、可用时发现受测场景中的人物，并区分发现、搜索和丢失状态。 | P1 | 按第 9 章冻结场景核对人物输出；无有效目标显示搜索或丢失，不承诺所有场景均可检出。 | 8.5、9.3—9.4 |
| FR-VISION-02 | 系统应持续提供当前人物位置、目标状态和观测时效，供视觉交互与自动控制使用。 | P1 | 人物移动时位置持续更新；中断和重获时状态随之变化；性能证据按第 9 章相同工况核对。 | 7.7、9.4—9.5、10.8 |
| FR-VISION-03 | 系统应在仅头部模式下按有效人物位置调整头部朝向，底盘始终不因该观测启动。 | P1 | 左右移动目标验证头部关注；轮组保持停止；无有效观测停止使用旧位置。 | 8.5、11.4、11.9 |
| FR-VISION-04 | 系统应允许家属查看当前画面及有效人物框和感知状态；关闭视野页面不应关闭本地感知。 | P1 | 查看及关闭远程视野，检查本地感知连续性；无新鲜目标隐藏框，断流清除旧画面。 | 7.7、8.5、B.3、B.5 |
| FR-VISION-05 | 系统应在家属显式开启后确认目标并调整朝向，仅在距离代理判为较远且安全条件满足时低速靠近，合适或过近时停车。 | P1 | 按既定受控场景验证确认、对齐、前进和停车；不自动倒车、横移或弧线追人。 | 8.5、11.9 |
| FR-VISION-06 | 系统应在目标丢失、过期或摄像头中断时停止依赖旧目标的自动动作；重获后重新确认目标。 | P0 | 遮挡人物或中断画面，旧框与旧控制目标失效，跟随底盘停车；新观测重新确认后才允许动作。 | 9.4—9.5、10.10、11.9 |
...
| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-CALL-01 | 系统应支持老人端发起视频呼叫，家属端接听或拒绝；支持家属端发起语音或视频呼叫，设备提示并准备媒体后接通。 | P0 | 分别走通两种方向及家属选择的两种媒体模式；老人等待可取消，被拒绝时获得提示。 | 8.6、10.3、B.3 |
| FR-CALL-02 | 系统应区分呼叫中、准备媒体、已接通和结束状态，并支持任一端挂断；媒体未就绪时不得显示已接通。 | P0 | 检查连接中及媒体失败状态；双端分别挂断后结束会话，旧通话操作不能影响新通话。 | 8.2、8.6、10.10、B.7 |
| FR-CALL-03 | 系统应按语音或视频模式启用所需媒体，在断线、设备占用或权限失败时反馈失败并释放已占用资源。 | P0 | 语音模式不依赖双方视频，视频模式有双向画面和声音；断开连接及模拟资源失败后可重新建会话。 | 7.10、8.6、B.3、B.7 |
...
| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-FAMILY-01 | 系统应向家属提供设备、网络、能力状态与今日关怀查看，并集中提供提醒、必要设置、AI 视野、通话及自动模式管理入口。 | P0 | 连接设备逐一访问入口；状态和已保存配置与设备一致；未知硬件数据不冒充实际测量。 | 4.3、7.8、8.7、10.3 |
| FR-FAMILY-02 | 系统应仅在设备确认后显示远程写入成功，连接失败、鉴权错误和修订冲突应分别反馈并允许刷新重试。 | P0 | 成功写入后本机可见；断网、错误令牌及旧 revision 不显示保存成功或覆盖设备新值。 | 8.3、8.7、10.3、10.10 |
| FR-FAMILY-03 | 系统应允许家属选择关闭、仅头部或人物跟随，并显示实际激活状态；条件不满足时拒绝切换并说明原因。 | P1 | 分别请求三种模式；视觉或运动不可用时不能显示自动控制已启动；反馈实际模式与状态。 | 8.5、8.7、B.3、11.8 |
| FR-FAMILY-04 | 系统应在取得人工控制权后支持按住前进、后退、原地左右转及头部左右调整、回中；松开底盘操作时停车。 | P1 | 逐一验证已开放动作和头部边界；松开、失焦、隐藏或退出页面后底盘停止；故障时禁用操控。 | 8.7、11.8、B.3 |

**表 3-11 FR-FAMILY 需求及验收口径。**

### 3.9 运动控制与安全需求

本节规定可观察的运动授权与停车结果，具体保护机制和时限见第 11 章。停车验收区分“撤销驱动输出”和“车体物理静止”；当前不规定未经标定的厘米级停距，也不宣称具备物理急停或完整硬件失效保护。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| FR-MOTION-01 | 系统应在 MCU 软件初始化阶段保持底盘禁能和安全模式，未经有效模式与命令授权不得自行运动。 | P0 | 复位及启动测试中轮组不自启；安全模式拒绝主动运动命令；不将此结论扩展至软件启动前电气状态。 | 5.13、11.3—11.4、D.9 |
| FR-MOTION-02 | 系统应隔离安全、仅头部、人工和跟随模式，切换模式时先停车并撤销旧运动授权。 | P0 | 跨模式命令被拒绝；切换后不沿用旧动作；仅头部不驱动底盘。 | 11.4、11.6 |
| FR-MOTION-03 | 系统应在主控或 UART 链路失效后由运动执行端独立撤销持续底盘动作，不依赖主控成功发送停止命令。 | P0 | 运动中断开 UART 或停止主控发送，按 11.6 的有效超时配置核对执行端撤销输出。 | 11.2、11.6、11.8 |
| FR-MOTION-04 | 系统应在人工底盘指令不再有效刷新后停车；诊断通信仍在线不得延长旧移动指令。 | P0 | 停止移动刷新但保留状态查询，底盘在 11.6 规定的命令有效期届满后撤销输出。 | 11.6、11.8 |
| FR-MOTION-05 | 系统应在跟随底盘指令失效后停车，持续目标或诊断消息不得替代新的跟随移动授权。 | P0 | 保留目标和诊断消息但停止跟随指令，按 11.6 的独立时限撤销底盘输出。 | 11.6、11.9 |
| FR-MOTION-06 | 系统应拒绝非法格式、越界参数及当前模式不允许的命令，且不得通过这些输入延长旧运动或目标有效期。 | P0 | 输入半包、超长、乱码、额外字段及越界值，核对无非授权动作且时效仍正常到期。 | 11.5、B.2 |
| FR-MOTION-07 | 系统应在严重运动故障后停车并禁止主动动作；普通通信恢复、停止或模式切换不得解除故障限制。 | P0 | 注入已覆盖 IMU 和控制周期故障，验证动作被拒绝；须排障并复位或重新上电后重新授权。 | 11.7 |
| FR-MOTION-08 | 系统应允许有效人工控制抢占自动运动，同一时刻仅允许一个人工控制者；退出接管后人物跟随须显式重新开启。 | P0 | 自动运动中接管，先停止自动动作再开放人工控制；第二控制者被拒绝；释放后不自行跟随。 | 7.9、8.7、11.8 |
| FR-MOTION-09 | 系统应在接受 STOP 后立即撤销底盘驱动和移动授权，头部保持当前位置；停止不等同于切断整机或舵机电源。 | P0 | 各模式发送 STOP，检查驱动撤销及授权清除；不自动回中；故障限制仍保留。 | 11.4、11.7、11.11、D.9 |
...
| FR-MM-02 | 系统应使 AI 视野、自动跟头和人物跟随共同使用具备时效语义的人物观测；查看远程视野不应额外争夺控制权。 | P1 | 同时查看视野与开启自动关注，检查目标语义一致；关闭查看不停止本地推理或获得运动权限。 | 7.7、9.5、10.8 |
| FR-MM-03 | 系统应协调通话、在线语音及离线陪伴对音频的占用，取得资源前暂停本地监听；交接失败应终止本次操作。 | P0 | 并发申请媒体时无重复占用；监听暂停确认超时则失败；释放音频后恢复可用监听。 | 7.10、8.4、8.6 |
| FR-MM-04 | 系统应在通话期间暂停视觉推理及仅头部自动关注、关闭人物跟随；结束后重新获取有效观测，仅头部按条件恢复，人物跟随须重新开启。 | P0 | 自动模式中接入通话，底盘停车；挂断后清旧观测；跟随不自行恢复，满足条件的仅头部可恢复。 | 7.10、8.5—8.6、11.8 |

**表 3-13 FR-MM 需求及验收口径。**

### 3.11 非功能需求

非功能需求约束当前业务质量，不新增未经实现的产品功能。验证状态及其具体范围集中列于附录 A。

#### 3.11.1 性能与响应时间

性能验收沿用明确工况的已有结果；第 9 章人物观测更新率不等同于模型检测 FPS。在线服务时延、UI 响应毫秒值及整机长稳时长尚无统一量化验收阈值，本轮不补造数字。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-PERF-01 | 系统应使关键本地导航、提醒查看及取消操作不等待外部网络请求完成。 | P0 | 让在线请求持续等待或超时，期间仍可导航、查看提醒和取消；不设无证据的毫秒级 UI 指标。 | 7.6、7.10、8.2 |
| NFR-PERF-02 | 系统应优先处理新画面并持续提供可用于当前交互闭环的目标观测，不因推理积压持续输出旧目标。 | P1 | 按第 9.4—9.6 节工况记录帧时刻和目标更新，受测闭环可运行；过期目标按 FR-VISION-06 失效。 | 4.1、7.7、9.4—9.6 |
| NFR-PERF-03 | 系统应在已配置 KWS、Vision 与媒体业务并发或切换时控制资源竞争，维持本地业务和停车保护。 | P0 | 按第 9.6 节已验证运行组合检查 CPU、内存、状态切换与提醒；不将单次测量作为全工况上限。 | 6.1、7.10、9.6、11.6 |
...
| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-STAB-01 | 系统应在单个 AI、天气、视觉或远程服务失败时报告该能力异常，并保持其他可用本地业务运行。 | P0 | 分别关闭 Provider、移走模型或禁用相机，检查提示及提醒数据可访问；不将核心数据库故障视为可选能力故障。 | 4.1、7.4—7.5、B.7 |
| NFR-STAB-02 | 系统应在语音、视觉或通话任务失败后释放相应资源或进入明确不可用状态，故障排除后允许重新发起可用功能。 | P0 | 模拟媒体准备失败、取消与断连，检查无旧会话占用；恢复设备条件后重新建会话。 | 7.5、7.10、8.6、B.7 |
| NFR-STAB-03 | 系统应支持板端应用自启动及异常退出后的服务恢复；恢复不得继承旧会话的运动授权。 | P0 | 整板重启与服务异常退出后检查应用恢复及新状态；旧人工/跟随会话不得自动恢复危险运动。 | 6.9—6.10、7.10、11.6—11.8 |
| NFR-STAB-04 | 系统应在核心数据库无法打开或版本初始化失败时明确启动失败，不以空白业务数据伪装正常就绪。 | P0 | 检查数据库失败返回及启动日志；仅在修复存储后重新启动，不能报告提醒已正常加载。 | 7.4—7.5、B.7 |

**表 3-15 NFR-STAB 需求及验收口径。**

#### 3.11.3 易用性与适老性

本节确认界面结构与可观察交互，不据此宣称已完成老人用户样本研究、阅读准确率或任务成功率的量化验证。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-USAB-01 | 产品应使高频入口直接可见，并用大按钮及简短状态降低操作负担；复杂管理集中于家属端。 | P1 | 按 8.2 导航完成说话、关怀和通话，老人无须进入部署配置；提醒由关怀入口到达。 | 4.1、7.2—7.3、8.2、8.7 |
| NFR-USAB-02 | 产品应在等待、失败、不可用及已结束时给出可读状态，并保留当前流程的退出或重试入口。 | P1 | 逐页走查网络失败、提醒冲突及通话失败，核对状态文案和退出路径可见可用。 | 7.3、8.2—8.7、B.7 |
...
| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-SAFE-01 | 产品应以失效优先停车为运动安全验收准则，并区分软件撤销驱动、故障锁定和仍带电的机构状态。 | P0 | 覆盖 FR-MOTION 各失效场景，记录驱动输出状态；停距及软件启动前电气保护不计入现有通过结论。 | 5.13、11.1—11.11、D.9 |

**表 3-17 NFR-SAFE 需求及验收口径。**
...
| NFR-SEC-03 | 系统应通过受控配置管理凭据，家属端常规状态仅返回凭据存在性摘要；正式文档、示例及交付证据不得包含真实个人信息或密钥。 | P0 | 检查配置权限与常规状态、连接弹窗清空行为；抽查提交材料以占位符代替真实凭据及个人路径。 | 10.7、10.9、B.6 |
| NFR-SEC-04 | 系统应将有限对话上下文及当前媒体、视觉、运动会话作为运行态管理，与持久提醒和照护记录区分。 | P1 | 清空或重启后不恢复旧对话和控制会话；本地提醒与真实照护记录仍按持久化规则保留。 | 10.6—10.8、B.6 |

**表 3-18 NFR-SEC 需求及验收口径。**

#### 3.11.5 可维护性与可扩展性

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-MAINT-01 | 系统应通过明确能力契约隔离外部服务和设备，使替换已支持 Provider 或适配器时不必改写提醒等核心业务规则。 | P1 | 采用不同已支持 Provider 组合运行同一业务测试；检查页面不绑定厂商协议。 | 4.6、7.1、7.5—7.6、10.4 |
| NFR-MAINT-02 | 系统应独立配置模型、服务及设备参数，并明确四工程和业务模块职责，使同一业务规则可被本地及远程入口复用。 | P1 | 核对配置覆盖规则、模块接口与部署说明；改变受支持配置无需修改页面业务规则。 | 4.7—4.8、7.1—7.9、10.9、B.6 |
| NFR-MAINT-03 | 系统应提供启动、能力故障和关键会话状态的诊断信息，使维护者能够区分网络、媒体、模型及运动故障。 | P1 | 逐类故障可定位到能力与原因；运动结合故障标志及首次故障记录，不仅凭最近停车原因推断。 | 6.9、7.5、7.10、11.7、B.7 |

**表 3-19 NFR-MAINT 需求及验收口径。**

#### 3.11.6 可复现性与可验证性

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| NFR-REPRO-01 | 交付软件应具有可定位的源码、构建配置、工具链和版本基线，使部署产物能够追溯到对应输入。 | P0 | 核对第 6 章基线、各工程源码与部署材料对应关系；未记录的输入不得宣称可复现。 | 4.7、6.3—6.4、6.8、10.9 |
| NFR-REPRO-02 | 交付模型应明确正式和实验版本，保留模型清单、训练及导出证据与完整性校验值。 | P0 | 核对 V1.2 正式模型、V1.3 实验模型及交接清单；校验文件与部署选择一致，不混用评测结果。 | 9.3、10.8 |
| NFR-REPRO-03 | Hybrid Rootfs 交付应可核对组件清单、依赖及受保护基础文件，并具有产物完整性证据。 | P0 | 检查第 6.8 节清单、ABI/ISA、基础文件及 hash 校验结果；不要求未经验证的跨主机逐字节相同构建。 | 6.7—6.10 |
| NFR-REPRO-04 | 关键能力应保留测试对象版本、环境、方法与结果，区分软件替身、板端数值及实机闭环证据。 | P0 | 按附录 A 找到既有证据或明确待回填位置；数值推理核对参考结果，不能仅凭可加载判定通过。 | 6.5、6.10、9.3—9.6、11.10 |

**表 3-20 NFR-REPRO 需求及验收口径。**

### 3.12 平台、硬件、网络与资源约束

约束参与验收与版本判断，优先级表示违反约束对当前交付的影响。下列规格和已测资源值不等同于额外性能保证。

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| CON-PLAT-01 | 交付应适配单核约 1 GHz 的 LoongArch64 目标平台及当前 ABI，不得依赖 LSX/LASX 指令。 | P0 | 按第 6 章检查产物架构、指令和板端运行；平台不支持的二进制不得列为可部署。 | 5.2、6.1、6.5、6.8 |
| CON-PLAT-02 | 功能配置应适应当前有限内存、存储及无 swap 的已测环境，不以全时高帧率大型视觉模型或完整本地 LLM 为交付前提。 | P0 | 以 512 MB 物理内存及第 6.1 节实际可见内存工况核查部署；按第 9.6 节核对受测组合。 | 5.2、5.10、6.1、9.6 |

**表 3-21 CON-PLAT 需求及验收口径。**

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| CON-HW-01 | 视觉交互应适应装配后固定镜头视角和单自由度左右头部，不假设具备独立俯仰、深度相机或 LiDAR。 | P0 | 核对实物与 D.5、D.7；仅承诺可见人物及左右关注，人物框大小不作为真实测距或碰撞传感器。 | 5.4、5.9、11.9、D.5、D.7 |
| CON-HW-02 | 当前运动应限制在有人看护的低速原型环境，避开障碍、悬崖及不适合跟随的区域；不将未开放横移、导航或量产电源/BMS 计入验收。 | P0 | 按第 11 章实机条件验收已开放动作；核对分路供电与软硬件保护边界。 | 5.8、5.10、5.13、11.11、D.8—D.9 |
| CON-HW-03 | 离线提醒验收应以供电、应用、存储及有效本机时间为前提，不假设未经复测的 RTC 断电保持可保证重启后的绝对时间正确。 | P0 | 记录测试时钟来源及重启后时间；RTC 读写结果与断电保持结果分列，未验证项不作为提醒精度证据。 | 6.9、8.3、D.1 |

**表 3-22 CON-HW 需求及验收口径。**

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| CON-NET-01 | 在线 AI、天气及家庭通话和远程管理应以相应服务可达为前提；外网或家庭链路失效时分别降级，本地核心业务仍按自身条件运行。 | P0 | 分别断开外部服务和家庭连接，核对可用性提示及本地提醒；不得承诺离线远程通话。 | 4.5、7.5、8.4、8.6、B.7 |
| CON-NET-02 | 当前家庭 HTTP/WS 通道应限于受控局域网，不直接暴露公网；跨不可信网络所需加密或可信中转不计入现有内置能力。 | P0 | 核对监听、令牌与网络暴露范围；现有鉴权通过不等同于传输加密通过。 | 4.5、10.9、11.8 |

**表 3-23 CON-NET 需求及验收口径。**

| ID | 需求 | 优先级 | 验收口径 | 对应设计章节 |
| --- | --- | --- | --- | --- |
| CON-RES-01 | 需求验收应考虑 KWS、Vision、Media 共享 CPU 和设备的竞争，不要求通话期间持续语音唤醒、持续视觉推理或自动跟随。 | P0 | 按 FR-MM-03—04 验证让渡和恢复；分别记录单模块与并发工况，不混用性能数据。 | 6.1、7.10、9.6 |

**表 3-24 CON-RES 需求及验收口径。**

### 3.13 需求验收口径与追踪方式

验收按“Requirement ID → 设计章节 → 实现模块 → 验证方式 → 证据结论”建立链路。第 3 章定义判据，附录 A 保存映射与状态，综合测试报告后续回填正式用例编号及结果，不通过修改需求顺序重编 ID。
...
**表 3-25 需求验证证据类型。** 同一需求可组合多类证据，但不得将列出的候选方法全视为已执行。

每项测试至少记录需求 ID、软件/固件/模型版本、设备与网络前提、步骤或脚本入口、期望结果、实际结果、通过/失败/跳过结论及证据位置。网络失败、过期目标、取消回调、错误鉴权和运动指令失效等反向场景应与正常场景同时关联；同一测试可覆盖多条需求，需求也可由多项测试共同证明。

附录 A 使用 VERIFIED（已有可定位的确认或验证结论）、IMPLEMENTED（设计/实现可定位，所列验收范围的证据尚待补齐）、PLANNED（未实现，不进入当前正式基线统计）。VERIFIED 只在引用材料明确覆盖的版本与条件内成立；软件通过不能外推为实机通过，约束的 DOC 核对通过也不能外推为产品认证。不得因综合报告尚未编号或旧报告曾标待测，将后续已确认完成的能力降级；同样不得因核心功能已完成而替适老用户研究、隐私审计、RTC 断电保持或精确停距补写“已通过”。

综合测试报告未完成时统一填“待综合测试报告编号”；已有运动报告编号保留原编号和报告名，现存软件测试保留真实文件或方法名，不编造 TC 编号。若需求、设计和证据出现冲突，记录差异和影响范围，待确认修订后再更新基线；本轮不修改第 4—11 章。

本轮共建立 74 条当前基线条目：FR 45 条、NFR 21 条、CON 8 条；P0 53 条、P1 19 条、P2 2 条。各 ID 的定义与附录 A 逐条对应，未来能力不计入上述数量。
...
本附录对应 REQ-BASE-2026-09-15。每个 Requirement ID 独立一行，覆盖本章全部当前条目；设计章节指向本文件，实现位置按 LongPet、Family Desktop、Motion MCU、Buildroot/LoongArch 和 Vision Handoff 工程别名定位，不包含个人账号或本机个人路径。

VERIFIED 表示已有引用材料支撑对应范围；IMPLEMENTED 表示实现可定位但该验收范围仍需补充专项证据；PLANNED 保留为后续状态，本轮没有将未来能力纳入正式需求。验证方式列列出已有证据及后续可用方法，具体已通过层级以证据索引和原报告为准。本轮是需求及证据整理，没有重新执行产品软件、板端或实机测试。

#### A.2 现有证据定位索引

下列 E- 标识仅是本附录的证据引用简称，**不是新增测试用例 ID**。工程文件均采用工程内相对位置，测试报告引用列中的简称须与本表一起使用。

| 引用简称 | 现有材料与定位 | 证据范围 |
| --- | --- | --- |
| E-APP | LongPet：`tests/V02Test.cpp`；`docs/LongPet-Vision-V2.0-Detector-Tracker-Report.md` 第 10 节完整软件回归；本设计 7.2—7.6、8.2—8.4、10.6 | V02 软件测试通过记录；覆盖提醒持久化、重复调度、关怀设置、接口、页面与语音状态。具体以测试方法断言为准，非统一实机验收报告 |
| E-WEATHER | LongPet：`docs/LongPet-Weather-Feature-Report.md` 第 3、6 节；`tests/V02Test.cpp` 的 `weatherServiceRefreshesUpdatesAndDegrades`、`voiceInteractionInjectsWeatherContext` | 天气软件测试及已记录板端问题修复；含快照降级、时效和语音上下文，不证明状态栏已有过期标记 |
| E-VOICE | LongPet：`docs/LongPet-Network-AI-Voice-V3-Report.md` 第 11 节；`tests/V02Test.cpp` 的 `voiceInteractionCompletesAndRetainsContext`、`voiceInteractionFailuresCancelAndRecover`、`v3ToolValidationAndPendingCancellation`、`v3CallsWaitForKwsBeforeAudio`；本设计 8.4、9.2 | 软件会话、工具权限、离线分流与媒体交接验证；本地声学及板端体验按已确认设计范围，综合 E2E 证据编号待补，历史报告待上板不覆盖后续确认 |
| E-CALL | LongPet：`docs/LongPet-LAN-Bidirectional-Voice-Video-Call-Report.md` 第 9 节；`tests/V02Test.cpp` 的 `videoCallStateAndApiTransition`、`mediaFrameProtocolAndIncomingCallLifecycle`；Family：`tests/video-call-media-protocol.test.js`；本设计 8.6 | 已有双端软件测试与当前双向通话设计；实机通话体验以当前交付确认范围为准，软件替身不证明真实音画质量 |
| E-FAMILY | LongPet：`tests/V02Test.cpp` 的 `familyLinkApiReadsAndWritesServiceData`；E-APP 回归结果；Family：`tests/family-link-service.test.js`、`tests/http-family-link-adapter.test.js`；本设计 8.7、B.3 | 服务与接口状态、读写和冲突验证；UI 操作及跨端结果可按当前流程复核 |
| E-VISION | LongPet：`docs/LongPet-Vision-V2.0-Detector-Tracker-Report.md` 第 10、12 节；`tests/VisionV1Test.cpp`；本设计 9.4—9.6 | 软件状态及最新帧处理测试、板端真人移动和 KWS 并发结果；不扩展为全场景识别保证 |
| E-VIEW | LongPet：`docs/LongPet-Vision-V2.1-Family-AI-View-Report.md` 第 11 节及补充验证；`tests/VisionV1Test.cpp` 的 `familyAiViewSharesCameraWithoutPausingVision`、`familyAiViewProtocolSerializesSafeNormalizedTargets`、`familyAiViewWebSocketRequiresEphemeralAuthentication`；Family：`tests/vision-monitor-protocol.test.js` | 软件视野共享、目标失效与鉴权通过；本设计 8.5 记录当前产品行为，历史板端待测不用于降级当前确认能力 |
| E-MOTION-SW | LongPet：`tests/MotionV1Test.cpp`；`docs/LongPet-Vision-V2.3-Head-Body-Person-Following-Report.md`；Family：`tests/motion-control-adapter.test.js`；本设计 11.8—11.10 | 软件人工接管、跟随抢占及不自动恢复、远控会话；实机跟随结果优先采用 E-MOT |
| E-MOT | [《LongPet 产品运动功能测试报告》V1.3](LongPet_产品运动功能测试报告_2026-09-13_V1.3.docx)，2026-09-13；正文 5.5—5.6、附录 A；本设计表 11-3 | 本设计已确认 58 项中 56 项通过，15、16 号横移暂缓，待验证 0；矩阵保留原报告编号，不创建替代编号 |
| E-MODEL | Vision Handoff：`FINAL_STATUS.md`、`10_reports/final/LongPet-Vision-Final-Test-Report.md`、`10_reports/final/LongPet-Vision-Training-Reproduction-Guide.md`、`03_models/model_manifest.csv` | V1.2 正式与 V1.3 实验定位、冻结/新域评测、模型 hash、源码及复现材料；最终冻结未重跑板端 benchmark |
| E-PLATFORM | 本设计 6.5.3、6.8—6.10、表 6-2；Buildroot/LoongArch：Hybrid Rootfs 组件清单和校验资产；LongPet：`deploy/longpet.service` | 已确认构建、算子数值、合并校验、启动与服务恢复；综合测试报告的正式编号及原始板端日志定位待回填，不降级已有板端通过结论 |

#### A.3 当前需求追踪矩阵

**FR-UI**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-UI-01 | 触摸首页与常用导航 | 7.2—7.3、8.2 | LongPet：MainWindow、AppController、页面及 SettingsService | UT；E2E 操作检查 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-UI-02 | 陪伴与业务状态反馈 | 7.2、8.2、10.10 | LongPet：MainWindow、AppController、页面及 SettingsService | UT；E2E 操作检查 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-UI-03 | 必要本机设置 | 7.4、8.2、10.6 | LongPet：MainWindow、AppController、页面及 SettingsService | UT；E2E 操作检查 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-UI-04 | 触摸与语音互补 | 7.6、8.2、8.4、8.6 | LongPet：MainWindow、AppController、页面及 SettingsService | UT；E2E 操作检查 | VERIFIED | E-APP；待综合测试报告编号 |

**FR-REM**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-REM-01 | 统一提醒入口 | 7.2—7.4、8.3—8.4、10.6 | LongPet：ReminderService、ReminderRepository；Family Desktop 提醒管理 | UT；ST；E2E 三入口联调 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-REM-02 | 提醒编辑与删除 | 8.3—8.4、10.3、10.10 | LongPet：ReminderService、ReminderRepository；Family Desktop 提醒管理 | UT；ST；E2E 三入口联调 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-REM-03 | 到时触发与完成确认 | 7.4、8.3、10.6 | LongPet：ReminderService、ReminderRepository；Family Desktop 提醒管理 | UT；ST；E2E 三入口联调 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-REM-04 | 提醒持久化与断网运行 | 4.1、7.4、8.3、10.6 | LongPet：ReminderService、ReminderRepository；Family Desktop 提醒管理 | UT；ST；E2E 三入口联调 | VERIFIED | E-APP；待综合测试报告编号 |

**FR-CARE**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-CARE-01 | 真实照护事件汇总 | 7.4、8.3、10.6 | LongPet：CareService、CareEventRepository；Family Desktop 关怀摘要 | UT；E2E 数据核对 | VERIFIED | E-APP；待综合测试报告编号 |
| FR-CARE-02 | 关怀信息真实性 | 8.3、8.8、10.6 | LongPet：CareService、CareEventRepository；Family Desktop 关怀摘要 | UT；E2E 数据核对 | IMPLEMENTED | 设计依据：8.3、8.8；展示与事件来源专项验收待归档；待综合测试报告编号 |

**FR-WEATHER**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-WEATHER-01 | 天气获取与失败降级 | 7.4—7.5、8.3、10.4 | LongPet：WeatherService、QWeatherProvider、VoiceInteractionService、状态栏 | UT；BT 联网/断网检查 | VERIFIED | E-WEATHER；待综合测试报告编号 |
| FR-WEATHER-02 | 天气时效与语音引用 | 8.3、10.10、B.4、B.7 | LongPet：WeatherService、QWeatherProvider、VoiceInteractionService、状态栏 | UT；BT 联网/断网检查 | VERIFIED | E-WEATHER；待综合测试报告编号 |

**FR-VOICE**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-VOICE-01 | 本地唤醒与快捷动作 | 7.6、8.4、9.2 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-02 | 在线语音问答 | 7.6、8.4、10.4 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-03 | 有限工具操作 | 7.6、8.4、10.4 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-04 | 有限会话上下文 | 8.4、10.7 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-05 | 离线基础陪伴 | 7.5—7.6、8.4、9.2 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-06 | 停止取消与结果隔离 | 7.6、7.10、8.4、10.10 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |
| FR-VOICE-07 | 语音失败反馈 | 7.5—7.6、8.4、B.7 | LongPet：VoiceCommandDispatcher、VoiceInteractionService、LocalCompanionService、VoiceToolRegistry、KwsProcessAdapter | UT/ST；BT/E2E 声学与联网链路 | VERIFIED | E-VOICE；待综合测试报告编号 |

**FR-VISION**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-VISION-01 | 人物发现 | 8.5、9.3—9.4 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-MODEL；E-VISION；待综合测试报告编号 |
| FR-VISION-02 | 持续位置观测 | 7.7、9.4—9.5、10.8 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-VISION；待综合测试报告编号 |
| FR-VISION-03 | 仅头部自动关注 | 8.5、11.4、11.9 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-MOT：附录 A 28—30、46—47；E-VISION；待综合测试报告编号 |
| FR-VISION-04 | 家属 AI 视野 | 7.7、8.5、B.3、B.5 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-VIEW；待综合测试报告编号 |
| FR-VISION-05 | 受限人物跟随 | 8.5、11.9 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-MOT：正文 5.5—5.6、附录 A 48—58；待综合测试报告编号 |
| FR-VISION-06 | 目标失效与重获 | 9.4—9.5、10.10、11.9 | LongPet：VisionService、AutomaticHeadTrackingService、FamilyVisionMonitorService、MotionService；Family Desktop AI 视野 | UT/ST；BT；HT/E2E 受控场景 | VERIFIED | E-VISION；E-MOT：附录 A 46—48、56—58；待综合测试报告编号 |

**FR-CALL**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-CALL-01 | 双向家庭呼叫 | 8.6、10.3、B.3 | LongPet：VideoCallService、VideoCallMediaAdapter；Family Desktop 通话页面及媒体适配器 | UT/ST；E2E 双端通话 | VERIFIED | E-CALL；待综合测试报告编号 |
| FR-CALL-02 | 明确通话状态与挂断 | 8.2、8.6、10.10、B.7 | LongPet：VideoCallService、VideoCallMediaAdapter；Family Desktop 通话页面及媒体适配器 | UT/ST；E2E 双端通话 | VERIFIED | E-CALL；待综合测试报告编号 |
| FR-CALL-03 | 媒体模式与异常退出 | 7.10、8.6、B.3、B.7 | LongPet：VideoCallService、VideoCallMediaAdapter；Family Desktop 通话页面及媒体适配器 | UT/ST；E2E 双端通话 | VERIFIED | E-CALL；待综合测试报告编号 |

**FR-FAMILY**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-FAMILY-01 | 设备状态与管理入口 | 4.3、7.8、8.7、10.3 | Family Desktop：FamilyLink Service、HTTP/媒体/远控 Adapter、Renderer；LongPet：FamilyLinkService | UT/ST；E2E 双端状态及控制 | VERIFIED | E-FAMILY；待综合测试报告编号 |
| FR-FAMILY-02 | 远程写入确认与冲突 | 8.3、8.7、10.3、10.10 | Family Desktop：FamilyLink Service、HTTP/媒体/远控 Adapter、Renderer；LongPet：FamilyLinkService | UT/ST；E2E 双端状态及控制 | VERIFIED | E-FAMILY；待综合测试报告编号 |
| FR-FAMILY-03 | 自动模式管理 | 8.5、8.7、B.3、11.8 | Family Desktop：FamilyLink Service、HTTP/媒体/远控 Adapter、Renderer；LongPet：FamilyLinkService | UT/ST；E2E 双端状态及控制 | VERIFIED | E-MOTION-SW；E-VIEW；待综合测试报告编号 |
| FR-FAMILY-04 | 手动远控与头部调整 | 8.7、11.8、B.3 | Family Desktop：FamilyLink Service、HTTP/媒体/远控 Adapter、Renderer；LongPet：FamilyLinkService | UT/ST；E2E 双端状态及控制 | VERIFIED | E-MOTION-SW；E-MOT：第 11 章所引实机通过结论；待综合测试报告编号 |

**FR-MOTION**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-MOTION-01 | 软件初始化安全 | 5.13、11.3—11.4、D.9 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 1—2、28；待综合测试报告编号 |
| FR-MOTION-02 | 模式隔离与切换停车 | 11.4、11.6 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 28—30、32、49、54—55；待综合测试报告编号 |
| FR-MOTION-03 | 主控失联独立停车 | 11.2、11.6、11.8 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 36、38—39、56—57；待综合测试报告编号 |
| FR-MOTION-04 | 人工命令失效停车 | 11.6、11.8 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 37；待综合测试报告编号 |
| FR-MOTION-05 | 跟随命令失效停车 | 11.6、11.9 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 50—51；待综合测试报告编号 |
| FR-MOTION-06 | 非法指令不产生运动 | 11.5、B.2 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 23—27、49、54；待综合测试报告编号 |
| FR-MOTION-07 | 严重故障禁止自动恢复 | 11.7 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 40—43；待综合测试报告编号 |
| FR-MOTION-08 | 人工接管优先 | 7.9、8.7、11.8 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOTION-SW：人工生命周期及抢占测试；8.5、11.8 已确认设计；待综合测试报告编号 |
| FR-MOTION-09 | 明确停止语义 | 11.4、11.7、11.11、D.9 | LongPet：MotionService、AutomaticHeadTrackingService、EspSerialAdapter；Motion MCU 执行与安全控制；Family Desktop 远控 | UT/ST；HT/E2E，按 E-MOT 受控条件 | VERIFIED | E-MOT：附录 A 33—34、53；待综合测试报告编号 |

**FR-MM**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| FR-MM-01 | 跨入口业务一致性 | 4.9、7.3—7.4、8.3、10.6 | LongPet：VoiceToolRegistry、业务 Service、VisionService、MediaSessionCoordinator、MotionService；Family Desktop | UT/ST；E2E 跨模态切换 | VERIFIED | E-APP；E-VOICE；E-VISION；E-MOTION-SW；待综合测试报告编号 |
| FR-MM-02 | 视觉观测共同使用 | 7.7、9.5、10.8 | LongPet：VoiceToolRegistry、业务 Service、VisionService、MediaSessionCoordinator、MotionService；Family Desktop | UT/ST；E2E 跨模态切换 | VERIFIED | E-APP；E-VOICE；E-VISION；E-MOTION-SW；待综合测试报告编号 |
| FR-MM-03 | 音频独占与监听让渡 | 7.10、8.4、8.6 | LongPet：VoiceToolRegistry、业务 Service、VisionService、MediaSessionCoordinator、MotionService；Family Desktop | UT/ST；E2E 跨模态切换 | VERIFIED | E-APP；E-VOICE；E-VISION；E-MOTION-SW；待综合测试报告编号 |
| FR-MM-04 | 通话期间自动控制让渡与恢复 | 7.10、8.5—8.6、11.8 | LongPet：VoiceToolRegistry、业务 Service、VisionService、MediaSessionCoordinator、MotionService；Family Desktop | UT/ST；E2E 跨模态切换 | VERIFIED | E-APP；E-VOICE；E-VISION；E-MOTION-SW；待综合测试报告编号 |

**NFR-PERF**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-PERF-01 | 本地操作不受网络阻塞 | 7.6、7.10、8.2 | LongPet：异步 Provider、AppController、VisionInferenceThread、媒体协调 | UT/ST；BT 并发与故障工况 | VERIFIED | E-APP；E-VISION；第 9.6 节；待综合测试报告编号 |
| NFR-PERF-02 | 有效观测优先 | 4.1、7.7、9.4—9.6 | LongPet：异步 Provider、AppController、VisionInferenceThread、媒体协调 | UT/ST；BT 并发与故障工况 | VERIFIED | E-APP；E-VISION；第 9.6 节；待综合测试报告编号 |
| NFR-PERF-03 | 并发资源受控 | 6.1、7.10、9.6、11.6 | LongPet：异步 Provider、AppController、VisionInferenceThread、媒体协调 | UT/ST；BT 并发与故障工况 | VERIFIED | E-APP；E-VISION；第 9.6 节；待综合测试报告编号 |

**NFR-STAB**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-STAB-01 | 可选能力独立降级 | 4.1、7.4—7.5、B.7 | LongPet：Application、能力 Service、Adapter；Buildroot/板端 systemd | UT/ST；BT 故障恢复 | VERIFIED | E-APP；E-VISION；E-PLATFORM；待综合测试报告编号 |
| NFR-STAB-02 | 媒体失败可结束并重试 | 7.5、7.10、8.6、B.7 | LongPet：Application、能力 Service、Adapter；Buildroot/板端 systemd | UT/ST；BT 故障恢复 | VERIFIED | E-APP；E-VISION；E-PLATFORM；待综合测试报告编号 |
| NFR-STAB-03 | 服务启动与异常恢复 | 6.9—6.10、7.10、11.6—11.8 | LongPet：Application、能力 Service、Adapter；Buildroot/板端 systemd | UT/ST；BT 故障恢复 | VERIFIED | E-APP；E-VISION；E-PLATFORM；待综合测试报告编号 |
| NFR-STAB-04 | 核心存储失败显式报告 | 7.4—7.5、B.7 | LongPet：Application、能力 Service、Adapter；Buildroot/板端 systemd | UT/ST；BT 故障恢复 | IMPLEMENTED | 设计与代码：Application::initialize、DatabaseManager；故障专项记录待归档；待综合测试报告编号 |

**NFR-USAB**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-USAB-01 | 浅层高频操作 | 4.1、7.2—7.3、8.2、8.7 | LongPet：首页、业务页面、状态栏；Family Desktop 管理入口 | UT 页面检查；E2E 操作走查 | VERIFIED | E-APP；8.2、8.7；待综合测试报告编号 |
| NFR-USAB-02 | 可理解的异常与退出 | 7.3、8.2—8.7、B.7 | LongPet：首页、业务页面、状态栏；Family Desktop 管理入口 | UT 页面检查；E2E 操作走查 | IMPLEMENTED | 设计及界面已实现；适老用户理解性专项记录待归档；待综合测试报告编号 |

**NFR-SAFE**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-SAFE-01 | 失效安全与物理边界 | 5.13、11.1—11.11、D.9 | LongPet 运动仲裁；Motion MCU；原型驱动与供电 | HT；DOC 边界核对 | VERIFIED | E-MOT；表 5-4、D.9；待综合测试报告编号 |

**NFR-SEC**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-SEC-01 | 远程鉴权与用途隔离 | 7.8、10.3、10.9、B.3 | LongPet：FamilyLinkController、实时会话适配器、VoiceToolRegistry、配置仓储；Family Desktop 主进程与连接窗口 | UT/ST；DOC 配置与数据流审查 | VERIFIED | E-APP；E-VOICE；E-VIEW；E-MOTION-SW；待综合测试报告编号 |
| NFR-SEC-02 | 工具权限边界 | 7.6、8.4、10.4 | LongPet：FamilyLinkController、实时会话适配器、VoiceToolRegistry、配置仓储；Family Desktop 主进程与连接窗口 | UT/ST；DOC 配置与数据流审查 | VERIFIED | E-APP；E-VOICE；E-VIEW；E-MOTION-SW；待综合测试报告编号 |
| NFR-SEC-03 | 敏感配置与交付匿名化 | 10.7、10.9、B.6 | LongPet：FamilyLinkController、实时会话适配器、VoiceToolRegistry、配置仓储；Family Desktop 主进程与连接窗口 | UT/ST；DOC 配置与数据流审查 | IMPLEMENTED | 配置和凭据隔离设计已实现；正式发布包权限及全量脱敏审计待归档；待综合测试报告编号 |
| NFR-SEC-04 | 对话与运行数据保留边界 | 10.6—10.8、B.6 | LongPet：FamilyLinkController、实时会话适配器、VoiceToolRegistry、配置仓储；Family Desktop 主进程与连接窗口 | UT/ST；DOC 配置与数据流审查 | VERIFIED | E-APP；E-VOICE；E-VIEW；E-MOTION-SW；待综合测试报告编号 |

**NFR-MAINT**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-MAINT-01 | 外部能力可替换 | 4.6、7.1、7.5—7.6、10.4 | LongPet：业务 Service、Repository、Port/Adapter、Provider 工厂、配置与日志；各工程部署资产 | UT；DOC 接口与部署审查 | VERIFIED | E-APP；E-VOICE；第 7、10 章；待综合测试报告编号 |
| NFR-MAINT-02 | 配置与模块职责清晰 | 4.7—4.8、7.1—7.9、10.9、B.6 | LongPet：业务 Service、Repository、Port/Adapter、Provider 工厂、配置与日志；各工程部署资产 | UT；DOC 接口与部署审查 | VERIFIED | E-APP；E-VOICE；第 7、10 章；待综合测试报告编号 |
| NFR-MAINT-03 | 故障可定位 | 6.9、7.5、7.10、11.7、B.7 | LongPet：业务 Service、Repository、Port/Adapter、Provider 工厂、配置与日志；各工程部署资产 | UT；DOC 接口与部署审查 | IMPLEMENTED | 设计与日志入口已实现；跨模块诊断专项验收待归档；待综合测试报告编号 |

**NFR-REPRO**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| NFR-REPRO-01 | 软件基线可识别 | 4.7、6.3—6.4、6.8、10.9 | Buildroot/LoongArch 构建与 Hybrid Rootfs；LongPet、Family Desktop、Motion MCU；Vision Handoff | DOC；BT 数值及部署复核 | VERIFIED | E-PLATFORM；E-MODEL；待综合测试报告编号 |
| NFR-REPRO-02 | 模型来源与版本可追踪 | 9.3、10.8 | Buildroot/LoongArch 构建与 Hybrid Rootfs；LongPet、Family Desktop、Motion MCU；Vision Handoff | DOC；BT 数值及部署复核 | VERIFIED | E-PLATFORM；E-MODEL；待综合测试报告编号 |
| NFR-REPRO-03 | 系统合并产物可校验 | 6.7—6.10 | Buildroot/LoongArch 构建与 Hybrid Rootfs；LongPet、Family Desktop、Motion MCU；Vision Handoff | DOC；BT 数值及部署复核 | VERIFIED | E-PLATFORM；E-MODEL；待综合测试报告编号 |
| NFR-REPRO-04 | 验证条件与结果可复核 | 6.5、6.10、9.3—9.6、11.10 | Buildroot/LoongArch 构建与 Hybrid Rootfs；LongPet、Family Desktop、Motion MCU；Vision Handoff | DOC；BT 数值及部署复核 | VERIFIED | E-PLATFORM；E-MODEL；待综合测试报告编号 |

**CON-PLAT**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| CON-PLAT-01 | 目标 ISA 与 ABI | 5.2、6.1、6.5、6.8 | Buildroot/LoongArch 构建与板端运行环境 | DOC；BT 兼容性与资源检查 | VERIFIED | E-PLATFORM；待综合测试报告编号 |
| CON-PLAT-02 | 有限内存与存储 | 5.2、5.10、6.1、9.6 | Buildroot/LoongArch 构建与板端运行环境 | DOC；BT 兼容性与资源检查 | VERIFIED | E-PLATFORM；待综合测试报告编号 |

**CON-HW**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| CON-HW-01 | 视觉与头部几何边界 | 5.4、5.9、11.9、D.5、D.7 | 原型主控、摄像头、头部、底盘与供电；LongPet 与 Motion MCU | DOC；HT 已开放功能核对 | VERIFIED | 表 5-4；附录 D；E-MOT；待综合测试报告编号 |
| CON-HW-02 | 受控低速运动环境 | 5.8、5.10、5.13、11.11、D.8—D.9 | 原型主控、摄像头、头部、底盘与供电；LongPet 与 Motion MCU | DOC；HT 已开放功能核对 | VERIFIED | 表 5-4；附录 D；E-MOT；待综合测试报告编号 |
| CON-HW-03 | 提醒时钟与供电前提 | 6.9、8.3、D.1 | 原型主控、摄像头、头部、底盘与供电；LongPet 与 Motion MCU | DOC；HT 已开放功能核对 | VERIFIED | 6.9、D.1：约束已确认，RTC 断电保持验证不在通过范围；待综合测试报告编号 |

**CON-NET**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| CON-NET-01 | 网络可达性前提 | 4.5、7.5、8.4、8.6、B.7 | LongPet 在线 Provider 与 FamilyLink；Family Desktop；部署网络 | DOC；ST/E2E 断网与鉴权 | VERIFIED | 第 4.5、10.9 节；E-APP；待综合测试报告编号 |
| CON-NET-02 | 受控局域网部署 | 4.5、10.9、11.8 | LongPet 在线 Provider 与 FamilyLink；Family Desktop；部署网络 | DOC；ST/E2E 断网与鉴权 | VERIFIED | 第 4.5、10.9 节；E-APP；待综合测试报告编号 |

**CON-RES**

| Requirement ID | 需求摘要 | 设计章节 | 实现模块/工程 | 验证方式 | 当前状态 | 测试报告引用 |
| --- | --- | --- | --- | --- | --- | --- |
| CON-RES-01 | 共享算力与媒体设备 | 6.1、7.10、9.6 | LongPet：KWS、Vision、Camera、MediaSessionCoordinator | UT/ST；BT 并发检查 | VERIFIED | E-VISION；第 7.10、9.6 节；待综合测试报告编号 |

#### A.4 后续回填与覆盖检查

全部条目已建立需求、设计、实现工程和验证方式映射；其中一般功能与非功能项需要在综合测试报告中回填正式用例 ID、执行版本、日志或截图位置。运动条目已有 E-MOT 原编号，综合报告可引用原报告并建立索引，无须改写旧编号。平台项保留第 6 章已确认结果，后续补原始构建/板端证据文件位置；不得用空缺编号否定当前确认结论。

IMPLEMENTED 条目需补充的是其所述专项验收证据，包括关怀未知数据展示、核心存储故障、适老异常提示、敏感配置/发布材料审计和跨模块诊断；这不表示这些业务整体未实现。未来研究、硬件保护及未开放接口不加入当前表，获准纳入新版时另建稳定需求 ID。

跨章覆盖关系为：第 4 章原则由 UI、FAMILY、MM、NFR 与 CON 承接；第 5 章与附录 D 由 UI、MOTION、SAFE、CON-HW 承接；第 6 章由 PERF、STAB、REPRO、CON-PLAT 承接；第 7 章由业务 FR、MM、STAB、MAINT 承接；第 8 章由全部业务 FR 承接；第 9 章由 VOICE、VISION、PERF、REPRO 承接；第 10 章与附录 B 由 REM、FAMILY、MM、SEC、MAINT、REPRO 承接；第 11 章由 MOTION、VISION、SAFE 和 CON 承接。底层诊断、预留接口和算法实现不要求各自派生独立产品需求。

本轮状态汇总：VERIFIED 69 条，IMPLEMENTED 5 条，PLANNED 0 条；共 74 条与第 3 章一一对应。状态表示引用证据范围内的结论，不是本轮新执行的测试结果。
